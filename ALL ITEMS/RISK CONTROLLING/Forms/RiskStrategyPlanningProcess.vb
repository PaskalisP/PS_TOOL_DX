...
Public Class RiskStrategyPlanningProcess
    ...
    Private Sub GridControl2_EmbeddedNavigator_ButtonClick(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.NavigatorButtonClickEventArgs)
        ...
        If e.Button.ButtonType = DevExpress.XtraEditors.NavigatorButtonType.Remove Then
            ...
            If InputType = "M" Then
                If MessageBox.Show("Should the row ID: " & ID_ProcName & " be deleted?", "DELETE MANUAL INPUTS", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub LoadData_btn_Click(sender As Object, e As EventArgs) Handles LoadData_btn.Click
        ...
        If MessageBox.Show("Should the Basic Data for " & Me.RiskDate_lbl.Text & " and for Creation Date " & Me.CreationDate_lbl.Text & " be re-loaded?" & vbNewLine & vbNewLine & "ATTENTION! ALL CURRENT DATA WILL BE DELETED!", "DATA LOAD", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
            Try
    ...
    Private Sub RecalculateEL_UL_GA_btn_Click(sender As Object, e As EventArgs) Handles RecalculateEL_UL_GA_btn.Click
        If MessageBox.Show("Should the Expected Loss, Unexpected Loss and Granularity approach be recalculated based on the current Data?", "RECALCULATION IN RISK STRATEGY PLANNING PROCESS", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
            rd = Me.RiskDate_lbl.Text
            rdsql = rd.ToString("yyyyMMdd")
            cd = Me.CreationDate_lbl.Text
            cdsql = cd.ToString("yyyyMMdd")
...
e.Graph.DrawString(reportfooter, New RectangleF(0, 0, e.Graph.ClientPageSize.Width, 20))
    ...
    Private Sub LevelOfConfidence_SpinEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles LevelOfConfidence_SpinEdit.ButtonClick
        ...
        If e.Button.Tag = "ChangeStandardValue" Then
            If MessageBox.Show("Should the LEVEL OF CONFIDENCE default value be changed to " & Me.LevelOfConfidence_SpinEdit.Text, "CHANGE DEFAULT VALUE", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                Try
    ...
    Private Sub nu_Value_SpinEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles nu_Value_SpinEdit.ButtonClick
        If e.Button.Tag = "nuChangeStandardValue" Then
            If MessageBox.Show("Should the nu default value be changed to " & Me.nu_Value_SpinEdit.Text, "CHANGE NU DEFAULT VALUE", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                Try
    ...
    Private Sub p_alpha_value_SpinEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles p_alpha_value_SpinEdit.ButtonClick
        If e.Button.Tag = "pAlphaChangeStandardValue" Then
            If MessageBox.Show("Should the p Alpha default value be changed to " & Me.p_alpha_value_SpinEdit.Text, "CHANGE P ALPHA DEFAULT VALUE", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                Try
    ...
    Private Sub b_beta_value_SpinEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles b_beta_value_SpinEdit.ButtonClick
        If e.Button.Tag = "bBetaChangeStandardValue" Then
            If MessageBox.Show("Should the b Beta default value be changed to " & Me.b_beta_value_SpinEdit.Text, "CHANGE B BETA DEFAULT VALUE", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                Try
    ...
    Private Sub BgwExcelLoad_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgwExcelLoad.DoWork
        ...
        If dt1.Rows.Count > 0 Then
            ...
            Dim RangeDETAILS_L As Range = worksheet1.Range("L3:L" & DETAILS_LastRow)
            RangeDETAILS_L.Formula = "=G3*J3*K3"
            Dim RangeDETAILS_M As Range = worksheet1.Range("M3:M" & DETAILS_LastRow)
            RangeDETAILS_M.Formula = "=H3*J3*K3"
            Dim RangeDETAILS_Q As Range = worksheet1.Range("Q3:Q" & DETAILS_LastRow)
            ...
            Dim RangeDETAILS_R As Range = worksheet1.Range("R3:R" & DETAILS_LastRow)
            RangeDETAILS_R.Formula = "=H3*Q3"
            Dim RangeDETAILS_S As Range = worksheet1.Range("S3:S" & DETAILS_LastRow)
        ...
        If dt.Rows.Count > 0 Then
            ...
            Dim RangeUL_Totals_C As Range = worksheet.Range("C12:C" & UL_Totals_LastRow)
            RangeUL_Totals_C.Formula = "=COUNTIF(Details!$O$3:$O$" & DETAILS_LastRow & ";A12)"

            Dim RangeUL_Totals_D As Range = worksheet.Range("D12:D" & UL_Totals_LastRow)
            RangeUL_Totals_D.Formula = "=SUMIF(Details!$O$3:$O$" & DETAILS_LastRow & ";A12;Details!$H$3:$H$" & DETAILS_LastRow & ")"

            Dim RangeUL_Totals_F As Range = worksheet.Range("F12:F" & UL_Totals_LastRow)
            ...
            Dim RangeUL_Totals_G As Range = worksheet.Range("G12:G" & UL_Totals_LastRow)
            RangeUL_Totals_G.Formula = "=MAX(F12;0,0003)*IF(EXACT(0;F12);0;1)"

            Dim RangeUL_Totals_H As Range = worksheet.Range("H12:H" & UL_Totals_LastRow)
            RangeUL_Totals_H.Formula = "=IF(G12-F12>0;1;0)"

            Dim RangeUL_Totals_I As Range = worksheet.Range("I12:I" & UL_Totals_LastRow)
            ...
            Dim RangeUL_Totals_J As Range = worksheet.Range("J12:J" & UL_Totals_LastRow)
            RangeUL_Totals_J.Formula = "=0,12*((1-EXP(-50*G12))/(1-EXP(-50)))+0,24*(1-((1-EXP(-50*G12))/(1-EXP(-50))))"

            Dim RangeUL_Totals_K As Range = worksheet.Range("K12:K" & UL_Totals_LastRow)
            RangeUL_Totals_K.Formula = "=I12"

            Dim RangeUL_Totals_L As Range = worksheet.Range("L12:L" & UL_Totals_LastRow)
            ...
            Dim RangeUL_Totals_M As Range = worksheet.Range("M12:M" & UL_Totals_LastRow)
            ...
            Dim RangeUL_Totals_N As Range = worksheet.Range("N12:N" & UL_Totals_LastRow)
            RangeUL_Totals_N.Formula = "=IF(ISERROR(M12*D12*0,08);0;M12*D12*0,08)"

            Dim RangeUL_Totals_O As Range = worksheet.Range("O12:O" & UL_Totals_LastRow)
            RangeUL_Totals_O.Formula = "=D12*E12*F12"

            Dim RangeUL_Totals_P As Range = worksheet.Range("P12:P" & UL_Totals_LastRow)
            RangeUL_Totals_P.Formula = "=D12*F12*0,45"

            Dim RangeUL_Totals_Q As Range = worksheet.Range("Q12:Q" & UL_Totals_LastRow)
            RangeUL_Totals_Q.Formula = "=D12/SUM($D$12:$D$" & UL_Totals_LastRow & ")"

            Dim RangeUL_Totals_R As Range = worksheet.Range("R12:R" & UL_Totals_LastRow)
            RangeUL_Totals_R.Formula = "=IF(ISERROR(N12/D12);0;N12/D12)"

            Dim RangeUL_Totals_S As Range = worksheet.Range("S12:S" & UL_Totals_LastRow)
            RangeUL_Totals_S.Formula = "=E12*G12"

            Dim RangeUL_Totals_T As Range = worksheet.Range("T12:T" & UL_Totals_LastRow)
            RangeUL_Totals_T.Formula = "=E12"

            Dim RangeUL_Totals_U As Range = worksheet.Range("U12:U" & UL_Totals_LastRow)
            RangeUL_Totals_U.Formula = "=$E$5*T12*(1-T12)"

            Dim RangeUL_Totals_V As Range = worksheet.Range("V12:V" & UL_Totals_LastRow)
            RangeUL_Totals_V.Formula = "=IF(ISERROR((T12^2+U12)/T12);0;(T12^2+U12)/T12)"

            Dim RangeUL_Totals_W As Range = worksheet.Range("W12:W" & UL_Totals_LastRow)
