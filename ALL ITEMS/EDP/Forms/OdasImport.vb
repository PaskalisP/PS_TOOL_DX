...
#Region "BUTTONS_AND_EDITORS_EVENTS"
    Private Sub LastOdasImportFile_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles LastOdasImportFile.ButtonClick
        If IsNumeric(LastOdasImportFile.Text) = True And Len(LastOdasImportFile.Text) = 8 Then
            If MessageBox.Show("Should the ODAS File Nr. " & LastOdasImportFile.Text & " be saved as Last Imported ODAS file", "CHANGE THE LAST IMPORTED ODAS FILE", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub OdasAutomatedImport_btn_Click(sender As Object, e As EventArgs) Handles OdasAutomatedImport_btn.Click
        ...
        If Me.LastOdasImportFile.Text <> "" Then
            If MessageBox.Show("Should the automated ODAS Fileimport be executed?" & vbNewLine & vbNewLine & "ATTENTION!!! - PLEASE SAVE & CLOSE ALL OPEN EXCEL FILES BEFORE START THE IMPORT", "AUTOMATED IMPORT ODAS FILES", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
            ...
            If cmd.ExecuteScalar > 0 Then
                ...
                If File.Exists(ExcelFileName) = True Then
                    ...
                    Try
                        ...
                        'worksheet.DeleteCells(worksheet.Range("M:T"), DeleteMode.ShiftCellsLeft)
                        'worksheet("G:H").NumberFormat = "yyyy-MM-dd"
                        'Me.BgwODASimport.ReportProgress(3, "Changing Column Names-(J1=CURRENCY)-(D1=Contract/Account)")
                        'worksheet.Cells("D1").Value = "Contract/Account"
                        ...
                        Me.BgwODASimport.ReportProgress(21, "Procedure:" & CURRENT_PROC & " - " & "Delete Same data")
                        ...
                        'DETAILS
                        Try
                            Me.BgwODASimport.ReportProgress(22, "Procedure:" & CURRENT_PROC & " - " & "Create temporary Table: #Temp_IMPORT_RATERISK_DETAILS")
                            'Alte Daten im IMPORT DATATABLE löschen
                            cmd.CommandText = "IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='#Temp_IMPORT_RATERISK_DETAILS' AND xtype='U') CREATE TABLE [#Temp_IMPORT_RATERISK_DETAILS]([ID] [int] IDENTITY(1,1) NOT NULL,[Contract Type] [nvarchar](255) NULL,[PERIOD] [nvarchar](255) NULL,[GLMaster / Account Type] [nvarchar](255) NULL,[ProductType] [nvarchar](255) NULL,[Contract/Account] [nvarchar](255) NULL,[Counterparty/Issuer] [nvarchar](255) NULL,[Next EventDate][datetime] NULL,[Next EventType] [nvarchar](255) NULL,[Final Maturity Date] [datetime] NULL,[CURRENCY] [nvarchar](255) NULL,[Type] [nvarchar](255) NULL,[Principal Amount/Value Balance] [float] NULL,[Principal Amount/Value Balance(EUR Equ)] [float] NULL,[RISK DATE][datetime] NULL) ELSE DELETE FROM [#Temp_IMPORT_RATERISK_DETAILS]"
                            cmd.ExecuteNonQuery()
                            ...
                            'Me.BgwODASimport.ReportProgress(23, "INTEREST RATE RISK SSI Result:" & pkgResults.ToString())
                            '******************************************************
                            Me.BgwODASimport.ReportProgress(22, "Procedure:" & CURRENT_PROC & " - " & "Import Data from Excel Sheet where CURRENCY is not NULL")
                            cmd.CommandText = "INSERT INTO [#Temp_IMPORT_RATERISK_DETAILS] ([Contract Type],[GLMaster / Account Type],[ProductType],[Contract/Account],[Counterparty/Issuer],[Next EventDate],[Next EventType],[Final Maturity Date],[CURRENCY],[Type],[Principal Amount/Value Balance],[Principal Amount/Value Balance(EUR Equ)],[RISK DATE])   SELECT [Contract Type],[GLMaster / Account Type],[ProductType],[Contract/Account],[Counterparty/Issuer],[Next EventDate],[Next EventType],[Final Maturity Date],[CURRENCY],[Type],[Principal Amount/Value Balance],[Principal Amount/Value Balance(EUR Equ)],'" & rdsql & "' FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0', 'EXCEL 12.0;HDR=YES;Database=" & ExcelFileName & ";','SELECT [Contract Type],[GLMaster / Account Type],[ProductType],[Contract/Account],[Counterparty/Issuer],[Next EventDate],[Next EventType],[Final Maturity Date],[CURRENCY],[Type],[Principal Amount/Value Balance],[Principal Amount/Value Balance(EUR Equ)] FROM [Sheet1$] where [CURRENCY] is not NULL')"
                            cmd.ExecuteNonQuery()
                            'Count Imported Rows
                            cmd.CommandText = "SELECT * FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0', 'EXCEL 12.0;HDR=YES;Database=" & ExcelFileName & ";','SELECT Count(*) FROM [Sheet1$] where [CURRENCY] is not NULL')"
                            Me.BgwODASimport.ReportProgress(22, "Procedure:" & CURRENT_PROC & " - " & cmd.ExecuteScalar & " rows imported in #Temp_IMPORT_RATERISK_DETAILS")

                            '*********************************************************************************
                            'Insert New Items Data to Table ALL_CONTRACTS_ACCOUNTS
                            Me.BgwODASimport.ReportProgress(22, "Procedure:" & CURRENT_PROC & " - " & "Import New Contracts/Accounts to Table:ALL_CONTRACTS_ACCOUNTS")
                            cmd.CommandText = "INSERT INTO [dbo].[ALL_CONTRACTS_ACCOUNTS] ([Contract_Account]) Select dbo.fn_StripCharacters([Contract/Account],'^0-9') from [#Temp_IMPORT_RATERISK_DETAILS] where  dbo.fn_StripCharacters([Contract/Account],'^0-9') not in (Select [Contract_Account] from [ALL_CONTRACTS_ACCOUNTS]) GROUP BY dbo.fn_StripCharacters([Contract/Account],'^0-9')"
                            cmd.ExecuteNonQuery()
                            '*********************************************************************************

                            Me.BgwODASimport.ReportProgress(22, "Procedure:" & CURRENT_PROC & " - " & "RATERISK DETAILS imported sucessfully")

                            Me.BgwODASimport.ReportProgress(24, "Procedure:" & CURRENT_PROC & " - " & "Insert Data in Table RATERISK DETAILS ALL DATA")
                            'Prüfen ob daten vorhanden sind
                            'cmd.CommandText = "SELECT distinct [RISK DATE] FROM [RATERISK DETAILS] Where [RISK DATE]='" & rdsql & "'"
                            'Dim t As String = cmd.ExecuteScalar()
                            'If IsNothing(t) = False Then
                            ...
                            Try
                                ...
                                cmd.CommandText = "INSERT INTO [RATERISK DETAILS ALL DATA]([CURRENCY],[PERIOD],[Contract Type],[ProductType],[GLMaster / Account Type],[Contract/Account],[Counterparty/Issuer],[Next EventType],[Next EventDate],[Final Maturity Date],[Type],[Principal Amount/Value Balance],[Principal Amount/Value Balance(EUR Equ)],[RISK DATE],[IdRateRiskDate])Select[CURRENCY],[PERIOD],[Contract Type],[ProductType],[GLMaster / Account Type],[Contract/Account],[Counterparty/Issuer],[Next EventType],[Next EventDate],[Final Maturity Date],[Type],[Principal Amount/Value Balance],[Principal Amount/Value Balance(EUR Equ)],[RISK DATE],[RISK DATE] FROM [#Temp_IMPORT_RATERISK_DETAILS]"
                                cmd.ExecuteNonQuery()
                                cmd.CommandText = "Update [RATERISK DETAILS ALL DATA] Set [DaysToEventDate]=DATEDIFF(day,[RISK DATE],[Next EventDate]) where [RISK DATE]='" & rdsql & "' and [Final Maturity Date] is not NULL"
                                cmd.ExecuteNonQuery()
                                Me.BgwODASimport.ReportProgress(24, "Procedure:" & CURRENT_PROC & " - " & "Table:[RATERISK DETAILS ALL DATA] Set correct PERIOD to all contracts based on the Next Event Date for: " & rd)
                                'cmd.CommandText = "UPDATE [RATERISK DETAILS ALL DATA] set PERIOD=Case when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>120 then '> 20 Years' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>84 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=120 then '7 - 10 Years' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>60 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=84 then '5 - 7 Years' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>48 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=60 then '4 - 5 Years' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>36 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=48 then '3 - 4 Years' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>24 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=36 then '2 - 3 Years' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>12 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=24 then '1 - 2 Years' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>6 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=12 then '6 - 12 Months' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>3 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=6 then '3 - 6 Months' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])>1 and DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=3 then '1 - 3 Months' when DATEDIFF(MONTH,'" & rdsql & "',[Next EventDate])<=1  then '<= 1 Month' end where [RISK DATE]='" & rdsql & "' and [Next EventDate] is not NULL"
                                cmd.CommandText = "UPDATE [RATERISK DETAILS ALL DATA] set PERIOD=Case when [DaysToEventDate]>3600 then '> 20 Years' when [DaysToEventDate]>2520 and [DaysToEventDate]<=3600 then '7 - 10 Years' when [DaysToEventDate]>1800 and [DaysToEventDate]<=2520 then '5 - 7 Years' when [DaysToEventDate]>1440 and [DaysToEventDate]<=1800 then '4 - 5 Years' when [DaysToEventDate]>1080 and [DaysToEventDate]<=1440 then '3 - 4 Years' when [DaysToEventDate]>720 and [DaysToEventDate]<=1080 then '2 - 3 Years' when [DaysToEventDate]>360 and [DaysToEventDate]<=720 then '1 - 2 Years' when [DaysToEventDate]>180 and [DaysToEventDate]<=360 then '6 - 12 Months' when [DaysToEventDate]>90 and [DaysToEventDate]<=180 then '3 - 6 Months' when [DaysToEventDate]>30 and [DaysToEventDate]<=90 then '1 - 3 Months' when [DaysToEventDate]<=30  then '<= 1 Month' end where [RISK DATE]='" & rdsql & "'"
                                cmd.ExecuteNonQuery()
                                'löschen der IMPORT Tabelle
                                cmd.CommandText = "DROP TABLE [#Temp_IMPORT_RATERISK_DETAILS]"
                                cmd.ExecuteNonQuery()
                            ...
                            Catch ex As System.Exception
                                Me.BgwODASimport.ReportProgress(24, "ERROR+++ " & "Procedure:" & CURRENT_PROC & " - " & ex.Message.Replace("'", ""))
        ...
        Try
            ...
            If cmd.ExecuteScalar > 0 Then
                ...
                If File.Exists(ExcelFileName) = True Then
                    ...
                    Me.BgwODASimport.ReportProgress(5, "Procedure:" & CURRENT_PROC & " - " & "Create Temporary Table")
                    cmd.CommandText = "IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='#Temp_FINRECON_NG' AND xtype='U') CREATE TABLE [#Temp_FINRECON_NG]([ID] [int] IDENTITY(1,1) NOT NULL,[Entity] [nvarchar](255) NULL,[Source_System] [nvarchar](255) NULL,[Contract_Nr] [nvarchar](255) NULL,[Serial_Nr] [nvarchar](255) NULL,[GL_Number] [nvarchar](255) NULL,[GL_Acc_Name] [nvarchar](255) NULL,[GL_Groups] [nvarchar](255) NULL,[Group_Client_Nr] [nvarchar](255) NULL,[Group_Client_Name] [nvarchar](255) NULL,[Consolidated_Group_Client_Nr] [nvarchar](255) NULL,[Consolidated_Group_Client_Name] [nvarchar](255) NULL,[Client_Nr] [nvarchar](255) NULL,[Client_Name] [nvarchar](255) NULL,[ContractType] [nvarchar](255) NULL,[ProductType] [nvarchar](255) NULL,[ContractType_Definition] [nvarchar](255) NULL,[Master_Nr] [nvarchar](255) NULL,[Accounting_Centre] [nvarchar](255) NULL,[InputDate] [datetime] NULL,[OriginalStartDate] [datetime] NULL,[StartDate] [datetime] NULL,[MaturityDate] [datetime] NULL,[FinalMaturityDate] [datetime] NULL,[LastCouponDate] [datetime] NULL,[NextCouponDate] [datetime] NULL,[InterestRate] [float] NULL,[Amount_ID] [nvarchar](255) NULL,[CCY] [nvarchar](255) NULL,[Amount] [float] NULL,[Amount_Equ] [float] NULL,[Accrued_Interest] [float] NULL,[Accrued_Interest_EUR] [float] NULL,[Dr_Cr_Indicator] [nvarchar](255) NULL,[IS_code] [nvarchar](255) NULL,[Connected_Status] [nvarchar](255) NULL,[ResidenceCountry] [nvarchar](255) NULL,[OriginalCountry] [nvarchar](255) NULL,[RiskCountry] [nvarchar](255) NULL,[CountryRating_RiskCountry] [nvarchar](255) NULL,[RegistrationCountry] [nvarchar](255) NULL,[CityCode] [nvarchar](255) NULL,[ClientType] [nvarchar](255) NULL,[InternalClientType] [nvarchar](255) NULL,[FI_Client_Type] [nvarchar](255) NULL,[ID_Nr_Type2] [nvarchar](255) NULL,[ID_Nr_2] [nvarchar](255) NULL,[Industrial_Classification_Local] [nvarchar](255) NULL,[Industrial_Classification_Others] [nvarchar](255) NULL,[Purpose_of_loan] [nvarchar](255) NULL,[Industrial_Classification_HO] [nvarchar](255) NULL,[Client_Credit_Rating_Agency_1] [nvarchar](255) NULL,[Client_Credit_Rating1] [nvarchar](255) NULL,[Client_Credit_Rating_Agency2] [nvarchar](255) NULL,[Client_Credit_Rating2] [nvarchar](255) NULL,[Loan_Credit_Rating_Long] [nvarchar](255) NULL,[Loan_Credit_Rating_Short] [nvarchar](255) NULL,[Pay_Receive_Indicator] [nvarchar](255) NULL,[Payment_Method] [nvarchar](255) NULL,[Far_Near_Indicator] [nvarchar](255) NULL,[Purchase_Sale] [nvarchar](255) NULL,[Investment_Type] [nvarchar](255) NULL,[Interest_Plan] [nvarchar](255) NULL,[FacilityType] [nvarchar](255) NULL,[DrawStartDate] [datetime] NULL,[DrawEndDate] [datetime] NULL,[Tenor] [nvarchar](255) NULL,[IS_Code_Description] [nvarchar](255) NULL,[Limit_Nr] [nvarchar](255) NULL,[Limit_Code] [nvarchar](255) NULL,[Limit_Type] [nvarchar](255) NULL,[Swap_Market_Value] [float] NULL,[FI_Market_Value] [float] NULL,[FI_Market_Value_EUR] [float] NULL,[Interest_Rate_Type] [nvarchar](255) NULL,[RateCode] [nvarchar](255) NULL,[Spread_Rate] [float] NULL,[Exchange_Rate] [float] NULL,[Original_Maturity] [float] NULL,[Remaining_Maturity] [float] NULL,[Collective_Impairment_Amount] [float] NULL,[Individual_Impairment_Amount] [float] NULL,[Total_Interest_Amount_OrgCCY] [float] NULL,[Total_Interest_AmountEUR] [float] NULL,[Class] [nvarchar](50) NULL,[RiskDate] [datetime] NULL) ELSE DELETE FROM [#Temp_FINRECON_NG]"
                    cmd.ExecuteNonQuery()
                    '******************************************************

                    Me.BgwODASimport.ReportProgress(5, "Procedure:" & CURRENT_PROC & " - " & "Insert Data from Excel File to Temporary Table")
                    cmd.CommandText = "INSERT INTO [#Temp_FINRECON_NG]([Entity],[Source_System],[Contract_Nr],[Serial_Nr],[GL_Number],[GL_Acc_Name],[GL_Groups],[Group_Client_Nr],[Group_Client_Name],[Consolidated_Group_Client_Nr],[Consolidated_Group_Client_Name],[Client_Nr],[Client_Name],[ContractType],[ProductType],[ContractType_Definition],[Master_Nr],[Accounting_Centre],[InputDate],[OriginalStartDate],[StartDate],[MaturityDate],[FinalMaturityDate],[LastCouponDate],[NextCouponDate],[InterestRate],[Amount_ID],[CCY],[Amount],[Amount_Equ],[Accrued_Interest],[Accrued_Interest_EUR],[Dr_Cr_Indicator],[IS_code],[Connected_Status],[ResidenceCountry],[OriginalCountry],[RiskCountry],[CountryRating_RiskCountry],[RegistrationCountry],[CityCode],[ClientType],[InternalClientType],[FI_Client_Type],[ID_Nr_Type2],[ID_Nr_2],[Industrial_Classification_Local],[Industrial_Classification_Others],[Purpose_of_loan],[Industrial_Classification_HO],[Client_Credit_Rating_Agency_1],[Client_Credit_Rating1],[Client_Credit_Rating_Agency2],[Client_Credit_Rating2],[Loan_Credit_Rating_Long],[Loan_Credit_Rating_Short],[Pay_Receive_Indicator],[Payment_Method],[Far_Near_Indicator],[Purchase_Sale],[Investment_Type],[Interest_Plan],[FacilityType],[DrawStartDate],[DrawEndDate],[Tenor],[IS_Code_Description],[Limit_Nr],[Limit_Code],[Limit_Type],[Swap_Market_Value],[FI_Market_Value],[FI_Market_Value_EUR],[Interest_Rate_Type],[RateCode],[Spread_Rate],[Exchange_Rate],[Original_Maturity],[Remaining_Maturity],[Collective_Impairment_Amount],[Individual_Impairment_Amount],[Total_Interest_Amount_OrgCCY],[Total_Interest_AmountEUR],[Class],[RiskDate])  SELECT [Entity],[Source_System],[Contract_Nr],[Serial_Nr],[GL_Number],[GL_Acc_Name],[GL_Groups],[Group_Client_Nr],[Group_Client_Name],[Consolidated_Group_Client_Nr],[Consolidated_Group_Client_Name],[Client_Nr],[Client_Name],[ContractType],[ProductType],[ContractType_Definition],[Master_Nr],[Accounting_Centre],[InputDate],[OriginalStartDate],[StartDate],[MaturityDate],[FinalMaturityDate],[LastCouponDate],[NextCouponDate],[InterestRate],[Amount_ID],[CCY],[Amount],[Amount_Equ],[Accrued_Interest],[Accrued_Interest_EUR],[Dr_Cr_Indicator],[IS_code],[Connected_Status],[ResidenceCountry],[OriginalCountry],[RiskCountry],[CountryRating_RiskCountry],[RegistrationCountry],[CityCode],[ClientType],[InternalClientType],[FI_Client_Type],[ID_Nr_Type2],[ID_Nr_2],[Industrial_Classification_Local],[Industrial_Classification_Others],[Purpose_of_loan],[Industrial_Classification_HO],[Client_Credit_Rating_Agency_1],[Client_Credit_Rating1],[Client_Credit_Rating_Agency2],[Client_Credit_Rating2],[Loan_Credit_Rating_Long],[Loan_Credit_Rating_Short],[Pay_Receive_Indicator],[Payment_Method],[Far_Near_Indicator],[Purchase_Sale],[Investment_Type],[Interest_Plan],[FacilityType],[DrawStartDate],[DrawEndDate],[Tenor],[IS_Code_Description],[Limit_Nr],[Limit_Code],[Limit_Type],[Swap_Market_Value],[FI_Market_Value],[FI_Market_Value_EUR],[Interest_Rate_Type],[RateCode],[Spread_Rate],[Exchange_Rate],[Original_Maturity],[Remaining_Maturity],[Collective_Impairment_Amount],[Individual_Impairment_Amount],[Total_Interest_Amount_OrgCCY],[Total_Interest_AmountEUR],[Class],'" & rdsql & "' FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0', 'EXCEL 12.0;HDR=YES;Database=" & ExcelFileName & ";','SELECT * FROM [Sheet1$]')"
                    cmd.ExecuteNonQuery()
                    Me.BgwODASimport.ReportProgress(5, "Procedure:" & CURRENT_PROC & " - " & "Delete data from Temporary Table where Source_System is NULL")
                    cmd.CommandText = "DELETE FROM [#Temp_FINRECON_NG] where [Source_System] is NULL"
                    cmd.ExecuteNonQuery()
                    'Count Imported Rows
                    Me.BgwODASimport.ReportProgress(5, "Procedure:" & CURRENT_PROC & " - " & "Count Imported rows from Excel File")
                    cmd.CommandText = "SELECT * FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0', 'EXCEL 12.0;HDR=YES;Database=" & ExcelFileName & ";','SELECT Count(*) FROM [Sheet1$]')"
                    Me.BgwODASimport.ReportProgress(5, "Procedure:" & CURRENT_PROC & " - " & cmd.ExecuteScalar & " rows imported in #Temp_FINRECON_NG")

                    Me.BgwODASimport.ReportProgress(5, "Procedure:" & CURRENT_PROC & " - " & "FINANCIAL RECONCILIATION DETAILS imported in Temporary Table")

                    Me.BgwODASimport.ReportProgress(6, "Procedure:" & CURRENT_PROC & " - " & "Delete Current Data from FINRECON for " & rd)
                    cmd.CommandText = "DELETE FROM [FINRECON_NG] where [RiskDate]='" & rdsql & "'"
                    cmd.ExecuteNonQuery()

                    Me.BgwODASimport.ReportProgress(7, "Procedure:" & CURRENT_PROC & " - " & "Check if Data are allready imported for the specific Date")
                    cmd.CommandText = "SELECT COUNT(ID) from [FINRECON_NG] where [RiskDate]='" & rdsql & "'"
                    Dim FINRECON_COUNT As Double = cmd.ExecuteScalar

                    If FINRECON_COUNT = 0 Then
                        Me.BgwODASimport.ReportProgress(8, "Procedure:" & CURRENT_PROC & " - " & "Insert into FINRECON_NG")
                        cmd.CommandText = "INSERT INTO [FINRECON_NG]([Entity],[Source_System],[Contract_Nr],[Serial_Nr],[GL_Number],[GL_Acc_Name],[GL_Groups],[Group_Client_Nr],[Group_Client_Name],[Consolidated_Group_Client_Nr],[Consolidated_Group_Client_Name],[Client_Nr],[Client_Name],[ContractType],[ProductType],[ContractType_Definition],[Master_Nr],[Accounting_Centre],[InputDate],[OriginalStartDate],[StartDate],[MaturityDate],[FinalMaturityDate],[LastCouponDate],[NextCouponDate],[InterestRate],[Amount_ID],[CCY],[Amount],[Amount_Equ],[Accrued_Interest],[Accrued_Interest_EUR],[Dr_Cr_Indicator],[IS_code],[Connected_Status],[ResidenceCountry],[OriginalCountry],[RiskCountry],[CountryRating_RiskCountry],[RegistrationCountry],[CityCode],[ClientType],[InternalClientType],[FI_Client_Type],[ID_Nr_Type2],[ID_Nr_2],[Industrial_Classification_Local],[Industrial_Classification_Others],[Purpose_of_loan],[Industrial_Classification_HO],[Client_Credit_Rating_Agency_1],[Client_Credit_Rating1],[Client_Credit_Rating_Agency2],[Client_Credit_Rating2],[Loan_Credit_Rating_Long],[Loan_Credit_Rating_Short],[Pay_Receive_Indicator],[Payment_Method],[Far_Near_Indicator],[Purchase_Sale],[Investment_Type],[Interest_Plan],[FacilityType],[DrawStartDate],[DrawEndDate],[Tenor],[IS_Code_Description],[Limit_Nr],[Limit_Code],[Limit_Type],[Swap_Market_Value],[FI_Market_Value],[FI_Market_Value_EUR],[Interest_Rate_Type],[RateCode],[Spread_Rate],[Exchange_Rate],[Original_Maturity],[Remaining_Maturity],[Collective_Impairment_Amount],[Individual_Impairment_Amount],[Total_Interest_Amount_OrgCCY],[Total_Interest_AmountEUR],[Class],[RiskDate]) SELECT [Entity],[Source_System],[Contract_Nr],[Serial_Nr],[GL_Number],[GL_Acc_Name],[GL_Groups],[Group_Client_Nr],[Group_Client_Name],[Consolidated_Group_Client_Nr],[Consolidated_Group_Client_Name],[Client_Nr],[Client_Name],[ContractType],[ProductType],[ContractType_Definition],[Master_Nr],[Accounting_Centre],[InputDate],[OriginalStartDate],[StartDate],[MaturityDate],[FinalMaturityDate],[LastCouponDate],[NextCouponDate],[InterestRate],[Amount_ID],[CCY],[Amount],[Amount_Equ],[Accrued_Interest],[Accrued_Interest_EUR],[Dr_Cr_Indicator],[IS_code],[Connected_Status],[ResidenceCountry],[OriginalCountry],[RiskCountry],[CountryRating_RiskCountry],[RegistrationCountry],[CityCode],[ClientType],[InternalClientType],[FI_Client_Type],[ID_Nr_Type2],[ID_Nr_2],[Industrial_Classification_Local],[Industrial_Classification_Others],[Purpose_of_loan],[Industrial_Classification_HO],[Client_Credit_Rating_Agency_1],[Client_Credit_Rating1],[Client_Credit_Rating_Agency2],[Client_Credit_Rating2],[Loan_Credit_Rating_Long],[Loan_Credit_Rating_Short],[Pay_Receive_Indicator],[Payment_Method],[Far_Near_Indicator],[Purchase_Sale],[Investment_Type],[Interest_Plan],[FacilityType],[DrawStartDate],[DrawEndDate],[Tenor],[IS_Code_Description],[Limit_Nr],[Limit_Code],[Limit_Type],[Swap_Market_Value],[FI_Market_Value],[FI_Market_Value_EUR],[Interest_Rate_Type],[RateCode],[Spread_Rate],[Exchange_Rate],[Original_Maturity],[Remaining_Maturity],[Collective_Impairment_Amount],[Individual_Impairment_Amount],[Total_Interest_Amount_OrgCCY],[Total_Interest_AmountEUR],[Class],[RiskDate] FROM [#Temp_FINRECON_NG]"
                        cmd.ExecuteNonQuery()

                        'löschen der IMPORT Tabelle
                        cmd.CommandText = "DROP TABLE [#Temp_FINRECON_NG]"
                        cmd.ExecuteNonQuery()
                    ...
                    Else
                        Me.BgwODASimport.ReportProgress(7, "Procedure:" & CURRENT_PROC & " - " & "FINRECON Data allready imported for " & rd)
                        cmd.CommandText = "DROP TABLE [#Temp_FINRECON_NG]"
                        cmd.ExecuteNonQuery()
                    End If
