...
#End Region
    ...
    Private Sub NACE_3_CODES_UPDATE()
        CurrentClientProcedure = "NUTS_3_CODES_UPDATE"
        cmd.CommandText = "SELECT [Status] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('NUTS_3_CODES_UPDATE') and [Id_SQL_Parameters] in ('PS TOOL CLIENT PROCEDURES')"
        Dim ParameterStatus As String = cmd.ExecuteScalar
        If ParameterStatus = "Y" Then
            Me.BgwClientRun.ReportProgress(1, "Start NUTS 3 Codes Update")
         
            'Anwenden der SQL Befehle
            Me.BgwClientRun.ReportProgress(3, "Execute SQL Commands for NUTS_3_CODES_UPDATE")
            Me.QueryText = "Select * from [SQL_PARAMETER_DETAILS_SECOND]  where [Id_SQL_Parameters_Details] in (Select [ID] from SQL_PARAMETER_DETAILS where SQL_Name_1 in ('NUTS_3_CODES_UPDATE')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
            da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
            For i = 0 To dt.Rows.Count - 1
                ...
                cmd.CommandText = SqlCommandText
                If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                    Me.BgwClientRun.ReportProgress(3, "Execute SQL Command: " & dt.Rows.Item(i).Item("SQL_Name_1"))
                    cmd.ExecuteNonQuery()
                End If
            Next

           
            Me.BgwClientRun.ReportProgress(5, "NUTS_3_CODES_UPDATE is finished")
        Else
            Me.BgwClientRun.ReportProgress(5, "NUTS_3_CODES_UPDATE is Invalid")
        End If
    End Sub 'ONLY IN APP
