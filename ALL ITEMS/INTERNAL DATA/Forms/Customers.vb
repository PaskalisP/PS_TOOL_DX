...
Public Class Customers
    ...
    Dim CustomerContract As New AllContractsAccounts()
        ...
        Try
            ...
            If CustomerNewAdd.ClientNr_TextEdit.Text <> "" AndAlso CustomerNewAdd.OPICS_Client_Code_TextEdit.Text <> "" AndAlso CustomerNewAdd.OPICS_Client_Nr_TextEdit.Text <> "" AndAlso CustomerNewAdd.ClientName_TextEdit.Text <> "" AndAlso CustomerNewAdd.BIC_CODE_TextEdit.Text <> "" AndAlso CustomerNewAdd.BIC_CODE_lbl.Text <> "" AndAlso CustomerNewAdd.CCB_Group_ImageComboBoxEdit.SelectedIndex <> 2 AndAlso CustomerNewAdd.CCB_Group_OwnID_ImageComboBoxEdit.SelectedIndex <> 2 AndAlso CustomerNewAdd.CIC_Group_ImageComboBoxEdit.SelectedIndex <> 2 Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Exit Sub
                Else 'VALIDITY OK
                    If XtraMessageBox.Show("Should the Client Nr.: " & CLIENT_NR & vbNewLine & "Name: " & CustomerNewAdd.ClientName_TextEdit.Text & vbNewLine & "be inserted to the Customer Datatable?", "ADDING NEW CUSTOMER", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                        If cmd.Connection.State = ConnectionState.Closed Then
                            cmd.Connection.Open()
                        End If
                        cmd.CommandText = "INSERT INTO [dbo].[CUSTOMER_INFO]([ClientNo],[ClientType],[English Name],[Chinese Name],[BIC11],[BIC11_NAME],[LEI_CODE],[OpicsCustomerCode],[OpicsCustomerNr],[CCB_Group],[CCB_Group_OwnID],[CIC_Group]) VALUES ('" & CustomerNewAdd.ClientNr_TextEdit.Text & "','F - FINANCIAL','" & CustomerNewAdd.ClientName_TextEdit.Text & "','" & CustomerNewAdd.ClientName_TextEdit.Text & "','" & CustomerNewAdd.BIC_CODE_TextEdit.Text & "','" & CustomerNewAdd.BIC_CODE_lbl.Text & "','" & CustomerNewAdd.LEI_Code_TextEdit.Text & "','" & CustomerNewAdd.OPICS_Client_Code_TextEdit.Text & "','" & CustomerNewAdd.OPICS_Client_Nr_TextEdit.Text & "','" & CCB_Group & "','" & CCB_Group_Own_ID & "','" & CIC_Group & "')"
                        cmd.ExecuteNonQuery()
...
#End Region
        ...
        If Me.LayoutControl1.Visible = False Then
            ...
            If Len(BICCODE) = 11 Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    End If
                    'Get BLZ
                    cmd.CommandText = "SELECT NATIONAL_ID from BIC_DIRECTORY_PLUS where BIC11='" & BICCODE & "' and NATIONAL_ID is not NULL and SUBSTRING(BIC11,5,2) in ('DE')"
    ...
    Private Sub AllCustomerContracts_GridView_DoubleClick(sender As Object, e As EventArgs) Handles AllCustomerContracts_GridView.DoubleClick
        ...
        '    Return
        'Else
        '    Dim Contract As String = Me.AllCustomerContracts_GridView.GetRowCellValue(Me.AllCustomerContracts_GridView.FocusedRowHandle, colContract_Account)
        '    SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
        '    SplashScreenManager.Default.SetWaitFormCaption("Load data for contract " & Contract)

        '    CustomerContract.ALL_CONTRACTS_ACCOUNTSTableAdapter.FillByContractNr(CustomerContract.AllContractsAccountsDataSet.ALL_CONTRACTS_ACCOUNTS, Contract)
        '    CustomerContract.ALL_CONTRACTS_ACCOUNTS_VALUE_ADJUSTTableAdapter.Fill(CustomerContract.AllContractsAccountsDataSet.ALL_CONTRACTS_ACCOUNTS_VALUE_ADJUST)

        '    CustomerContract.LayoutControl1.Visible = False
        '    For Each control As Control In CustomerContract.Controls
        '        If TypeOf control Is DevExpress.XtraEditors.GroupControl Then
        '            control.Enabled = False
        '        End If
        '    Next
        '    CustomerContract.ContractSearch_GridLookUpEdit.Visible = False
        '    CustomerContract.SaveChanges_btn.Visible = False
        '    CustomerContract.Cancel_btn.Visible = False
        '    CustomerContract.KeyPreview = False
        '    CustomerContract.Name = "Contract Nr. " & Contract
        '    CustomerContract.ShowDialog()
