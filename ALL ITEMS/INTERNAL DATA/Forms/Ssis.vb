    ...
    Private Sub Ssis_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Try
            ...
            If Me.PSTOOLDataset.HasChanges = True Then
                If XtraMessageBox.Show("Should the Changes in SSIS be saved?", "SAVE CHANGES", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                    Me.Validate()
                    Me.SSISBindingSource.EndEdit()
                    Me.TableAdapterManager.UpdateAll(Me.PSTOOLDataset)
                End If
...
#Region "SSIS_CHANGE_VIEWS"
    ...
    Protected Sub HideDetail(ByVal rowHandle As Integer)
        ...
        '***********Save Changes****************
        If Me.PSTOOLDataset.HasChanges = True Then
            If XtraMessageBox.Show("Should the Changes in NOSTRO Accounts be saved?", "SAVE CHANGES", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                Me.Validate()
                Me.SSISBindingSource.EndEdit()
                Me.TableAdapterManager.UpdateAll(Me.PSTOOLDataset)
                Me.SSISTableAdapter.Fill(Me.PSTOOLDataset.SSIS)
...
#End Region
    ...
    Private Sub AddNewSSI_btn_Click(sender As Object, e As EventArgs) Handles AddNewSSI_btn.Click
        If Me.Currencies_LookUpEdit.EditValue <> Nothing AndAlso Me.CorrespondentName_lbl.Text <> Nothing Then
            Try
                Me.Validate()
                conn.ConnectionString = My.Settings.PS_TOOL_DX_SQL_Client_ConnectionString
                cmd.Connection = conn
                cmd.Connection.Open()
                cmd.CommandText = "INSERT INTO  [SSIS] ([CURRENCY CODE],[CURRENCY NAME],[INTERNAL ACCOUNT],[CORRESPONDENT BIC],[CORRESPONDENT NAME],[EXTERNAL ACCOUNT],[INTERMEDIARY BIC],[INTERMEDIARY NAME],[VALID])VALUES('" & Me.Currencies_LookUpEdit.Text & " ','" & Me.CurrencyName_lbl.Text & "','" & Me.InternalAccount_txt.Text & "','" & Me.CorrespondentBIC_ButtonEdit.Text & "','" & Me.CorrespondentName_lbl.Text & "','" & Me.ExternalAccount_txt.Text & "','" & Me.IntermediaryBIC_ButtonEdit.Text & "','" & Me.IntermediaryName_lbl.Text & "','Y')"
                cmd.ExecuteNonQuery()
                cmd.CommandText = "UPDATE [SSIS] set [CURRENCY CODE]=RTRIM(LTRIM([CURRENCY CODE]))"
                cmd.ExecuteNonQuery()
                cmd.Connection.Close()
                Me.SSISTableAdapter.Fill(Me.PSTOOLDataset.SSIS)
                XtraMessageBox.Show("The Settlement Instruction has being added to the SSIS", "SSI INSERTED", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
    ...
    Private Sub ViewEdit_btn_Click_1(sender As Object, e As EventArgs) Handles ViewEdit_btn.Click
