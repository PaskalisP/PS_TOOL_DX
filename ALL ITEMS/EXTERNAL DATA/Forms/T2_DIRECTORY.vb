    ...
    Protected Sub HideDetail(ByVal rowHandle As Integer)
        ...
        If Me.EXTERNALDataset.HasChanges = True Then
            ...
            If AddresseeBIC_Value <> "" AndAlso AccountHolderBIC_Value <> "" AndAlso IsDate(Participant_ValidFrom) = True AndAlso IsDate(Participant_ValidTill) = True Then
                ...
                Try
                    If XtraMessageBox.Show("Should the Changes for Participant BIC:" & ParticipantBIC11 & " in T2 Directory be saved?", "SAVE CHANGES", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                        SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                        SplashScreenManager.Default.SetWaitFormCaption("Updating T2 Directory-Please wait")
                        Me.Validate()
                        Me.T2_DIRECTORYBindingSource.EndEdit()
                        Me.TableAdapterManager.UpdateAll(Me.EXTERNALDataset)
                        If XtraMessageBox.Show("Should the Changes for Participant BIC:" & ParticipantBIC11 & " inserted also in " & ParticipantBIC8 & " ?", "SAVE CHANGES in ALL BIC8", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
...
#End Region
    ...
    Private Sub AddNewT2ParticipantAllBranches_btn_Click(sender As Object, e As EventArgs) Handles AddNewT2ParticipantAllBranches_btn.Click
        If Me.ParticipantName_lbl.Text <> Nothing AndAlso Me.AddresseeName_lbl.Text <> Nothing AndAlso Me.AccountholderName_lbl.Text <> Nothing Then
            ...
            da1.Fill(dt1)
            If dt1.Rows.Count = 0 Then
                If XtraMessageBox.Show("Should the BIC: " & BIC8 & " and all of his Branches be added to the T2 Directory?", "INSERT T2 PARTICIPANT and his BRANCHES", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                    Try
                        Dim dsql As String = d.ToString("yyyyMMdd")
                        Dim dtillSql As String = tilld.ToString("yyyyMMdd")
                        Me.Validate()
    ...
    Private Sub FullT2Dir_BarButtonItem_ItemClick(sender As Object, e As ItemClickEventArgs) Handles FullT2Dir_BarButtonItem.ItemClick
        If XtraMessageBox.Show("Should the Full T2 Directory be re-created?" & vbNewLine & vbNewLine & "ATTENTION: DELETED T2 PARTICIPANTS WILL NOT BE INCLUDED!!!" & vbNewLine & vbNewLine & "Existing File will be deleted!", "Full T2 Directory creation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
            Try
                SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                SplashScreenManager.Default.SetWaitFormCaption("Select all Data from T2 Directory")
                If File.Exists("C:\T2DIRFULLVA.ASCII_FullNew.ORIG") = True Then
                    File.Delete("C:\T2DIRFULLVA.ASCII_FullNew.ORIG")
                End If
                ...
                For i = 0 To dt.Rows.Count - 1
                    ...
                    T2_ROW = BIC11 & ADDRESSEE & ACCOUNT_HOLDER & BIC11_NAME & CITY & SORTCODE & MAIN_BIC_FLAG & TYPE_OF_CHANGE & ValidFromSql & ValidTillSql & PARTICIPATION_TYPE & RESERVE

                    System.IO.File.AppendAllText("C:\T2DIRFULLVA.ASCII_FullNew.ORIG", T2_ROW & vbCrLf)
                Next

                SplashScreenManager.CloseForm(False)
                XtraMessageBox.Show("The following T2 Directory file has being created: C:\T2DIRFULLVA.ASCII_FullNew.ORIG", "NEW T2 DIRECTORY FILE", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
    ...
    Private Sub ManualT2Dir_BarButtonItem_ItemClick(sender As Object, e As ItemClickEventArgs) Handles ManualT2Dir_BarButtonItem.ItemClick
        If XtraMessageBox.Show("Should the T2 Directory for Manual inputs be re-created?" & vbNewLine & vbNewLine & "Existing File will be deleted!", "T2 Directory (Manual Inputs) creation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
            Try
                SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                SplashScreenManager.Default.SetWaitFormCaption("Select only manually inputed Data from T2 Directory")
                If File.Exists("C:\T2DIRFULLVA.ASCII_ManInp.ORIG") = True Then
                    File.Delete("C:\T2DIRFULLVA.ASCII_ManInp.ORIG")
                End If
                ...
                For i = 0 To dt.Rows.Count - 1
                    ...
                    System.IO.File.AppendAllText("C:\T2DIRFULLVA.ASCII_ManInp.ORIG", T2_ROW & vbCrLf)
                Next

                SplashScreenManager.CloseForm(False)
                XtraMessageBox.Show("The following T2 Directory file has being created: C:\T2DIRFULLVA.ASCII_ManInp.ORIG", "NEW T2 DIRECTORY FILE", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
