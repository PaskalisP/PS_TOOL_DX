...
Public Class AnaCreditContracts
    ...
    Private Sub AnaCreditContracts_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.QueryText = "Select CONVERT(VARCHAR(10),[RLDC Date],104) as 'RLDC Date' from [RISK LIMIT DAILY CONTROL] where [RLDC Date]>='20170601' and [PL Result] is not NULL ORDER BY [ID] desc"
        ...
        Dim objCMD As SqlCommand = New SqlCommand("Select * from [ANACREDIT_CONTRACTS] where [RiskDate]=' " & dsql & "' ORDER BY CASE [ContractInput] when 'NEW' then 1 when 'CURRENT' then 2 END", conn)
        ...
        Dim objCMD2 As SqlCommand = New SqlCommand("Select 'NEW' as ContractInput,A.ClientNr,A.ClientName,A.CCY,B.Contract_Account,A.StartDate,A.MaturityDate,A.BusinessType,A.Valid from ALL_CONTRACTS_ACCOUNTS A INNER JOIN (Select Contract_Account from ANACREDIT_CONTRACTS where ID in (Select Min(ID) from ANACREDIT_CONTRACTS group by Contract_Account) and [RiskDate] BETWEEN DATEADD(dd,-(DAY('" & dsql & "')-1),'" & dsql & "') and DATEADD(dd,-(DAY(DATEADD(mm,1,'" & dsql & "' ))),DATEADD(mm,1,'" & dsql & "')) and ContractInput in ('NEW') GROUP BY Contract_Account)B On A.Contract_Account=B.Contract_Account UNION ALL Select 'CURRENT' as ContractInput,A.ClientNr,A.ClientName,A.CCY,B.Contract_Account,A.StartDate,A.MaturityDate,A.BusinessType,A.Valid from ALL_CONTRACTS_ACCOUNTS A INNER JOIN (Select Contract_Account from ANACREDIT_CONTRACTS where ID in (Select Max(ID) from ANACREDIT_CONTRACTS group by Contract_Account) and [RiskDate] BETWEEN DATEADD(dd,-(DAY('" & dsql & "')-1),'" & dsql & "') and DATEADD(dd,-(DAY(DATEADD(mm,1,'" & dsql & "' ))),DATEADD(mm,1,'" & dsql & "')) and ContractInput in ('CURRENT') GROUP BY Contract_Account)B On A.Contract_Account=B.Contract_Account", conn)
    ...
    Private Sub PrintableComponentLink1_CreateMarginalHeaderArea(sender As Object, e As CreateAreaEventArgs) Handles PrintableComponentLink1.CreateMarginalHeaderArea
        Dim reportfooter As String = "AnaCredit Contracts + Customers (Daily)" & "   " & "Date: " & Me.FromDateEdit.Text
    ...
    Private Sub PrintableComponentLink3_CreateMarginalHeaderArea(sender As Object, e As CreateAreaEventArgs) Handles PrintableComponentLink3.CreateMarginalHeaderArea
        Dim reportfooter As String = "AnaCredit Contracts + Customers (Monthly)" & "  " & Me.AnaCreditContractsMonthly_GridView.ViewCaption.ToString
        e.Graph.DrawString(reportfooter, New RectangleF(0, 0, e.Graph.ClientPageSize.Width, 20))
    End Sub
    ...
    Private Sub TabbedControlGroup1_SelectedPageChanged(sender As Object, e As DevExpress.XtraLayout.LayoutTabPageChangedEventArgs) Handles TabbedControlGroup1.SelectedPageChanged
        If Me.TabbedControlGroup1.SelectedTabPage.Text = "AnaCredit Contracts/Customers (Daily)" Then
            ActiveTabGroup = 0
    ...
    Private Sub GridView1_ShownEditor(sender As Object, e As EventArgs) Handles GridView1.ShownEditor
        ...
        End If
    End Sub
    Private Sub AnaCreditContractsMonthly_GridView_RowStyle(sender As Object, e As RowStyleEventArgs) Handles AnaCreditContractsMonthly_GridView.RowStyle
        If e.RowHandle = DevExpress.XtraGrid.GridControl.AutoFilterRowHandle Then
            e.Appearance.BackColor = SystemColors.InactiveCaptionText
        End If
    End Sub

    Private Sub AnaCreditContractsMonthly_GridView_ShownEditor(sender As Object, e As EventArgs) Handles AnaCreditContractsMonthly_GridView.ShownEditor
        Dim view As GridView = CType(sender, GridView)
        If view.FocusedRowHandle = DevExpress.XtraGrid.GridControl.AutoFilterRowHandle Then
            view.ActiveEditor.Properties.Appearance.ForeColor = Color.Yellow
        End If
