    ...
    Private Sub NostroAcc_SearchLookUpEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles NostroAcc_SearchLookUpEdit.ButtonClick
        ...
        If e.Button.Caption = "Restart" Then
            If Me.NostroAcc_SearchLookUpEdit.Text <> "" Then
                If XtraMessageBox.Show("Should the Reconciliation for Suspence Account: " & NostroAccount & " be re-executed for the last outstanding Bookings?", "MANUAL RECONCILIATION", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    Try
...
#Region "CREATE INTERNAL DUMMY BOOKING for RECONCILIATION EXTERNAL BOOKING"
    ...
    Private Sub CreateDummyBooking_ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles CreateDummyBooking_ToolStripMenuItem.Click
        ...
        If BOOKING_ROUTE_MAIN = "INTERNAL" Then
            If LastRecDate = rd Then
                If XtraMessageBox.Show("Should for this Internal Booking a dummy Internal booking be created in order to reconcile?", "RECONCILIATION - CREATE INTERNAL DUMMY BOOKING", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub CreateInternalDummyToReconcileTheTotalReconciliationDifferenceToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles CreateInternalDummyToReconcileTheTotalReconciliationDifferenceToolStripMenuItem.Click
        ...
        If TOTAL_DIFFERENCE_AMOUNT <> 0 Then
            If LastRecDate = rd Then
                If XtraMessageBox.Show("Should for the Total Reconciliation Difference a dummy Internal booking be created in order to reconcile?", "RECONCILIATION - CREATE INTERNAL DUMMY BOOKING FOR TOTAL RECONCILIATION DIFFERENCE", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub CreateMissingRecon_btn_Click(sender As Object, e As EventArgs) Handles CreateMissingRecon_btn.Click
        ...
        If IsNothing(Me.NostrosForMissingRecon_SearchLookUpEdit.Text) = False AndAlso IsNumeric(Me.NostrosForMissingRecon_SearchLookUpEdit.Text) = True Then
            ...
            If ReconCount = 0 Then
                If XtraMessageBox.Show("Should the missing Reconciliation for Suspence Account: " & Me.NostrosForMissingRecon_SearchLookUpEdit.Text & " be executed?", "CREATION OF MISSING RECONCILIATION", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    Try
                        SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
