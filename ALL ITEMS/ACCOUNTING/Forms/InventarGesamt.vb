    ...
    Private Sub GridControl3_EmbeddedNavigator_ButtonClick(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.NavigatorButtonClickEventArgs)
        If e.Button.ButtonType = DevExpress.XtraEditors.NavigatorButtonType.EndEdit Then
            ...
            Try
                ...
                End If
                cmd.CommandTimeout = 500
                cmd.CommandText = "UPDATE A SET A.[MWST_Rueck]=B.[Vorsteuersatz] from [INVENTAR_ALL_ITEMS] A INNER JOIN [INVENTAR_VORSTEUER_SAETZE] B ON DATEPART(Year,A.[Anschaffungsdatum])=B.[Jahr] where DATEPART(Year,A.[Anschaffungsdatum])>=2015 and ([MWST_Rueck_IsZero] is NULL or [MWST_Rueck_IsZero]=0)"
                cmd.ExecuteNonQuery()
                cmd.CommandText = "Update [INVENTAR_ALL_ITEMS] set [Kontonummer_Vorsteuer_Amount]=Round([Nettowert]*([MWST_Satz]/100)*([MWST_Rueck]/100)*(-1),2) where DATEPART(Year,[Anschaffungsdatum])>=2015 and [MWST_Satz]<>0 and [MWST_Rueck]<>0"
                cmd.ExecuteNonQuery()
                cmd.CommandText = "Update [INVENTAR_ALL_ITEMS] set [Anschaffungswert]=[KontonummerAmount]+[Kontonummer_MWST_Amount]+[Kontonummer_Vorsteuer_Amount] where DATEPART(Year,[Anschaffungsdatum])>=2015"
                cmd.ExecuteNonQuery()
                If cmd.Connection.State = ConnectionState.Open Then
                    cmd.Connection.Close()
                End If
                    ...
                    If Result = 0 Then
                        ...
                        ElseIf Anzahl_Inventar_Items > 1 Then
                            ...
                            If XtraMessageBox.Show("Die Inventar Nr. " & Me.InventarNr_TextEdit.Text & " wird bis zu " & Anzahl_Inventar_Items & " mal eingefügt!" _
                                                ...
                                                & vbNewLine & "Soll fortgefahren werden?", "EINFÜGEN INVENTAR NR. (ANZAHL > 1)", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
...
#Region "IMPORT INVENTAR FROM EXCEL"
    Private Sub ImportFromExcel_btn_Click(sender As Object, e As EventArgs) Handles ImportFromExcel_btn.Click
        If XtraMessageBox.Show("Sollen die eingetragenen Inventar Items in die Datenbank eingefügt werden?", "IMPORT NEUE INVENTAR ITEMS", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
            Try
                SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
