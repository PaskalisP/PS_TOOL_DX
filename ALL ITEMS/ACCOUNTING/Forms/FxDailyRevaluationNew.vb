    ...
    Private Sub GridControl5_EmbeddedNavigator_ButtonClick(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.NavigatorButtonClickEventArgs)
        If e.Button.Tag = "FxPairing" Then
            ...
            If dtID.Rows.Count > 1 Then
                If XtraMessageBox.Show("Should the following FX Deals: " & vbNewLine & vbNewLine & FxDealsToPaired & vbNewLine & "be paired?", "FX DEALS - PAIRING", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    'Set as paired
                    If cmd.Connection.State = ConnectionState.Closed Then
                        cmd.Connection.Open()
                    End If
...
#End Region
    ...
    Private Sub UnpairFX_ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles UnpairFX_ToolStripMenuItem.Click
        ...
        If XtraMessageBox.Show("Should the paired FX Deals:" & vbNewLine & vbNewLine & RelatedFX_Deals & "be changed to unpaired ?", "FX DEALS - UNPAIRING", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub OpenPaired_UNEF_Totals_GridView_CustomSummaryCalculate(sender As Object, e As CustomSummaryEventArgs) Handles OpenPaired_UNEF_Totals_GridView.CustomSummaryCalculate
        ...
        If e.SummaryProcess = DevExpress.Data.CustomSummaryProcess.Calculate Then
            ...
            If summaryID = 0 Then
                ...
                If CheckFieldAmount_OpenPairsUNEF_Drohverlust < 0 Then
                    OpenPairsUNEF_Drohverlust += Convert.ToDecimal(e.FieldValue)
                End If
            End If
        End If
        ' Finalization 
        If e.SummaryProcess = DevExpress.Data.CustomSummaryProcess.Finalize Then
            If summaryID = 0 Then
                e.TotalValue = OpenPairsUNEF_Drohverlust
            End If
        End If
    End Sub
        ...
        If IsDate(Me.FxDailyRevaluationDateEdit.Text) = True Then
            Try
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Dim RangeOP_ALL_PL As Range = worksheet.Range("AD3:AD" & OP_ALL_LastDtRow)
                    RangeOP_ALL_PL.Formula = "=T3/X3-W3/Y3"
                    Dim RangeOP_DIFFERENCES As Range = worksheet.Range("AE3:AE" & OP_ALL_LastDtRow)
                    RangeOP_DIFFERENCES.Formula = "=AC3-AD3"
                    'Range for Formating Calculation
                    Dim RangeOP_ALL_FORMAT As Range = worksheet.Range("AF3:AF" & OP_ALL_LastDtRow)
                ...
                SplashScreenManager.Default.SetWaitFormCaption("Clear Contents in Excel Cell Ranges before importing new Data")
                worksheet.ClearContents(worksheet("A4:E5000"))
                SplashScreenManager.Default.SetWaitFormCaption("Import new Data to Excel File")
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Dim RangeOP_ALL_TOTALS_PL As Range = worksheet.Range("C4:C" & OP_ALL_TOTALS_LastDtRow)
                    ...
                    Dim RangeOP_ALL_TOTALS_FV_BW_1 As Range = worksheet.Range("D4:D" & OP_ALL_TOTALS_LastDtRow)
                    ...
                    Dim RangeOP_ALL_TOTALS_FV_BW_2 As Range = worksheet.Range("E4:E" & OP_ALL_TOTALS_LastDtRow)
                ...
                Me.QueryText = "SELECT [PairNr],[Description],[CCY],[AMT_B],[AMT_S],0,[REVAL_RATE_DESCR],[REVAL_RATE] FROM [FX_BEWERTUNGEN_UNEF_TOTALS] where [RiskDate]='" & rdsql & "'"
                da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                ...
                SplashScreenManager.Default.SetWaitFormCaption("Clear Contents in Excel Cell Ranges before importing new Data")
                worksheet.ClearContents(worksheet("A4:J5000"))
                SplashScreenManager.Default.SetWaitFormCaption("Import new Data to Excel File")
                worksheet.Import(dt, False, 3, 0)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Dim RangeOP_UNEF_DIFF As Range = worksheet.Range("F4:F" & OP_UNEF_TOTALS_LastDtRow)
                    RangeOP_UNEF_DIFF.Formula = "=D4-E4"

                    Dim RangeOP_UNEF_KURSBEWERTUNG As Range = worksheet.Range("I4:I" & OP_UNEF_TOTALS_LastDtRow)
                    RangeOP_UNEF_KURSBEWERTUNG.Formula = "=F4/H4"

                    'Range for Formating Calculation
                    Dim RangeOP_UNEF_TOTALS_FORMAT As Range = worksheet.Range("J4:J" & OP_UNEF_TOTALS_LastDtRow)
                    ...
                    workbook.Worksheets(3).Cells("I" & OP_UNEF_TOTALS_SumDefault + 1).Formula = "=SUMIF(I4:I" & OP_UNEF_TOTALS_LastDtRow & ";""<0"";" & "I4:I" & OP_UNEF_TOTALS_LastDtRow & ")"
                End If
                ...
                Me.QueryText = "SELECT [ClientNo],[ClientName],[ContractNr],[DealType],[DealStatus],[DealSellBuy],[InputDate],[ValueDate],[MaturityDate],[BuyCCY],[BuyAmount],[SellCCY],[SellAmount],[PL_inEUR_Equ],[SpotBuyRate],[SpotSellRate],[RevalBuyRate],[RevalSellRate],[TradeBuyRate],[TradeSellRate] FROM [FX DAILY REVALUATION] where OwnDeal in ('Y') and [DealStatus]='U' and [RiskDate]='" & rdsql & "'"
                da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Dim RangeOW_BD1_U As Range = worksheet.Range("U3:U" & OW_BD1_LastDtRow)
                    RangeOW_BD1_U.Formula = "=(K3/S3)-(M3/T3)"
                    Dim RangeOW_BD1_V As Range = worksheet.Range("V3:V" & OW_BD1_LastDtRow)
                    RangeOW_BD1_V.Formula = "=(U3/(I3-H3))*(($C$1-H3))"
                    Dim RangeOW_BD1_W As Range = worksheet.Range("W3:W" & OW_BD1_LastDtRow)
                    RangeOW_BD1_W.Formula = "=(K3/Q3)-(K3/S3)"
                    Dim RangeOW_BD1_X As Range = worksheet.Range("X3:X" & OW_BD1_LastDtRow)
                    RangeOW_BD1_X.Formula = "=(M3/R3)-(M3/T3)"
                    Dim RangeOW_BD1_Y As Range = worksheet.Range("Y3:Y" & OW_BD1_LastDtRow)
                    RangeOW_BD1_Y.Formula = "=W3-X3"
                    Dim RangeOW_BD1_Z As Range = worksheet.Range("Z3:Z" & OW_BD1_LastDtRow)
                    RangeOW_BD1_Z.Formula = "=V3+Y3-N3"
                    Dim RangeOW_BD1_AA As Range = worksheet.Range("AA3:AA" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AA.Formula = "=K3/S3"
                    Dim RangeOW_BD1_AB As Range = worksheet.Range("AB3:AB" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AB.Formula = "=M3/T3"
                    Dim RangeOW_BD1_AC As Range = worksheet.Range("AC3:AC" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AC.Formula = "=AA3-AB3"
                    Dim RangeOW_BD1_AD As Range = worksheet.Range("AD3:AD" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AD.Formula = "=K3/O3"
                    Dim RangeOW_BD1_AE As Range = worksheet.Range("AE3:AE" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AE.Formula = "=M3/P3"
                    Dim RangeOW_BD1_AF As Range = worksheet.Range("AF3:AF" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AF.Formula = "=AD3-AE3"

                    Dim RangeOW_BD1_AG As Range = worksheet.Range("AG3:AG" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AG.Formula = "=K3/Q3"
                    Dim RangeOW_BD1_AH As Range = worksheet.Range("AH3:AH" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AH.Formula = "=M3/R3"
                    Dim RangeOW_BD1_AI As Range = worksheet.Range("AI3:AI" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AI.Formula = "=AG3-AH3"

                    Dim RangeOW_BD1_AJ As Range = worksheet.Range("AJ3:AJ" & OW_BD1_LastDtRow)
                    RangeOW_BD1_AJ.Formula = "=AC3-(AF3-AI3)"

                    Dim RangeOW_BD1_AK As Range = worksheet.Range("AK3:AK" & OW_BD1_LastDtRow)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Dim RangeOW_BD2_Q As Range = worksheet.Range("Q3:Q" & OW_BD2_LastDtRow)
                    RangeOW_BD2_Q.Formula = "=(H3/O3)-(J3/P3)"
                    Dim RangeOW_BD2_R As Range = worksheet.Range("R3:R" & OW_BD2_LastDtRow)
                    RangeOW_BD2_R.Formula = "=(Q3/(F3-D3))*(($C$1-D3))"
                    Dim RangeOW_BD2_S As Range = worksheet.Range("S3:S" & OW_BD2_LastDtRow)
                    RangeOW_BD2_S.Formula = "=(H3/M3)-(H3/O3)"
                    Dim RangeOW_BD2_T As Range = worksheet.Range("T3:T" & OW_BD2_LastDtRow)
                    RangeOW_BD2_T.Formula = "=(J3/N3)-(J3/P3)"
                    Dim RangeOW_BD2_U As Range = worksheet.Range("U3:U" & OW_BD2_LastDtRow)
                    RangeOW_BD2_U.Formula = "=S3-T3"
                    Dim RangeOW_BD2_V As Range = worksheet.Range("V3:V" & OW_BD2_LastDtRow)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Dim RangeOW_BD3_M As Range = worksheet.Range("M3:M" & OW_BD3_LastDtRow)
                    RangeOW_BD3_M.Formula = "=IF(E3<=$M$1;F3-$M$1;0)"
                    Dim RangeOW_BD3_N As Range = worksheet.Range("N3:N" & OW_BD3_LastDtRow)
                    RangeOW_BD3_N.Formula = "=IF(M3<>0;F3-E3;0)"
                    Dim RangeOW_BD3_O As Range = worksheet.Range("O3:O" & OW_BD3_LastDtRow)
                    RangeOW_BD3_O.Formula = "=H3/K3"
                    Dim RangeOW_BD3_P As Range = worksheet.Range("P3:P" & OW_BD3_LastDtRow)
                    RangeOW_BD3_P.Formula = "=-J3/L3"
                    Dim RangeOW_BD3_Q As Range = worksheet.Range("Q3:Q" & OW_BD3_LastDtRow)
                    RangeOW_BD3_Q.Formula = "=IF(N3<>0;(O3+P3)/N3*M3;O3+P3)"

                    'Set Conditional formating only for FX Deals with Value Date<=Last Day Last Year
                    'Dim RangeOW_BD3_AllData As Range = worksheet.Range("A3:A" & OW_BD3_LastDtRow)
