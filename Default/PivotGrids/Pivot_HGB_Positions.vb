...
Public Class Pivot_HGB_Positions
    ...
    Private Sub Pivot_HGB_Positions_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.QueryText = "Select CONVERT(VARCHAR(10),[BSDate],104) as 'RiskDate' from [HGB_GL_Items] GROUP BY [BSDate] ORDER BY [BSDate] desc"
        ...
        Try
            ...
            Me.QueryText = "Select *,'HGB Position'=Case when HGB_GV_MatchCode is not NULL then 'G+V Position' else 'BILANZ Position' end from HGB_GL_Items  where GL_Item in (Select  GL_Item from  DailyBalanceDetailsSheets  where BSDate='" & rdsql1 & "' and RilibiCode is not NULL) and BSDate='" & rdsql1 & "' and (HGB_BS_MatchCode is not NULL or HGB_GV_MatchCode is not NULL) order by GL_Item_Number asc"
            da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
    ...
    Private Sub PrintableComponentLink1_CreateMarginalHeaderArea(sender As Object, e As CreateAreaEventArgs) Handles PrintableComponentLink1.CreateMarginalHeaderArea
        Dim reportfooter As String = "HGB Positions" 'from " & "   " & rd1 & " till  " & rd2
    ...
    Private Sub Print_Export_btn_Click(sender As Object, e As EventArgs) Handles Print_Export_btn.Click
        If ActiveTabGroup = 0 Then
            ...
            SplashScreenManager.Default.SetWaitFormCaption("Load HGB Positions Data from " & rd1 & " till " & rd2)
    ...
    Private Sub TabbedControlGroup1_SelectedPageChanged(sender As Object, e As DevExpress.XtraLayout.LayoutTabPageChangedEventArgs) Handles TabbedControlGroup1.SelectedPageChanged
        If Me.TabbedControlGroup1.SelectedTabPage.Text = "All Business Dates" Then
            ActiveTabGroup = 0
            Me.LayoutControlGroup3.Text = "HGB Positions"
    ...
    Private Sub PivotGridControl1_CellDoubleClick(sender As Object, e As DevExpress.XtraPivotGrid.PivotCellEventArgs) Handles PivotGridControl1.CellDoubleClick
        ...
        Try
            ...
            For Each col As GridColumn In c.PivotDetailsBaseView.Columns
                If col.FieldName.StartsWith("Amount") Or col.FieldName.StartsWith("Balance") Or col.Name.Contains("Sum") Or col.FieldName.Contains("Amount") _
        ...
        If rd2 >= rd1 Then
            If ActiveTabGroup = 0 Then
                ...
                If DaysCount <= 180 Then
                    ...
                    Me.QueryText = "Select *,'HGB Position'=Case when HGB_GV_MatchCode is not NULL then 'G+V Position' else 'BILANZ Position' end from HGB_GL_Items  where GL_Item in (Select  GL_Item from  DailyBalanceDetailsSheets  where BSDate BETWEEN '" & rdsql1 & "' AND '" & rdsql2 & "' and RilibiCode is not NULL) and BSDate BETWEEN '" & rdsql1 & "' AND '" & rdsql2 & "' and (HGB_BS_MatchCode is not NULL or HGB_GV_MatchCode is not NULL) order by GL_Item_Number asc"
                        da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                        dt = New System.Data.DataTable()
                        da.Fill(dt)
                        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    ...
    Private Sub LoadDataSelectedDate_BarButtonItem_ItemClick(sender As Object, e As ItemClickEventArgs) Handles LoadDataSelectedDate_BarButtonItem.ItemClick
        ...
        If rd2 >= rd1 Then
            If ActiveTabGroup = 0 Then
                SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                SplashScreenManager.Default.SetWaitFormCaption("Load HGB Positions Data for " & rd1 & " and " & rd2)
                Me.QueryText = "Select *,'HGB Position'=Case when HGB_GV_MatchCode is not NULL then 'G+V Position' else 'BILANZ Position' end from HGB_GL_Items  where GL_Item in (Select  GL_Item from  DailyBalanceDetailsSheets  where BSDate in ('" & rdsql1 & "','" & rdsql2 & "') and RilibiCode is not NULL) and BSDate in ('" & rdsql1 & "','" & rdsql2 & "') and (HGB_BS_MatchCode is not NULL or HGB_GV_MatchCode is not NULL) order by GL_Item_Number asc"
                da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    ...
    Private Sub dxOK_SelectList_click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If SelectList.ListBoxControl1.Items.Count > 0 Then
            ...
            SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
            SplashScreenManager.Default.SetWaitFormCaption("Load HGB Positions Data for " & commaSeparatedDates)
            Me.QueryText = "Select *,'HGB Position'=Case when HGB_GV_MatchCode is not NULL then 'G+V Position' else 'BILANZ Position' end from HGB_GL_Items  where GL_Item in (Select  GL_Item from  DailyBalanceDetailsSheets  where BSDate in ( " & SelectedDates & ") and RilibiCode is not NULL) and BSDate in ( " & SelectedDates & ") and (HGB_BS_MatchCode is not NULL or HGB_GV_MatchCode is not NULL) order by GL_Item_Number asc"
            da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
