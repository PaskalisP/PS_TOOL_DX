...
Public Class Pivot_BS_Details
    ...
    Private Sub Pivot_BS_Details_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'TODO: This line of code loads data into the 'PS_TOOL_DX_LiveDataSet.DailyBalanceDetailsSheets' table. You can move, or remove it, as needed.
        'Me.DailyBalanceDetailsSheetsTableAdapter.FillByBS_Dates(Me.PS_TOOL_DX_LiveDataSet.DailyBalanceDetailsSheets, rd1, rd2)

        Me.QueryText = "Select * from SQL_PARAMETER_DETAILS where Id_SQL_Parameters in ('SEVERAL SELECTIONS') and SQL_Name_1 in ('BS_Details_Fill_From_Till') and Status in ('Y')"
        ...
        Me.QueryText = "Select * from SQL_PARAMETER_DETAILS where Id_SQL_Parameters in ('SEVERAL SELECTIONS') and SQL_Name_1 in ('BS_Details_Compare') and Status in ('Y')"
        da1 = New SqlDataAdapter(Me.QueryText.Trim(), conn)
        da1.SelectCommand.CommandTimeout = 60000
        dt1 = New System.Data.DataTable()
        da1.Fill(dt1)
    ...
    Private Sub PrintableComponentLink1_CreateMarginalHeaderArea(sender As Object, e As CreateAreaEventArgs) Handles PrintableComponentLink1.CreateMarginalHeaderArea
        Dim reportfooter As String = "Balance Sheet Details" 'from " & "   " & rd1 & " till  " & rd2
    ...
    Private Sub PrintableComponentLink2_CreateMarginalHeaderArea(sender As Object, e As CreateAreaEventArgs) Handles PrintableComponentLink2.CreateMarginalHeaderArea
        Dim reportfooter As String = "Balance Sheet Details compared between " & "   " & rd1 & " and  " & rd2
        e.Graph.DrawString(reportfooter, New RectangleF(0, 0, e.Graph.ClientPageSize.Width, 20))
    End Sub
    ...
    Private Sub Print_Export_btn_Click(sender As Object, e As EventArgs) Handles Print_Export_btn.Click
        If ActiveTabGroup = 0 Then
            ...
            SplashScreenManager.Default.SetWaitFormCaption("Load Balance Sheet details from " & rd1 & " till " & rd2)
        ...
        If ActiveTabGroup = 1 Then
            SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
            SplashScreenManager.Default.SetWaitFormCaption("Balance Sheet details compared between " & rd1 & " and " & rd2)
    ...
    Private Sub PivotGridControl1_CellDoubleClick(sender As Object, e As DevExpress.XtraPivotGrid.PivotCellEventArgs) Handles PivotGridControl1.CellDoubleClick
        ...
        Try
            ...
            For Each col As GridColumn In c.PivotDetailsBaseView.Columns
                If col.FieldName.StartsWith("Amount") Or col.Name = "Total_Balance" Or col.FieldName.Contains("Amount") _
                    ...
                    Or col.FieldName.Contains("EUR Equ") Or col.FieldName = "Orig_Balance" Or col.FieldName = "Balance_EUR_CR" Or col.FieldName = "Balance_EUR_DR" Then
        ...
        If rd2 >= rd1 Then
            If ActiveTabGroup = 0 Then
                ...
                If DaysCount <= 180 Then
                    SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                    SplashScreenManager.Default.SetWaitFormCaption("Load Balance Sheet Data from " & rd1 & " till " & rd2 & vbNewLine & "(SQL PROCEDURES PAREMETER/SEVERAL SELECTIONS/BS_Details_Fill_From_Till)")
                    ...
                    Else
                        SplashScreenManager.CloseForm(False)
                        MessageBox.Show("The SQL Command:SQL PROCEDURES PAREMETER/SEVERAL SELECTIONS/BS_Details_Fill_From_Till is either deactivated or not present!", "NO SQL COMMAND", MessageBoxButtons.OK, MessageBoxIcon.Stop, MessageBoxDefaultButton.Button1)
                        Exit Sub
                    End If
                Else
    ...
    Private Sub LoadDataSelectedDate_BarButtonItem_ItemClick(sender As Object, e As ItemClickEventArgs) Handles LoadDataSelectedDate_BarButtonItem.ItemClick
        ...
        If rd2 >= rd1 Then
            If ActiveTabGroup = 0 Then
                ...
                SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                SplashScreenManager.Default.SetWaitFormCaption("Load Balance Sheet Data for " & rd1 & " and " & rd2)
                Me.QueryText = "SELECT  ID, GL_Art, GL_Item, GL_Item_Number, ReleatedAccountNr, GL_Account_Nr, GL_Account_Name, Orig_CUR, Orig_Balance, Balance_EUR_CR, Balance_EUR_DR, Total_Balance, BSDate, RepDate, RilibiCode, RilibiName, BISTA_Sign, BISTA_Pos_Sign, IdBalanceSheets FROM  DailyBalanceDetailsSheets WHERE BSDate  in ('" & rdsql1 & "','" & rdsql2 & "')"
                da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                da.SelectCommand.CommandTimeout = 60000
                dt = New System.Data.DataTable()
                da.Fill(dt)
    ...
    Private Sub CompareData_BarButtonItem_ItemClick(sender As Object, e As ItemClickEventArgs) Handles CompareData_BarButtonItem.ItemClick
        ...
        If rd2 >= rd1 Then
            ...
            If ActiveTabGroup = 1 Then
                SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                SplashScreenManager.Default.SetWaitFormCaption("Load Balance Sheet Data from " & rd1 & " till " & rd2 & vbNewLine & "(SQL PROCEDURES PAREMETER/SEVERAL SELECTIONS/BS_Details_Compare)")
                Me.QueryText = "Select * from SQL_PARAMETER_DETAILS where Id_SQL_Parameters in ('SEVERAL SELECTIONS') and SQL_Name_1 in ('BS_Details_Compare') and Status in ('Y')"
                da1 = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                da1.SelectCommand.CommandTimeout = 60000
                dt1 = New System.Data.DataTable()
                ...
                Else
                    SplashScreenManager.CloseForm(False)
                    MessageBox.Show("The SQL Command:SQL PROCEDURES PAREMETER/SEVERAL SELECTIONS/BS_Details_Compare is either deactivated or not present!", "NO SQL COMMAND", MessageBoxButtons.OK, MessageBoxIcon.Stop, MessageBoxDefaultButton.Button1)
                    Exit Sub
                End If
            End If
    ...
    Private Sub dxOK_SelectList_click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If SelectList.ListBoxControl1.Items.Count > 0 Then
            ...
            SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
            SplashScreenManager.Default.SetWaitFormCaption("Load Balance Sheet Details Data for " & commaSeparatedDates)
            Me.QueryText = "SELECT * FROM DailyBalanceDetailsSheets WHERE BSDate  in ( " & SelectedDates & ")"
            da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
            da.SelectCommand.CommandTimeout = 60000
            dt = New System.Data.DataTable()
            da.Fill(dt)
