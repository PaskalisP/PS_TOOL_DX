...
Public Class InterestRateRiskCalc3
    ...
    Dim CheckNrField1 As Double = 0
    Dim CheckNrField2 As Double = 0
    ...
    Dim MinSumAM1 As Double = 0
    Dim MinSumAM2 As Double = 0
    ...
    Private Sub InterestRateRiskCalc3_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.QueryText = "Select CONVERT(VARCHAR(10),[RateRiskDate],104) as 'RLDC Date' from [RATERISK DATE] where [RateRiskDate]>='20181231' ORDER BY [RateRiskDate] desc"
        ...
        Me.RATERISK_TOTALS2TableAdapter.FillByRATERISK_TOTALS_FILL(Me.RiskControllingDataSet.RATERISK_TOTALS2, d)
        Me.RATERISK_DETAILS1TableAdapter.FillByRATERISK_DETAILS_FILL(Me.RiskControllingDataSet.RATERISK_DETAILS1, d)
    ...
    Private Sub TOTALSBandedGridView_CustomSummaryCalculate(sender As Object, e As DevExpress.Data.CustomSummaryEventArgs) Handles TOTALSBandedGridView.CustomSummaryCalculate
        ...
        '        CheckNrField1 = CDbl(View.GetGroupRowValue(e.GroupRowHandle, colAM1))
        '        CheckNrField2 = CDbl(View.GetGroupRowValue(e.GroupRowHandle, colAM2))
        '        'Calculation Derivative Adressenausfallrisikopositionen und Aufrechnungspositionen nach § 12 Abs. 1 SolvV 
        '        If summaryID = 1 Then
        '            For Each groupColumn As GridColumn In View.GroupedColumns
        '                Dim value As Object = CDbl(View.GetGroupRowValue(e.RowHandle, colAM1))
        '                Dim value1 As Object = CDbl(View.GetGroupRowValue(e.GroupRowHandle, colAM1))
        '                MsgBox(value & "  " & value1)

        '            Next
        '            Dim SumField As Double = CDbl(View.GetGroupRowValue(e.GroupRowHandle, colAM1))
        '            Dim SumField1 As Double = CDbl(View.GetGroupRowValue(e.GroupRowHandle, colAM2))

        '            If CheckNrField1 < CheckNrField2 Then
        '                MinSumAM1 += Convert.ToDecimal(SumField)
        '            End If
        '            If CheckNrField1 > CheckNrField2 Then
        '                MinSumAM1 += Convert.ToDecimal(SumField1)
        '            End If
        ...
        '    If summaryID = 1 Then
        '        e.TotalValue = MinSumAM1 + MinSumAM2
        '        'MsgBox(e.TotalValue)
        '    End If
        '    If summaryID = 3 Then
    ...
    Private Sub InterestRateRiskDateEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles InterestRateRiskDateEdit.ButtonClick
        If e.Button.Caption = "Reload" Then
            Me.GridControl1.DataSource = Me.RATERISKTOTALS2BindingSource
            Me.GridControl2.DataSource = Me.RATERISKDETAILS1BindingSource
...
e.Graph.DrawString(reportfooter, New RectangleF(0, 0, e.Graph.ClientPageSize.Width, 20))
    ...
    Private Sub InterestRateRiskRBCBarButtonItem_ItemClick(sender As Object, e As ItemClickEventArgs) Handles InterestRateRiskRBCBarButtonItem.ItemClick
        If IsDate(Me.InterestRateRiskDateEdit.Text) = True Then
            ...
            Dim dsql As String = d.ToString("yyyyMMdd")
            SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
            SplashScreenManager.Default.SetWaitFormCaption("Creating INTEREST RATE RISK AMOUNT (Risk bearing Capacity) for " & Me.InterestRateRiskDateEdit.Text)
            Dim IRR_DATE_Da As New SqlDataAdapter("Select * from [RATERISK DATE] where RateRiskDate='" & dsql & "' ", conn)
            Dim IRR_TOTALS_Da As New SqlDataAdapter("Select * from [RATERISK TOTALS] where [RISK DATE]='" & dsql & "'", conn)
