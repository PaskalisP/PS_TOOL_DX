...
Public Class ProfitAndLossBalances
    ...
    Private Sub PROFIT_and_LOSS_VOLUMESBindingNavigatorSaveItem_Click(sender As Object, e As EventArgs)
        Me.Validate()
        Me.PROFIT_and_LOSS_VOLUMESBindingSource.EndEdit()
        Me.TableAdapterManager.UpdateAll(Me.BalancesDataset)

    End Sub

    Private Sub ProfitAndLossBalances_Load(sender As Object, e As EventArgs) Handles MyBase.Load
          ...
          IsDate(Me.GL_BookingDate_From.Text) = True AndAlso IsDate(Me.GL_BookingDate_Till.Text) = True Then
            ...
            If d2 >= d1 Then
                Try
                    ...
                    cmd.CommandText = "SELECT [GL_ITEM_NAME] from [PROFIT and LOSS GL ITEMS] where [GL_ITEM_NR]='" & Me.GL_LookUpEdit.Text & "'"
                    cmd.Connection.Open()
                    ...
                    Dim objCMD As SqlCommand = New SqlCommand("execute [PL_VOLUMES_GL_ITEM]  @FROMDATE='" & sqld1 & "', @TILLDATE='" & sqld2 & "',@GLITEM_NR='" & Me.GL_LookUpEdit.Text & "'  ", conn)
                    objCMD.CommandTimeout = 5000
                    da = New SqlDataAdapter(objCMD)

                    'OPENING BALANCE FOR GL ITEM
                    cmd.CommandText = "SELECT [AmountInEuro] from [PROFIT and LOSS VOLUMES] where [GL_Item_Nr]='" & Me.GL_LookUpEdit.Text & "' and [BatchNo]='GL ITEM OPENING BALANCE' and [GL_Rep_Date]='" & sqld1 & "'"
                    ...
                    cmd.CommandText = "SELECT [AmountInEuro] from [PROFIT and LOSS VOLUMES] where [GL_Item_Nr]='" & Me.GL_LookUpEdit.Text & "' and [BatchNo]='GL ITEM CLOSING BALANCE' and [GL_Rep_Date]='" & sqld2 & "'"
                    ...
                    Me.GridControl1.MainView = PL_Balances_BasicView
                    dt = New DataTable()
                    da.Fill(dt)
                    'Results
          ...
          IsDate(Me.OCBS_BookingDate_From.Text) = True AndAlso IsDate(Me.OCBS_BookingDate_Till.Text) = True Then
            ...
            If d2 >= d1 Then
                Try
                    ...
                    End If
                    cmd.CommandTimeout = 120
                    cmd.CommandText = "SELECT [GL_ACC_Name] from [PROFIT and LOSS MAPPING] where [GL_ACC_Nr]='" & Me.OCBS_LookUpEdit.Text & "'"
                    cmd.Connection.Open()
                    ...
                    If Me.ComboBoxEdit1.Text = "***" Then
                        ...
                        Dim objCMD As SqlCommand = New SqlCommand("execute [PL_VOLUMES_OCBS_ACC_ALL_CURRENCIES]  @FROMDATE='" & sqld1 & "', @TILLDATE='" & sqld2 & "',@OCBS_ACC_NR='" & Me.OCBS_LookUpEdit.Text & "'  ", conn)
                        objCMD.CommandTimeout = 5000
                        da = New SqlDataAdapter(objCMD)

                        'OPENING BALANCE FOR OCBS ACCOUNT
                        cmd.CommandText = "SELECT Sum([AmountInEuro]) from [PROFIT and LOSS VOLUMES] where [GL_AC_No]='" & Me.OCBS_LookUpEdit.Text & "' and [BatchNo]='OPENING BALANCE OCBS ACC.' and [GL_Rep_Date]='" & sqld1 & "'"
                        Dim obglitem As Double = 0
                        If cmd.ExecuteScalar IsNot DBNull.Value Then
                            obglitem = cmd.ExecuteScalar
                        End If
                        ...
                        cmd.CommandText = "SELECT Sum([AmountInEuro]) from [PROFIT and LOSS VOLUMES] where [GL_AC_No]='" & Me.OCBS_LookUpEdit.Text & "' and [BatchNo]='CLOSING BALANCE OCBS ACC.' and [GL_Rep_Date]='" & sqld2 & "'"
                        Dim cbglitem As Double = 0
                        If cmd.ExecuteScalar IsNot DBNull.Value Then
                            cbglitem = cmd.ExecuteScalar
                        End If
                    ...
                    Else
                        ...
                        Dim objCMD As SqlCommand = New SqlCommand("execute [PL_VOLUMES_OCBS_ACC_CUR]  @FROMDATE='" & sqld1 & "', @TILLDATE='" & sqld2 & "',@OCBS_ACC_NR='" & Me.OCBS_LookUpEdit.Text & "',@CUR= '" & Me.ComboBoxEdit1.Text & "' ", conn)
                        objCMD.CommandTimeout = 5000
                        da = New SqlDataAdapter(objCMD)

                        'OPENING BALANCE FOR OCBS ACCOUNT
                        cmd.CommandText = "SELECT Sum([AmountInEuro]) from [PROFIT and LOSS VOLUMES] where [GL_AC_No]='" & Me.OCBS_LookUpEdit.Text & "' and [BatchNo]='OPENING BALANCE OCBS ACC.' and [GL_Rep_Date]='" & sqld1 & "' and [CCY]='" & Me.ComboBoxEdit1.Text & "'"
                        Dim obglitem As Double = 0
                        If cmd.ExecuteScalar IsNot DBNull.Value Then
                            obglitem = cmd.ExecuteScalar
                        End If
                        ...
                        'CLOSING BALANCE FOR OCBS ACCOUNT
                        cmd.CommandText = "SELECT Sum([AmountInEuro]) from [PROFIT and LOSS VOLUMES] where [GL_AC_No]='" & Me.OCBS_LookUpEdit.Text & "' and [BatchNo]='CLOSING BALANCE OCBS ACC.' and [GL_Rep_Date]='" & sqld2 & "' and [CCY]='" & Me.ComboBoxEdit1.Text & "'"
                        Dim cbglitem As Double = 0
                        If cmd.ExecuteScalar IsNot DBNull.Value Then
                            cbglitem = cmd.ExecuteScalar
                        End If
        ...
        If IsDate(Me.OCBS_BookingDate_From.Text) = True AndAlso IsDate(Me.OCBS_BookingDate_Till.Text) = True Then
            ...
            If d2 >= d1 Then
                Try
                    ...
                    Dim objCMD As SqlCommand = New SqlCommand("execute [PL_VOLUMES_OCBS_ACC_ALL_VOLUMES]  @FROMDATE='" & sqld1 & "', @TILLDATE='" & sqld2 & "' ", conn)
                    objCMD.CommandTimeout = 5000
                    da = New SqlDataAdapter(objCMD)
    ...
    Private Sub PrintableComponentLink1_CreateMarginalHeaderArea(ByVal sender As Object, ByVal e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLink1.CreateMarginalHeaderArea
        If Me.SearchText_lbl.Text.StartsWith("Search results by GL ITEM") = True Then
            Dim reportfooter As String = "UMSATZABFRAGE für GL Item" & "  " & GLItemlbl & " (" & GLitemNamelbl & ")" & "  " _
                                         & "von " & d1 & " bis " & d2
        ...
        ElseIf Me.SearchText_lbl.Text.StartsWith("Search results by OCBS Account") = True Then
            ...
            End If
            Dim reportfooter As String = "UMSATZABFRAGE für OCBS Konto" & "  " & OCBSaccountlbl & " (" & OCBSaccountNamelbl & ")" & "  " _
                                        & currency & "  " _
                                        & "von " & d1 & " bis " & d2
             e.Graph.DrawString(reportfooter, New RectangleF(0, 0, e.Graph.ClientPageSize.Width, 20))
        ElseIf Me.SearchText_lbl.Text.StartsWith("Postings of all mapped OCBS Accounts") = True Then
            ...
            If specialFindtext <> "" Then
                Dim reportfooter As String = "UMSATZABFRAGE für alle OCBS Konten" & "  " _
                                                        & "von " & d1 & " bis " & d2 & "  " _
                                                        & "Gesucht nach: " & specialFindtext
                 e.Graph.DrawString(reportfooter, New RectangleF(0, 0, e.Graph.ClientPageSize.Width, 20))
            Else
                Dim reportfooter As String = "UMSATZABFRAGE für alle OCBS Konten" & "  " _
                                                       & "von " & d1 & " bis " & d2
    ...
    Private Sub PL_Balances_BasicView_RowStyle(sender As Object, e As RowStyleEventArgs) Handles PL_Balances_BasicView.RowStyle
        'Set Backcolor to Filter row
        If e.RowHandle = DevExpress.XtraGrid.GridControl.AutoFilterRowHandle Then
            e.Appearance.BackColor = SystemColors.InactiveCaptionText
        End If
    ...
    Private Sub PL_Balances_BasicView_ShownEditor(sender As Object, e As EventArgs) Handles PL_Balances_BasicView.ShownEditor
        Dim view As GridView = CType(sender, GridView)
        If view.FocusedRowHandle = DevExpress.XtraGrid.GridControl.AutoFilterRowHandle Then
            view.ActiveEditor.Properties.Appearance.ForeColor = Color.Yellow
        End If
