...
Public Class CustomerVostroBalances
            ...
            If d2 >= d1 Then
                ...
                If IsNothing(Me.Account_LookUpEdit.Text) = False AndAlso IsNumeric(Me.Account_LookUpEdit.Text) = True Then
                    Try
                        ...
                        SplashScreenManager.Default.SetWaitFormCaption("Load  Postings + Balances for Vostro Account: " & Me.Account_LookUpEdit.Text & " from: " & d1 & " till " & d2)
                        
                        'Dim objCMD As SqlCommand = New SqlCommand("execute [VOSTRO_VOLUMES_ACC_ONLY]  @FROMDATE='" & rdsql1 & "', @TILLDATE='" & rdsql2 & "',@ACCOUNT_NR='" & Me.Account_LookUpEdit.Text & "'  ", conn)
                        ...
                        Using sqlConn As New SqlConnection(My.Settings.PS_TOOL_DX_SQL_Client_ConnectionString)
                            Using sqlCmd As New SqlCommand()
                                sqlCmd.CommandText = "SELECT * FROM   CUSTOMER_VOSTRO_VOLUMES  WHERE  (AccountNo = '" & Me.Account_LookUpEdit.Text & "') AND (GL_Rep_Date >= '" & rdsql1 & "') AND (GL_Rep_Date <= '" & rdsql2 & "') ORDER BY IdNr"
                        ...
                        Me.SearchText_lbl.Text = "Search results for Vostro Account: " & Me.Account_LookUpEdit.Text & " -Currency: " & OCBSaccountCurlbl & " from " & d1 & " till " & d2
                ...
                ElseIf Me.Account_LookUpEdit.Text = "" Then
                    Try
                        ...
                        SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                        SplashScreenManager.Default.SetWaitFormCaption("Load  closing Balances for all Vostro Accounts from: " & d1 & " till " & d2)

                        'Dim objCMD As SqlCommand = New SqlCommand("execute [VOSTRO_VOLUMES_BALANCES_ONLY]  @FROMDATE='" & rdsql1 & "', @TILLDATE='" & rdsql2 & "' ", conn)
                        'objCMD.CommandTimeout = 5000
                        'da = New SqlDataAdapter(objCMD)
                        ...
                        Using sqlConn As New SqlConnection(My.Settings.PS_TOOL_DX_SQL_Client_ConnectionString)
                            Using sqlCmd As New SqlCommand()
                                sqlCmd.CommandText = "SELECT * FROM   CUSTOMER_VOSTRO_VOLUMES  WHERE   (GL_Rep_Date >= '" & rdsql1 & "') AND (GL_Rep_Date <= '" & rdsql2 & "') AND BatchNo in ('CLOSING BALANCE')  ORDER BY IdNr"
                                sqlCmd.Connection = sqlConn
                                sqlCmd.CommandTimeout = 5000
                                If sqlConn.State = ConnectionState.Closed Then
                                    sqlConn.Open()
    ...
    Private Sub PrintableComponentLink1_CreateMarginalHeaderArea(sender As Object, e As DevExpress.XtraPrinting.CreateAreaEventArgs) Handles PrintableComponentLink1.CreateMarginalHeaderArea
        Dim reportfooter As String = "UMSATZABFRAGE für " & OCBSaccountNamelbl & " Vostrokonto " & OCBSaccountlbl & " (" & OCBSaccountCurlbl & ")" & "von " & d1 & " bis " & d2 & " Account Status: " & OCBSaccountStatuslbl
    ...
    Private Sub Customer_Vostro_Balances_BasicView_RowStyle(sender As Object, e As RowStyleEventArgs) Handles Customer_Vostro_Balances_BasicView.RowStyle
    ...
    Private Sub Customer_Vostro_Balances_BasicView_ShownEditor(sender As Object, e As EventArgs) Handles Customer_Vostro_Balances_BasicView.ShownEditor
            ...
            If d2 >= d1 Then
                ...
                If IsNothing(Me.Account_LookUpEdit.Text) = False AndAlso IsNumeric(Me.Account_LookUpEdit.Text) = True Then
                    Try
                        ...
                        SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                        SplashScreenManager.Default.SetWaitFormCaption("Load  Closing Balances for Vostro Account: " & Me.Account_LookUpEdit.Text & " from: " & d1 & " till " & d2)
                        ...
                        Using sqlConn As New SqlConnection(My.Settings.PS_TOOL_DX_SQL_Client_ConnectionString)
                            Using sqlCmd As New SqlCommand()
                                sqlCmd.CommandText = "SELECT * FROM   CUSTOMER_VOSTRO_VOLUMES  WHERE  (AccountNo = '" & Me.Account_LookUpEdit.Text & "') AND (GL_Rep_Date >= '" & rdsql1 & "') AND (GL_Rep_Date <= '" & rdsql2 & "') AND BatchNo in ('CLOSING BALANCE') ORDER BY IdNr"
                                sqlCmd.Connection = sqlConn
                                sqlCmd.CommandTimeout = 5000
                                If sqlConn.State = ConnectionState.Closed Then
                                    sqlConn.Open()
                        ...
                        Me.SearchText_lbl.Text = "Closing Balances for Vostro Account: " & Me.Account_LookUpEdit.Text & " -Currency: " & OCBSaccountCurlbl & " from " & d1 & " till " & d2
