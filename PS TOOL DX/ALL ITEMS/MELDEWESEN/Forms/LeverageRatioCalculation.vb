...
Public Class LeverageRatioCalculation
    ...
    Private Sub CAR_Calc_Report_btn_Click(sender As Object, e As EventArgs) Handles CAR_Calc_Report_btn.Click
        If IsDate(Me.SolvDate_lbl.Text) = True Then
            ...
            SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
            SplashScreenManager.Default.SetWaitFormCaption("Creating LEVERAGE RATIO REPORT for " & Me.SolvDate_lbl.Text)
            Dim SOLVV_Date_Da As New SqlDataAdapter("Select * from [SOLVV_Date] where [Solvv_Date]='" & dsql & "' ", conn)
            Dim SOLVV_PositionsTotals_Da As New SqlDataAdapter("Select * from [SOLVV_PositionsTotals] where [CRSA_Position_Date]='" & dsql & "'", conn)
            Dim SOLVV_PositionsDetails_Da As New SqlDataAdapter("Select * from [SOLVV_PositionsDetails] where [RiskDate]='" & dsql & "'", conn)

            Dim SOLVV_Dataset As New DataSet("SOLVV_Date")
            SOLVV_Date_Da.Fill(SOLVV_Dataset, "SOLVV_Date")
            SOLVV_PositionsTotals_Da.Fill(SOLVV_Dataset, "SOLVV_PositionsTotals")
            SOLVV_PositionsDetails_Da.Fill(SOLVV_Dataset, "SOLVV_PositionsDetails")

            Dim CrRep As New ReportDocument
            CrRep.Load(CrystalRepDir & "\LEVERAGE_RATIO_REP.rpt")
            'Dim r As New INT_RATE_RISK_REP
            CrRep.SetDataSource(SOLVV_Dataset)
            ...
            c.Text = "Leverage Ratio report from " & d
    ...
    Private Sub CRSA_Totals_GridView_DoubleClick(sender As Object, e As EventArgs) Handles CRSA_Totals_GridView.DoubleClick
        Dim view As GridView = CType(sender, GridView)
        If view.FocusedRowHandle <> DevExpress.XtraGrid.GridControl.AutoFilterRowHandle Then
