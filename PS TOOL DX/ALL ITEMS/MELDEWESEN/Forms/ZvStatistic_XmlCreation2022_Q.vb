...
Imports Ionic.Zip
Public Class ZvStatistic_XmlCreation2022_Q
    ...
    Public Sub ZvStatistic_XmlCreation2022_Q_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Public Sub ZVSTA_2022_H_XML_CREATE(ByVal ZVSTA_MELDESCHEMA As String, ByVal ZVSTA_MELDEPERIODE As String)
        Try
            ...
            SplashScreenManager.Default.SetWaitFormCaption("Select ZV_STATISTIK_2022_Q Parameters for reporting Period: " & ZVSTA_MELDEPERIODE)
            System.Threading.Thread.Sleep(ExecutingSQLThreadSleep)
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                End If
                'ZVSTAT_2022_TEST_FILE
                If dt.Rows.Item(i).Item("PARAMETER1") = "ZVSTAT_2022_TEST_FILE" Then
                    XMLTESTFILE = dt.Rows.Item(i).Item("PARAMETER2")
                End If
            Next
            dt.Clear()

            'QueryText = "Select * from [PARAMETER_C] where  [ParameterStatus]='Y' and [ParameterCodeA]='REGREP' and [ParameterB_Name] in ('ZV_STATISTIK_2022_Q')"
            'da = New SqlDataAdapter(QueryText.Trim(), conn)
            'dt = New DataTable()
            'da.Fill(dt)
            ...
            '        'End If
            '        If dt.Rows.Item(i).Item("PARAMETER1").ToString = "ZVSTA_2022_Q_XML_CREATION_FOLDER" Then
            '            XMLSAVEFILE = dt.Rows.Item(i).Item("PARAMETER2").ToString
            '        End If
            '        If dt.Rows.Item(i).Item("PARAMETER1").ToString = "ZVSTA_2022_Q_XML_FILE_TEST" Then
            ...
            MyWriter = System.Xml.XmlWriter.Create(XMLSAVEFILE & "\payq_" & MFI_CODE & "_" & XMLMELDETERMIN & "_" & XMLMESSAGEID & ".xml", MySettings)

            'Check if FEHLANZEIGE or REPORT
            QueryText = "SELECT TOP 1 ID FROM [ZVSTAT_Reporting] where [ReportingPeriod]=" & MeldeJahr & " and [ZVSTA_Schema]='ZVSTA_2022_Q' and PositionName not in ('FEHLANZEIGE')"
            da = New SqlDataAdapter(QueryText.Trim(), conn)
            dt = New DataTable()
            da.Fill(dt)
            If dt.Rows.Count > 0 Then
                With MyWriter
                    ...
                    .WriteElementString("message", "Email", Nothing, ABSENDEREMAIL)
                    ...
                    '##############################
                    'Structure BBK_PYMNT_TRNSCTNS_C
                    .WriteStartElement("message", "Structure", Nothing)
                    .WriteAttributeString("structureID", "BBK_PYMNT_TRNSCTNS_C")
                    .WriteAttributeString("dimensionAtObservation", "AllDimensions")
                    .WriteAttributeString("namespace", "BBK_PYMNT_TRNSCTNS_C")
                    .WriteStartElement("common", "Structure", Nothing)
                    .WriteStartElement(Nothing, "Ref", Nothing)
                    .WriteAttributeString("agencyID", "BBK")
                    .WriteAttributeString("id", "BBK_PYMNT_TRNSCTNS_C")
                    .WriteEndElement() 'common structure
                    .WriteEndElement() 'Ref
                    .WriteEndElement() 'message Structure
                    '##############################
                    ...
                    .WriteAttributeString("SRVY_ID", "PAY_Q")
                    ...
                    'FORMULAR ZVS9+++++++++++++++++++++
                    QueryText = "SELECT * FROM [ZVSTAT_Reporting] where [FormNr]='ZVS9' and [ReportingPeriod]=" & MeldeJahr & " and [ZVSTA_Schema]='ZVSTA_2022_Q' and (SubdivisionOfCountryCode is NULL and SubdivisionOfCountryCode_T is NULL) and ([Anzahl_Value]<>0 OR [Wert_Value]<>0)"
                    da = New SqlDataAdapter(QueryText.Trim(), conn)
                    dt = New DataTable()
                    da.Fill(dt)
                    If dt.Rows.Count > 0 AndAlso Me.ZVS9_CheckEdit.CheckState = CheckState.Checked Then
                        .WriteStartElement("message", "DataSet", Nothing)
                        .WriteAttributeString("data", "structureRef", Nothing, "BBK_PYMNT_TRNSCTNS_C")
                        .WriteAttributeString("xsi", "type", Nothing, "PAY:BBK_PYMNT_TRNSCTNS_C")
                        .WriteAttributeString("data", "dataScope", Nothing, "DataStructure")
                        ...
                        For i = 0 To dt.Rows.Count - 1
                            ...
                            If dt.Rows.Item(i).Item("PayCardSystem").ToString <> "N" Then
                                .WriteAttributeString("MCC", dt.Rows.Item(i).Item("PayCardSystem").ToString)
                            End If
            ...
            System.Threading.Thread.Sleep(ExecutingSQLThreadSleep)
            Dim xmlfilestring As String = File.ReadAllText(XMLSAVEFILE & "\payq_" & MFI_CODE & "_" & XMLMELDETERMIN & "_" & XMLMESSAGEID & ".xml")
            File.WriteAllText(XMLSAVEFILE & "\payq_" & MFI_CODE & "_" & XMLMELDETERMIN & "_" & XMLMESSAGEID & ".xml", xmlfilestring.Replace("utf-8", "UTF-8"))
            ...
            If MessageBox.Show("Following ZVSTA XML file has being generated: " & vbNewLine & "payq_" & MFI_CODE & "_" & XMLMELDETERMIN & "_" & XMLMESSAGEID & ".xml" & vbNewLine _
    ...
    Private Sub SelectAll_btn_Click(sender As Object, e As EventArgs) Handles SelectAll_btn.Click
        ...
        Me.ZVS9_CheckEdit.CheckState = CheckState.Checked
    End Sub

    Private Sub ClearAll_btn_Click(sender As Object, e As EventArgs) Handles ClearAll_btn.Click

        Me.ZVS9_CheckEdit.CheckState = CheckState.Unchecked
    End Sub
