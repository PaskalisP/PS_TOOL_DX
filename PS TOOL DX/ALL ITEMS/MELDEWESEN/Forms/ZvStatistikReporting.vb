...
Public Class ZvStatistikReporting
    ...
    Private Sub BgwZVSTA_Processing_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgwZVSTA_Processing.DoWork
        Try
            ...
            Me.BgwZVSTA_Processing.ReportProgress(10, "Execute SQL Parameter: ZV_STAT\ZVSTAT_2022_PAYMENTS_LOAD - Load Payments into ZVSTA_Payments")
            System.Threading.Thread.Sleep(ExecutingSQLThreadSleep)
            cmd.CommandText = "Select [Status] FROM SQL_PARAMETER_DETAILS where  [SQL_Name_1] In ('ZVSTAT_2022_PAYMENTS_LOAD') and [Id_SQL_Parameters] in ('ZV_STAT')"
            Dim ParameterStatusZVSTA_LOAD As String = cmd.ExecuteScalar.ToString
            If ParameterStatusZVSTA_LOAD = "Y" Then
                QueryText = "Select * from SQL_PARAMETER_DETAILS_SECOND  where [Id_SQL_Parameters_Details] in (Select [ID] from SQL_PARAMETER_DETAILS where SQL_Name_1 in ('ZVSTAT_2022_PAYMENTS_LOAD')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
                    SqlCommandText = dt.Rows.Item(i).Item("SQL_Command_1").ToString.Replace("<ZVSTA_SCHEMA>", ZVSTA_SCHEMA).ToString.Replace("<ZVSTA_REPORTING_PERIOD>", ZVSTA_REPORTING_PERIOD)
    ...
    Private Sub BgwZVSTA_Executing_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgwZVSTA_Executing.DoWork
        Try
            ...
            Dim ParameterStatusZVSTA_LOAD As String = cmd.ExecuteScalar.ToString
            If ParameterStatusZVSTA_LOAD = "Y" Then
                QueryText = "Select * from SQL_PARAMETER_DETAILS_SECOND  where [Id_SQL_Parameters_Details] in (Select [ID] from SQL_PARAMETER_DETAILS where SQL_Name_1 in ('ZVSTAT_2022_PAYMENTS_LOAD_ONLY')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
