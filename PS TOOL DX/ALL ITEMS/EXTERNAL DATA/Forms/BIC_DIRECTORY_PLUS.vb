...
#Region "BIC_DIRECTORY_CHANGE_VIEWS"
    ...
    Protected Sub HideDetail(ByVal rowHandle As Integer)
        ...
        DisplayListDetails_bbi.ImageIndex = 4
    ...
    Protected Sub ShowDetail(ByVal rowHandle As Integer)
        ...
        DisplayListDetails_bbi.ImageIndex = 5
        fExtendedEditMode = (GridControl2.MainView Is BICLayoutView)
...
#End Region
    ...
    Private Sub BIC_DIRECTORY_PLUS_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub SearchField_ButtonEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles SearchField_ButtonEdit.ButtonClick
        ...
        If e.Button.Tag = 5 Then
            ...
            'Me.SearchField_BarEditItem.Links(0).Focus()
        End If
        If e.Button.Tag = 0 OrElse e.Button.Tag = 2 OrElse e.Button.Tag = 4 Then
    ...
    Private Sub BgwSearchDirectory_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgwSearchDirectory.DoWork
        Try
            ...
            Select Case SearchButtonClicked
                Case = 0
                    Me.BgwSearchDirectory.ReportProgress(10, "Start searching for Intitution Name like: " & SearchText)
                    'Data reader
                    Using Sqlconn As New SqlConnection(My.Settings.PS_TOOL_DX_SQL_Client_ConnectionString)
                        Using Sqlcmd As New SqlCommand()
                            Sqlcmd.CommandText = "SELECT * FROM [BIC_DIRECTORY_PLUS] where [INSTITUTION_NAME] LIKE LTRIM(RTRIM('%" & SearchText & "%')) ORDER BY [INSTITUTION_NAME] asc"
                ...
                Case = 2
                    Me.BgwSearchDirectory.ReportProgress(10, "Start searching for City Heading like: " & SearchText)
                    'Data reader
                    Using Sqlconn As New SqlConnection(My.Settings.PS_TOOL_DX_SQL_Client_ConnectionString)
                        Using Sqlcmd As New SqlCommand()
                            Sqlcmd.CommandText = "SELECT * FROM [BIC_DIRECTORY_PLUS] where [CITY_HEADING] LIKE LTRIM(RTRIM('%" & SearchText & "%')) ORDER BY [CITY_HEADING] asc"
                ...
                Case = 4
                    Me.BgwSearchDirectory.ReportProgress(10, "Start searching for Country Name like: " & SearchText)
                    'Data reader
                    Using Sqlconn As New SqlConnection(My.Settings.PS_TOOL_DX_SQL_Client_ConnectionString)
                        Using Sqlcmd As New SqlCommand()
                            Sqlcmd.CommandText = "SELECT * FROM [BIC_DIRECTORY_PLUS] where [COUNTRY_NAME] LIKE LTRIM(RTRIM('%" & SearchText & "%')) ORDER BY [COUNTRY_NAME] asc"
    ...
    Private Sub BICPrint_Export_btn_Click(sender As Object, e As EventArgs)
