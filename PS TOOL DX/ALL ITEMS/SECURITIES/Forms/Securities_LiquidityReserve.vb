...
#End Region
    ...
    Private Sub SecuritiesLiquidBaseView_RowStyle(sender As Object, e As RowStyleEventArgs)
        If e.RowHandle = DevExpress.XtraGrid.GridControl.AutoFilterRowHandle Then
            e.Appearance.BackColor = SystemColors.InactiveCaptionText
        End If
    End Sub

    Private Sub SecuritiesLiquidBaseView_ShownEditor(sender As Object, e As EventArgs)
            ...
            Try
                If IsDate(Me.LiquidityDateEdit.Text) = True Then
                    ...
                    Else 'Liquidity Date exists
                        If MessageBox.Show("The Liquidity reserve Date is present in the current Table" & vbNewLine & "Should the Liquidity reserve be re-created for the given Date?", "LIQUIDITY RESERVE PRESENT", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                            'Check if DailyPrice exists with the same liquidity date
                            cmd.CommandText = "Select distinct [Date] from [SECURITIES_DailyPrice] where [Date]='" & DateSql & "' "
                            Dim resultDP As String = cmd.ExecuteScalar
                            'DailyPrice Date exists
