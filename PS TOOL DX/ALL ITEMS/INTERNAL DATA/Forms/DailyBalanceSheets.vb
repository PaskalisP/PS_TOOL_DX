...
Public Class DailyBalanceSheets
    ...
    Private Sub DailyBalanceSheets_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If EDP_USER = "Y" Or ACCOUNTING_USER = "Y" Then
            ...
            Me.FromDateEdit.Properties.Buttons.Item(3).Visible = True
            GL_ITEMS_initData()
            GL_ITEMS_InitLookUp()
    ...
    Private Sub FromDateEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles FromDateEdit.ButtonClick
        ...
        If e.Button.Caption = "Auto Adjust" Then
            If XtraMessageBox.Show("Should the automated Balance Sheet Adjustment be started?" & vbNewLine & "All GL Item Sums will be recalculated!!", "AUTOMATED BALANCE SHEET ADJUSTMENT", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                Try
                    rd = Me.FromDateEdit.Text
                    rdsql = rd.ToString("yyyyMMdd")
                    If cmd.Connection.State = ConnectionState.Closed Then
                    ...
                    cmd.CommandText = "SELECT [Status] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('DAILY_BALANCE_SHEET_ADJUSTMENT') and [Id_SQL_Parameters] in ('SEVERAL SELECTIONS')"
                    ParameterStatus = cmd.ExecuteScalar
                    If ParameterStatus = "Y" Then
                        SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                        SplashScreenManager.Default.SetWaitFormCaption("Start SEVERAL SELECTIONS - DAILY_BALANCE_SHEET_ADJUSTMENT")

                        Me.QueryText = "Select * from [SQL_PARAMETER_DETAILS_SECOND]  where [Id_SQL_Parameters_Details] in (Select [ID] from SQL_PARAMETER_DETAILS where SQL_Name_1 in ('DAILY_BALANCE_SHEET_ADJUSTMENT')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
                        da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                        dt = New System.Data.DataTable()
                        da.Fill(dt)
                        For i = 0 To dt.Rows.Count - 1
                            ...
                            End If
                        Next
                        SplashScreenManager.Default.SetWaitFormCaption("Recalculate all Sums in GL Items")
                        ...
                        End If
                    Else
                        XtraMessageBox.Show("SQL Parameter:SEVERAL SELECTIONS/DAILY_BALANCE_SHEET_ADJUSTMENT is Invalid", "Invalid Parameter", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1)
                        Return
                    End If
                    If cmd.Connection.State = ConnectionState.Open Then
                        cmd.Connection.Close()
    ...
    Private Sub linkGrid22Report_CreateDetailArea(ByVal sender As Object,
    ByVal e As CreateAreaEventArgs)
        Dim tb As TextBrick = New TextBrick()
    ...
    Private Sub Add_btn_Click(sender As Object, e As EventArgs) Handles Add_btn.Click
        If Me.DxValidationProvider1.Validate() = True Then
            ...
            If XtraMessageBox.Show("Should the Data be inserted to the Balance Sheet?" & vbNewLine & "All GL Item Sums will be recalculated!!", "INSERT GL ACCOUNT DATA TO BALANCE SHEET", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                Try
                    SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
    ...
    Private Sub DeleteManual_ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DeleteManual_ToolStripMenuItem.Click
        If IsNothing(ID_toDELETE) = False AndAlso Insert_Status = "M" Then
            rd = Me.FromDateEdit.Text
            rdsql = rd.ToString("yyyyMMdd")
            If XtraMessageBox.Show("Should the current selected manual Inserted GL Item with the ID: " & ID_toDELETE & vbNewLine & "be deleted?" & vbNewLine & "All GL Item Sums will be reclaculated!!", "DELETE MANUANL INSERTED GL ITEM", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                Try
                    SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
    ...
    Private Sub FromDateEdit_GotFocus(sender As Object, e As EventArgs) Handles FromDateEdit.GotFocus
        Me.FromDateEdit.Properties.Buttons.Item(0).Appearance.ForeColor = Color.Black
        Me.FromDateEdit.Properties.Buttons.Item(0).AppearanceHovered.ForeColor = DefaultForeColor
        Me.FromDateEdit.Properties.Buttons.Item(1).Appearance.ForeColor = Color.Black
        Me.FromDateEdit.Properties.Buttons.Item(1).AppearanceHovered.ForeColor = DefaultForeColor
        Me.FromDateEdit.Properties.Buttons.Item(2).Appearance.ForeColor = Color.Black
        Me.FromDateEdit.Properties.Buttons.Item(2).AppearanceHovered.ForeColor = DefaultForeColor
        Me.FromDateEdit.Properties.Buttons.Item(3).Appearance.ForeColor = Color.Black
        Me.FromDateEdit.Properties.Buttons.Item(3).AppearanceHovered.ForeColor = DefaultForeColor
    End Sub

    Private Sub FromDateEdit_LostFocus(sender As Object, e As EventArgs) Handles FromDateEdit.LostFocus
        Me.FromDateEdit.Properties.Buttons.Item(0).Appearance.ForeColor = DefaultForeColor
        Me.FromDateEdit.Properties.Buttons.Item(0).AppearanceHovered.ForeColor = DefaultForeColor
        Me.FromDateEdit.Properties.Buttons.Item(1).Appearance.ForeColor = DefaultForeColor
        Me.FromDateEdit.Properties.Buttons.Item(2).Appearance.ForeColor = DefaultForeColor
        Me.FromDateEdit.Properties.Buttons.Item(3).Appearance.ForeColor = DefaultForeColor
    End Sub
End Class