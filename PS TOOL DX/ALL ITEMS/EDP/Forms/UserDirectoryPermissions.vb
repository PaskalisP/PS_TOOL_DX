    ...
    Private Sub AddNewPSTOOL_User_btn_Click(sender As Object, e As EventArgs) Handles AddNewPSTOOL_User_btn.Click
        ...
        Try
            If Me.PS_UserLoginName_ComboBoxEdit.Text <> "" Then
                If Me.SuperUser_CheckEdit.CheckState = CheckState.Checked Then
                    If MessageBox.Show("Should the Windows User: " & Me.PS_UserLoginName_ComboBoxEdit.Text & " be added in the PS TOOL Users as SUPER USER?", "ADD NEW PS TOOL USER", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
               ...
               TRES_CheckEdit.CheckState = CheckState.Checked OrElse
               COMPLIANCE_CheckEdit.CheckState = CheckState.Checked OrElse CLEARING_CheckEdit.CheckState = CheckState.Checked Then
                    If MessageBox.Show("Should the Windows User: " & Me.PS_UserLoginName_ComboBoxEdit.Text & " be added in the PS TOOL Users for specific Departments?", "ADD NEW PS TOOL USER", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                        'FOR SQL SERVER SPECIFIC
                        cmd.CommandText = "CREATE LOGIN [CCB-FRANKFURT\" & PS_UserLoginName_ComboBoxEdit.Text & "] FROM WINDOWS WITH DEFAULT_DATABASE=[PS TOOL DX Live], DEFAULT_LANGUAGE=[us_english]"
                        cmd.ExecuteNonQuery()
                        cmd.CommandText = "EXEC master..sp_addsrvrolemember @loginame = N'CCB-FRANKFURT\" & PS_UserLoginName_ComboBoxEdit.Text & "', @rolename = N'serveradmin'"
                ...
                If TRES_CheckEdit.CheckState = CheckState.Checked Then
                    ...
                    cmd.ExecuteNonQuery()
                End If
                If CLEARING_CheckEdit.CheckState = CheckState.Checked Then
                    'PS TOOL SPECIFIC
                    cmd.CommandText = "DELETE FROM [PARAMETER] where [PARAMETER2]='" & PS_UserLoginName_ComboBoxEdit.Text & "' and [IdABTEILUNGSPARAMETER]='CLEARING_USER' and [IdABTEILUNGSCODE_NAME]='CLEARING'"
                    cmd.ExecuteNonQuery()
                    cmd.CommandText = "INSERT INTO [dbo].[PARAMETER]([PARAMETER1],[PARAMETER2],[PARAMETER STATUS],[IdABTEILUNGSPARAMETER],[IdABTEILUNGSCODE_NAME])VALUES ((Select Surname + '  ' + Name from [ActiveDirectoryUsers] where Member_Name='" & PS_UserLoginName_ComboBoxEdit.Text & "'),'" & PS_UserLoginName_ComboBoxEdit.Text & "','Y','CLEARING_USER','CLEARING')"
                    cmd.ExecuteNonQuery()
                End If
                If COMPLIANCE_CheckEdit.CheckState = CheckState.Checked Then
                    'PS TOOL SPECIFIC
                    cmd.CommandText = "DELETE FROM [PARAMETER] where [PARAMETER2]='" & PS_UserLoginName_ComboBoxEdit.Text & "' and [IdABTEILUNGSPARAMETER]='COMPLIANCE_USERS' and [IdABTEILUNGSCODE_NAME]='COMPLIANCE'"
                    cmd.ExecuteNonQuery()
                    cmd.CommandText = "INSERT INTO [dbo].[PARAMETER]([PARAMETER1],[PARAMETER2],[PARAMETER STATUS],[IdABTEILUNGSPARAMETER],[IdABTEILUNGSCODE_NAME])VALUES ((Select Surname + '  ' + Name from [ActiveDirectoryUsers] where Member_Name='" & PS_UserLoginName_ComboBoxEdit.Text & "'),'" & PS_UserLoginName_ComboBoxEdit.Text & "','Y','COMPLIANCE_USERS','COMPLIANCE')"
                    cmd.ExecuteNonQuery()
                End If
