...
Public Class SqlFilesCompare
    ...
    Private Sub SqlFilesCompare_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub SqlFilesCompare_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub SqlFilesCompare_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub DISABLE_CONTROLS_ON_SQL_EXECUTION()
        ...
        Me.bbiSecondFile.Enabled = False
    ...
    Private Sub BbiExecuteSqlCommand_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BbiExecuteSqlCommand.ItemClick
        If Me.bbiFirstFile.Text <> "" AndAlso Me.bbiSecondFile.Text <> "" Then
            'EXCEL
            If FIRST_FILE_EXTENSION.ToUpper.StartsWith(".XLS") AndAlso SECOND_FILE_EXTENSION.ToUpper.StartsWith(".XLS") Then
                If XtraMessageBox.Show(CType("Should the Files: " & FIRST_FILE_NAME & vbNewLine & SECOND_FILE_NAME & " be compared together? ", String), "EXCEL FILES COMPARE", buttons:=MessageBoxButtons.YesNo, icon:=MessageBoxIcon.Question, defaultButton:=MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                    Try
            ...
            End If
            'CSV
            If FIRST_FILE_EXTENSION.ToUpper.StartsWith(".CSV") AndAlso SECOND_FILE_EXTENSION.ToUpper.StartsWith(".CSV") Then
                If Me.CsvFieldseparator_File1_cmb.EditValue.ToString <> "" AndAlso Me.CsvFieldseparator_File2_cmb.EditValue.ToString <> "" Then
                    If XtraMessageBox.Show(CType("Should the Files: " & FIRST_FILE_NAME & vbNewLine & SECOND_FILE_NAME & " be compared together? ", String), "CSV FILES COMPARE", buttons:=MessageBoxButtons.YesNo, icon:=MessageBoxIcon.Question, defaultButton:=MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                        Try
                            DISABLE_CONTROLS_ON_SQL_EXECUTION()
    ...
    Private Sub BgwSqlExecute_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgwSqlExecute.DoWork
        ...
        Try
            ...
            Me.BgwSqlExecute.ReportProgress(10, "Check validity of SQL Parameter:SEVERAL SELECTIONS\SQL_FILES_COMPARE\EXCEL_FILES_COMPARE")
            cmd.CommandText = "SELECT [Status] FROM [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('EXCEL_FILES_COMPARE') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where [SQL_Name_1] in ('SQL_FILES_COMPARE'))"
            Dim ParameterStatus As String = cmd.ExecuteScalar.ToString
            If ParameterStatus = "Y" Then
                QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD]  where [Id_SQL_Parameters_Details] in (Select [ID] from [SQL_PARAMETER_DETAILS_SECOND] where SQL_Name_1 in ('EXCEL_FILES_COMPARE')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
                ...
                For i = 0 To dtSqlFileCompare.Rows.Count - 1
                    SqlCommandText = dtSqlFileCompare.Rows.Item(i).Item("SQL_Command_1").ToString.Replace("<IMPORT_DIR_FILE1>", Me.bbiFirstFile.Text).ToString.Replace("<IMPORT_DIR_FILE2>", Me.bbiSecondFile.Text)
                    If FIRST_FILE_SHEET <> Nothing Then
                        SqlCommandText = SqlCommandText.ToString.Replace("<ExcelSheet1>", FIRST_FILE_SHEET)
                    End If
                    If SECOND_FILE_SHEET <> Nothing Then
                        SqlCommandText = SqlCommandText.ToString.Replace("<ExcelSheet2>", SECOND_FILE_SHEET)
                    End If
                    cmd.CommandText = SqlCommandText
                    If dtSqlFileCompare.Rows.Item(i).Item("SQL_Name_1").ToString <> "" Then
                        Me.BgwSqlExecute.ReportProgress(10, dtSqlFileCompare.Rows.Item(i).Item("SQL_Name_1").ToString & " - SQL Procedure:SQL_FILES_COMPARE\EXCEL_FILES_COMPARE - Nr.: " & dtSqlFileCompare.Rows.Item(i).Item("SQL_Float_1").ToString)
                        'cmd.ExecuteNonQuery()
    ...
    Private Sub BgwSqlExecute_ProgressChanged(sender As Object, e As ProgressChangedEventArgs) Handles BgwSqlExecute.ProgressChanged
        ...
        OpenSqlConnections()
        Try
            cmd1.CommandText = "INSERT INTO [Events_Journal] ([ProcDate],[Event],[ProcName],[SystemName],[SystemUser]) Values(GETDATE(),@Event,'EXCEL_FILES_COMPARISON','FILES_COMPARISON','" & CurrentUserWindowsID & "')"
        ...
        Catch ex As Exception
            cmd1.CommandText = "INSERT INTO [Events_Journal] ([ProcDate],[Event],[ProcName],[SystemName],[SystemUser]) Values(GETDATE(),@ErrorMessage,'EXCEL_FILES_COMPARISON','FILES_COMPARISON','" & CurrentUserWindowsID & "')"
        ...
        If dt1SqlFileCompare IsNot Nothing AndAlso dt1SqlFileCompare.Rows.Count > 0 Then
            Try
                ...
                For Each col As GridColumn In Gridview1.Columns
                    If col.FieldName.ToUpper.StartsWith("AMOUNT") Or col.FieldName.ToUpper.Contains("AMOUNT") Or col.FieldName.ToUpper.EndsWith("AMOUNT") _
                         Or col.FieldName.EndsWith("_bal") _
                        Or col.FieldName.Contains("Credit Exposure") Or col.FieldName.EndsWith("_gamt") _
                        Or col.FieldName.Contains("EUR Equ") Or col.FieldName = "Principal" Or col.FieldName = "c215" _
                        Or col.FieldName.ToUpper.Contains("BUCHWERT") Or col.FieldName.ToUpper.EndsWith("BUCHWERT") Or col.FieldName.ToUpper.StartsWith("BUCHWERT") Or col.FieldName = "c215" _
                        Or col.FieldName = "balance" Or col.FieldName = "int_accru" Then
    ...
    Private Sub PrintableComponentLink1_CreateMarginalHeaderArea(sender As Object, e As CreateAreaEventArgs) Handles PrintableComponentLink1.CreateMarginalHeaderArea
        Dim reportfooter As String = "File compare result"
    ...
    Private Sub BbiNewSqlQueryForm_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BbiNewSqlQueryForm.ItemClick
        SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
        SplashScreenManager.Default.SetWaitFormCaption("SQL FILES COMPARE")
        ' Place code here
    ...
    Private Sub BbiClearCurrentSqlCommand_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BbiClearCurrentSqlCommand.ItemClick
        ...
        Me.bbiSecondFile.ResetText()
        Me.CsvFieldseparator_File1_LayoutControlItem.Visibility = LayoutVisibility.Never
        Me.CsvFieldseparator_File2_LayoutControlItem.Visibility = LayoutVisibility.Never
    ...
    Private Sub bbiFirstFile_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles bbiFirstFile.ButtonClick
        If e.Button.Tag Is "OpenFirstFile" Then
            With OpenFileDialog1
                If Me.bbiFirstFile.Text = "" And Me.bbiSecondFile.Text = "" Then
                    .Filter = "Excel Files|*.xls;*.xlsx|Comma separated Value Files|*.csv;*.CSV"
                ElseIf Me.bbiSecondFile.Text.ToUpper.ToString.EndsWith(".XLS") Or Me.bbiSecondFile.Text.ToUpper.ToString.EndsWith(".XLSX") Then
                    .Filter = "Excel Files|*.xls;*.xlsx"
                ElseIf Me.bbiSecondFile.Text.ToUpper.ToString.EndsWith(".CSV") Then
                    ...
                    If IsNothing(Me.OpenFileDialog1.FileName) = False Then
                        ...
                        Select Case FIRST_FILE_EXTENSION
                            ...
                            Case ".csv", ".CSV"
                                ...
                                Me.CsvFieldseparator_File1_cmb.EditValue = Me.CsvFieldseparator_File1_cmb.Properties.Items(0).ToString
                        End Select
                    End If
                End If
            End With
    ...
    Private Sub bbiSecondFile_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles bbiSecondFile.ButtonClick
        If e.Button.Tag Is "OpenSecondFile" Then
            With OpenFileDialog1
                If Me.bbiFirstFile.Text = "" And Me.bbiSecondFile.Text = "" Then
                    .Filter = "Excel Files|*.xls;*.xlsx|Comma separated Value Files|*.csv;*.CSV"
                ...
                If Me.OpenFileDialog1.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                    If IsNothing(Me.OpenFileDialog1.FileName) = False Then
                        Me.bbiSecondFile.Text = Me.OpenFileDialog1.FileName
                        SECOND_FILE = Me.bbiSecondFile.Text
                        SECOND_FILE_EXTENSION = System.IO.Path.GetExtension(Me.OpenFileDialog1.FileName)
                        SECOND_FILE_NAME = System.IO.Path.GetFileName(Me.OpenFileDialog1.FileName)
                        Select Case SECOND_FILE_EXTENSION
                            Case ".xls", ".xlsx"
                                Dim workbook As Workbook = New DevExpress.Spreadsheet.Workbook()
                                workbook.LoadDocument(Me.OpenFileDialog1.FileName, DevExpress.Spreadsheet.DocumentFormat.Xlsx)
                                Dim worksheet As Worksheet = workbook.Worksheets(0)
                                SECOND_FILE_SHEET = worksheet.Name
                            Case ".csv", ".CSV"
                                Me.CsvFieldseparator_File2_LayoutControlItem.Visibility = LayoutVisibility.Always
                                Me.CsvFieldseparator_File2_cmb.EditValue = Me.CsvFieldseparator_File2_cmb.Properties.Items(0).ToString
                        End Select
                    End If
                End If
            End With
    ...
    Private Sub BgwSqlCsvExecute_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgwSqlCsvExecute.DoWork
        Try
            Me.BgwSqlCsvExecute.ReportProgress(10, "Execute csv file comparison")
            Me.BgwSqlCsvExecute.ReportProgress(10, "Check validity of SQL Parameter:SEVERAL SELECTIONS\SQL_FILES_COMPARE\CSV_FILES_COMPARE")
            OpenSqlConnections()
            cmd.CommandText = "SELECT [Status] FROM [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('CSV_FILES_COMPARE') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where [SQL_Name_1] in ('SQL_FILES_COMPARE'))"
            Dim ParameterStatus As String = cmd.ExecuteScalar.ToString
            If ParameterStatus = "Y" Then
                QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD]  where [Id_SQL_Parameters_Details] in (Select [ID] from [SQL_PARAMETER_DETAILS_SECOND] where SQL_Name_1 in ('CSV_FILES_COMPARE')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
                daSqlFileCompare = New SqlDataAdapter(QueryText.Trim(), conn)
                dtSqlFileCompare = New System.Data.DataTable()
                daSqlFileCompare.Fill(dtSqlFileCompare)
                For i = 0 To dtSqlFileCompare.Rows.Count - 1
                    SqlCommandText = dtSqlFileCompare.Rows.Item(i).Item("SQL_Command_1").ToString.Replace("<IMPORT_DIR_FILE1>", Me.bbiFirstFile.Text).ToString.Replace("<IMPORT_DIR_FILE2>", Me.bbiSecondFile.Text).ToString.Replace("<FIELDTERMINATOR_1>", Me.CsvFieldseparator_File1_cmb.EditValue.ToString).ToString.Replace("<FIELDTERMINATOR_2>", Me.CsvFieldseparator_File2_cmb.EditValue.ToString)
                    cmd.CommandText = SqlCommandText
                    If dtSqlFileCompare.Rows.Item(i).Item("SQL_Name_1").ToString <> "" Then
                        Me.BgwSqlCsvExecute.ReportProgress(10, dtSqlFileCompare.Rows.Item(i).Item("SQL_Name_1").ToString & " - SQL Procedure:SQL_FILES_COMPARE\CSV_FILES_COMPARE - Nr.: " & dtSqlFileCompare.Rows.Item(i).Item("SQL_Float_1").ToString)
                        cmd.ExecuteNonQuery()
                        System.Threading.Thread.Sleep(ExecutingSQLThreadSleep)
                        da1SqlFileCompare = New SqlDataAdapter(SqlCommandText.Trim(), conn)
                        da1SqlFileCompare.SelectCommand.CommandTimeout = 50000
    ...
    Private Sub BgwSqlCsvExecute_ProgressChanged(sender As Object, e As ProgressChangedEventArgs) Handles BgwSqlCsvExecute.ProgressChanged
        ...
        OpenSqlConnections()
        Try
            cmd1.CommandText = "INSERT INTO [Events_Journal] ([ProcDate],[Event],[ProcName],[SystemName],[SystemUser]) Values(GETDATE(),@Event,'CSV_FILES_COMPARISON','FILES_COMPARISON','" & CurrentUserWindowsID & "')"
            cmd1.Parameters.Add("@Event", SqlDbType.NVarChar).Value = e.UserState.ToString
            cmd1.ExecuteNonQuery()
            cmd1.Parameters.Clear()
            'TextImportFileRow = Now & "  " & "CUSTOMER INFO IMPORT" & "  " & e.UserState & "  " & "MANUAL IMPORTS"
            'System.IO.File.AppendAllText(ImportEventsDirectory, TextImportFileRow & vbCrLf)
        Catch ex As Exception
            cmd1.CommandText = "INSERT INTO [Events_Journal] ([ProcDate],[Event],[ProcName],[SystemName],[SystemUser]) Values(GETDATE(),@ErrorMessage,'CSV_FILES_COMPARISON','FILES_COMPARISON','" & CurrentUserWindowsID & "')"
            cmd1.Parameters.Add("@ErrorMessage", SqlDbType.NVarChar).Value = "ERROR +++ " & ex.Message.ToString
            cmd1.ExecuteNonQuery()
            cmd1.Parameters.Clear()
            'TextImportFileRow = Now & "  " & "CUSTOMER INFO IMPORT" & "  " & e.UserState & "  " & "MANUAL IMPORTS"
