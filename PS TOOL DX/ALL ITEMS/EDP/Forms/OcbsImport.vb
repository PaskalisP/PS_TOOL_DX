...
#Region "BUTTONS_AND_EDITORS_EVENTS"
    Private Sub LastOcbsImportFile_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles LastOcbsImportFile.ButtonClick
        If IsNumeric(LastOcbsImportFile.Text) = True And Len(LastOcbsImportFile.Text) = 8 Then
            If MessageBox.Show("Should the NGS File Nr. " & LastOcbsImportFile.Text & " be saved as Last Imported NGS file", "CHANGE THE LAST IMPORTED NGS FILE", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub OcbsAutomatedImport_btn_Click(sender As Object, e As EventArgs) Handles OcbsAutomatedImport_btn.Click
        ...
        If Me.LastOcbsImportFile.Text <> "" Then
            If MessageBox.Show("Should the automated NGS Fileimport be executed?" & vbNewLine & vbNewLine & "ATTENTION!!! - PLEASE SAVE & CLOSE ALL OPEN EXCEL FILES BEFORE START THE IMPORT", "AUTOMATED IMPORT NGS FILES", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
...
#Region "OCBS_IMPORT_BACKGROUNDWORKER"
    Private Sub BgwOCBSimport_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgwOCBSimport.DoWork
        ...
        If OcbsSelectedFileImport_btn_clicked = True Then
            Try
                ...
                MsgBox(OCBSDirectory & Me.SelectedOcbsImportFile.Text)
    ...
    Private Sub BgwOCBSimport_ProgressChanged(sender As Object, e As ProgressChangedEventArgs) Handles BgwOCBSimport.ProgressChanged
        ...
        Try
            cmdEVENT.CommandText = "INSERT INTO [IMPORT EVENTS] ([EVENT],[ProcDate],[ProcName],[SystemName]) Values('" & Me.EVENTSloadtext.Text & "',(SELECT DATEADD(dd, 0, DATEDIFF(dd, 0, GETDATE()))),'" & Me.CURRENT_PROCEDURE_Text.Text & "','NGS')"
        ...
        Catch ex As System.Exception
            cmdEVENT.CommandText = "INSERT INTO [IMPORT EVENTS] ([EVENT],[ProcDate],[ProcName],[SystemName]) Values('ERROR +++ " & ex.Message.ToString.Replace("'", " ") & "',(SELECT DATEADD(dd, 0, DATEDIFF(dd, 0, GETDATE()))),'" & Me.CURRENT_PROCEDURE_Text.Text & "','NGS')"
            cmdEVENT.ExecuteNonQuery()
            '***************************************************
            TextImportFileRow = Now & "  " & "NGS" & "  " & Me.EVENTSloadtext.Text & "  " & Me.CURRENT_PROCEDURE_Text.Text
            System.IO.File.AppendAllText(ImportEventsDirectory, TextImportFileRow & vbCrLf)
    ...
    Private Sub StartImport_RepositoryItemButtonEdit_Click(sender As Object, e As EventArgs) Handles StartImport_RepositoryItemButtonEdit.Click
        If Me.SelectedOcbsImportFile.Text <> "" Then
            If OcbsImportProcedures_BasicView.GetFocusedRowCellValue(colValid) = "Y" Then
                DISABLE_START_IMPORT()
                If MessageBox.Show("Should the NGS Import Procedure: " & OcbsImportProcedures_BasicView.GetFocusedRowCellValue(colProcName) & " be executed?", "Start NGS manual Import", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                    Try
                        SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
    ...
    Private Sub OCBS_IMPORT_EXCHANGE_RATES_OCBS()
        Try
            ...
            If cmd.ExecuteScalar > 0 Then
                ...
                Using zip As Ionic.Zip.ZipFile = Ionic.Zip.ZipFile.Read("\\wdmycloud\Public\ImportFiles\SMIS_REPORT_ALL_" & rdsql & "_710030000.zip")
                ...
                Dim ExcelFileName As String = ""
                ExcelFileName = Me.OCBS_Temp_Directory & "\AI-D-218_" & rdsql & "_710030000_norm.xls"

                Me.BgwOCBSimport.ReportProgress(20, "Procedure:" & CURRENT_PROC & " - " & "Import File: ...\AI-D-218_" & rdsql & "_710030000_norm.xls-Sheet:Rate-2")
                ...
                If File.Exists(ExcelFileName) = True Then
                    ...
                    cmd.CommandText = "IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='#Temp_IMPORT_EXCHANGE_RATES_OCBS' AND xtype='U') CREATE TABLE [#Temp_IMPORT_EXCHANGE_RATES_OCBS]([ID] [int] IDENTITY(1,1) NOT NULL,[CURRENCY CODE] [nvarchar](Max) NULL,[CURRENCY NAME][nvarchar](255) NULL,[MIDDLE RATE] [float] NULL) ELSE DELETE FROM [#Temp_IMPORT_EXCHANGE_RATES_OCBS] "
                    cmd.ExecuteNonQuery()
                    ...
                    cmd.CommandText = "INSERT INTO [#Temp_IMPORT_EXCHANGE_RATES_OCBS] ([CURRENCY CODE],[MIDDLE RATE]) SELECT LTRIM(RTRIM(LEFT(F1,3))),ROUND(1/F3,5) FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0', 'EXCEL 12.0;HDR=NO;Database=" & ExcelFileName & ";','SELECT * FROM [Rate-2$A9:F1000]')"
                    cmd.ExecuteNonQuery()
                    'Me.BgwOCBSimport.ReportProgress(50, "Procedure:" & CURRENT_PROC & " - " & "Exchange rates IMPORTED to Table:#Temp_IMPORT_EXCHANGE_RATES_OCBS")
                    'Count Imported Rows
                    cmd.CommandText = "SELECT COUNT([ID]) FROM [#Temp_IMPORT_EXCHANGE_RATES_OCBS]"
                    ...
                    cmd.CommandText = "DELETE FROM [#Temp_IMPORT_EXCHANGE_RATES_OCBS] WHERE [CURRENCY CODE] NOT IN (Select [OWN_CURRENCIES].[CURRENCY CODE] from [OWN_CURRENCIES])"
                    cmd.ExecuteNonQuery()
        ...
        Try
            ...
            If cmd.ExecuteScalar > 0 Then
                ...
                If Result > 0 Then
                    ...
                    Me.BgwOCBSimport.ReportProgress(12, "Procedure:" & CURRENT_PROC & " - " & "Start Import to the Temporary Table: #Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP")
                    cmd.CommandText = "IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='#Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP' AND xtype='U') 
                                       CREATE TABLE [#Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP]
                                        ([ID] [int] IDENTITY(1,1) NOT NULL
                                        ,[GL_ACC_NR] float NULL
                                        ,[ValDateFrom] [datetime] NULL
                                        ,[ValDate] [datetime] NULL
                                        ,[Customer] [nvarchar](255) NULL
                                        ,[ValYear] [float] NULL
                                        ,[CustomerName] [nvarchar](255) NULL
                                        ,[Account] [nvarchar](255) NULL
                                        ,[RegistrationCountry] [nvarchar](255) NULL
                                        ,[Contract] [nvarchar](255) NULL
                                        ,[CCY] [nvarchar](255) NULL
                                        ,[Product] [nvarchar](255) NULL
                                        ,[Amount] [float] NULL
                                        ,[ExchangeRate] [float] NULL
                                        ,[AmountEuro] [float] NULL
                                        ,[DB] [nvarchar](255) NULL
                                        ,[KapertstG] [float] NULL
                                        ,[Remark] [nvarchar](255) NULL
                                        ,[Soli] [float] NULL
                                        ,[KAPISTPFLICHTIG] [nvarchar](255) NULL
                                        ,[BUNDESLAND] [nvarchar](255) NULL
                                        ,[IdValueCustomer] [nvarchar](255) NULL
                                        ,[IdZinsertragsMonat] [nvarchar](255) NULL
                                        ,[IdErtragJahr] [float] NULL) 
                                        ELSE DELETE FROM [#Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP] "
                    cmd.ExecuteNonQuery()
                    'In temporäre Tabelle einfügen
                    cmd.CommandText = "INSERT INTO [#Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP] 
                                        ([GL_ACC_NR]
                                  ...
                                  SELECT NULL
                                        ...
                                        ,'INTEREST ON A/C'
                                        ,ClientName 
                                        FROM CUSTOMER_ACC_BALANCES 
                                        where BalanceDate='" & rdsql & "' and  [CR_Interest]>0"
                    cmd.ExecuteNonQuery()
                    cmd.CommandText = "DECLARE @RISKDATE Datetime='" & rdsql & "'
                                       DECLARE @FORELAST_RISKDATE Datetime =(SELECT DATEADD(DD,-(DAY(@RISKDATE)+1), DATEADD(MM, 1, @RISKDATE)))"
                    Me.BgwOCBSimport.ReportProgress(5, "Procedure:" & CURRENT_PROC & " - " & "Determine Interest Payment Year and Month...after seting Language in German")
                    cmd.CommandText = "SET LANGUAGE German"
                    cmd.ExecuteNonQuery()
                    cmd.CommandText = "UPDATE [#Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP] 
                                        SET [ValYear]=DATEPART(Year,[ValDate])
                                        ,[IdErtragJahr]=DATEPART(Year,[ValDate])
                                        ,[IdValueCustomer]=CONVERT(VARCHAR(10), [ValDate], 104)+[Account]
                                        ,[IdZinsertragsMonat]=DATENAME(MONTH, [ValDate])+ ' ' + LTRIM(RTRIM(STR(DATEPART(Year,[ValDate]),4,0)))"
                    cmd.ExecuteNonQuery()
                    cmd.CommandText = "SET LANGUAGE English"
                    cmd.ExecuteNonQuery()
                    Me.BgwOCBSimport.ReportProgress(6, "Procedure:" & CURRENT_PROC & " - " & "Check if Customer Nr is in Table:ZINSERTRAG KDBASIC and insert if not...")
                    cmd.CommandText = "INSERT INTO [ZINSERTRAG KDBASIC]
                                        ([KDSTAMM],[IdBank]) 
                                       SELECT distinct [Customer],'3' 
                                        FROM [#Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP] 
                                        Where [Customer] not in (Select [KDSTAMM] from [ZINSERTRAG KDBASIC])"
                    cmd.ExecuteNonQuery()
                    Me.BgwOCBSimport.ReportProgress(8, "Procedure:" & CURRENT_PROC & " - " & "Check if ZINSERTRAGSJAHR in Table ZINSERTRAG KUNDEN JAHR")
                    cmd.CommandText = "INSERT INTO [ZINSERTRAG KUNDEN JAHR]
                                       ([ErtragsJahr],[IdBank]) 
                                        SELECT 
                                        DATEPART(Year,[ValDate]),'3' from 
                                        [#Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP] 
                                        where DATEPART(Year,[ValDate]) not in  (Select [ErtragsJahr] from  [ZINSERTRAG KUNDEN JAHR])"
                    cmd.ExecuteNonQuery()
                    Me.BgwOCBSimport.ReportProgress(10, "Procedure:" & CURRENT_PROC & " - " & "Check if ZINSERTRAGSMONAT in Table ZINSERTRAG KUNDEN MONAT ")
                    cmd.CommandText = "INSERT INTO [ZINSERTRAG KUNDEN MONAT]
                                        ([Zinsertragsmonat]
                                        ,[IdZinsertragJahr]
                                        ,[ZinsertragsmonatDATE]) 
                                       SELECT DISTINCT 
                                        [IdZinsertragsMonat]
                                        ,[IdErtragJahr]
                                        ,DATEADD(month, DATEDIFF(month, 0, [ValDate]), 0) 
                                        from [#Temp_ZINSERTRAG_KUNDEN_DETAILS_TEMP] 
                                        where [IdZinsertragsMonat] not in (Select [Zinsertragsmonat] from [ZINSERTRAG KUNDEN MONAT])"
                    cmd.ExecuteNonQuery()
                    ...
                    'FÜR ZAHLUNGSVERKEHRSSTATISTIK
                    'In KUNDEN INTEREST ON ACCOUNT einfügen
                    cmd.CommandText = "INSERT INTO [KUNDEN INTEREST ON AC] 
                                        ([ValDate]
                                        ,[Account]
                                        ,[Contract]
                                        ,[Product]
                    ...
                    'CUSTOMER NAME-REGISTRATION COUNTRY EINFÜGEN
                    Me.BgwOCBSimport.ReportProgress(15, "Procedure:" & CURRENT_PROC & " - " & "Update Customer Registration country in Table: KUNDEN INTEREST ON AC from Table CUSTOMER_INFO")
                    cmd.CommandText = "UPDATE [KUNDEN INTEREST ON AC] 
                                        SET [CustomerName]=B.[English Name],[RegistrationCountry]=B.[COUNTRY_OF_REGISTRATION] 
                                        from [KUNDEN INTEREST ON AC] A  INNER JOIN [CUSTOMER_INFO] B ON A.[Customer]=B.[ClientNo]"
                    cmd.ExecuteNonQuery()
                    'LÖSCHEN WENN KEIN  KUNDENNAME GEFUNDEN WURDE
                    Me.BgwOCBSimport.ReportProgress(16, "Procedure:" & CURRENT_PROC & " - " & "Delete from Table:KUNDEN INTEREST ON AC if Customer Name is NULL")
                    cmd.CommandText = "DELETE  FROM [KUNDEN INTEREST ON AC] where [CustomerName] IS NULL"
                    ...
                    'cmd.CommandText = "UPDATE [KUNDEN INTEREST ON AC] SET [ExchangeRate]= (Select [MIDDLE RATE] from [EXCHANGE RATES OCBS] WHERE [KUNDEN INTEREST ON AC].[CCY]=[EXCHANGE RATES OCBS].[CURRENCY CODE] and [KUNDEN INTEREST ON AC].[ValDate]=[EXCHANGE RATES OCBS].[EXCHANGE RATE DATE]) where [ExchangeRate]=0"
                    'cmd.ExecuteNonQuery()
                    cmd.CommandText = "UPDATE A SET A.[ExchangeRate]=B.[MIDDLE RATE] FROM [KUNDEN INTEREST ON AC] A 
                                        INNER JOIN [EXCHANGE RATES OCBS] B ON A.[ValDate]=B.[EXCHANGE RATE DATE] 
                                        WHERE A.[CCY]=B.[CURRENCY CODE] and A.[ExchangeRate]=0"
                    cmd.ExecuteNonQuery()
                    'UMRECHNUNG
                    Me.BgwOCBSimport.ReportProgress(19, "Procedure:" & CURRENT_PROC & " - " & "Calculate all Amounts in EURO")
                    cmd.CommandText = "UPDATE [KUNDEN INTEREST ON AC] SET [AmountEuro]= [Amount]/[ExchangeRate] where [AmountEuro]=0 and [ExchangeRate]<>0"
                    ...
                    'In live Tabelle einfügen
                    Me.BgwOCBSimport.ReportProgress(25, "Procedure:" & CURRENT_PROC & " - " & "Insert Data to Table: ZINSERTRAG KUNDEN DETAILS ")
                    cmd.CommandText = "INSERT INTO [ZINSERTRAG KUNDEN DETAILS] 
                                        ([ValDateFrom]
                                        ,[ValDate]
                                        ,[Customer]
                                        ,[ValYear]
                    ...
                    'CUSTOMER NAME-REGISTRATION COUNTRY EINFÜGEN
                    Me.BgwOCBSimport.ReportProgress(26, "Procedure:" & CURRENT_PROC & " - " & "Update Customer Name and Registration country in Table: ZINSERTRAG KUNDEN DETAILS from Table CUSTOMER_INFO")
                    cmd.CommandText = "UPDATE [ZINSERTRAG KUNDEN DETAILS] SET [CustomerName]=B.[English Name],[RegistrationCountry]=B.[COUNTRY_OF_REGISTRATION] 
                                        from [ZINSERTRAG KUNDEN DETAILS] A INNER JOIN [CUSTOMER_INFO] B ON A.[Customer]=B.[ClientNo] 
                                        where B.[ClientType] NOT IN ('F - FINANCIAL')"
                    cmd.ExecuteNonQuery()
                    'ÜBERPRPFEBN VON Tabelle ZINSERTRAG KDBASIC ob Kunde Kapitalertragssteuerpflichtig ist
                    Me.BgwOCBSimport.ReportProgress(26, "Procedure:" & CURRENT_PROC & " - " & "Update Field: KAPISTPFLICHTIG in Table: ZINSERTRAG KUNDEN DETAILS from Table ZINSERTRAG KDBASIC")
                    cmd.CommandText = "UPDATE [ZINSERTRAG KUNDEN DETAILS] SET [KAPISTPFLICHTIG]=B.[KAPISTPFLICHTIG] from [ZINSERTRAG KUNDEN DETAILS] A 
                                       INNER JOIN [ZINSERTRAG KDBASIC] B ON A.[Customer]=B.[KDSTAMM]"
                    cmd.ExecuteNonQuery()
                    'LÖSCHEN WENN KEIN  KUNDENNAME GEFUNDEN WURDE
                    Me.BgwOCBSimport.ReportProgress(27, "Procedure:" & CURRENT_PROC & " - " & "Delete from Table: ZINSERTRAG KUNDEN DETAILS where Customer Name is NULL")
                    cmd.CommandText = "DELETE  FROM [ZINSERTRAG KUNDEN DETAILS] where [CustomerName] IS NULL"
                    ...
                    'cmd.CommandText = "UPDATE [ZINSERTRAG KUNDEN DETAILS] SET [ExchangeRate]= (Select [MIDDLE RATE] from [EXCHANGE RATES OCBS] WHERE [ZINSERTRAG KUNDEN DETAILS].[CCY]=[EXCHANGE RATES OCBS].[CURRENCY CODE] and [ZINSERTRAG KUNDEN DETAILS].[ValDate]=[EXCHANGE RATES OCBS].[EXCHANGE RATE DATE]) where [ExchangeRate]=0"
                    'cmd.ExecuteNonQuery()
                    cmd.CommandText = "UPDATE A SET A.[ExchangeRate]=B.[MIDDLE RATE] FROM [ZINSERTRAG KUNDEN DETAILS] A 
                                        INNER JOIN [EXCHANGE RATES OCBS] B ON A.[ValDate]=B.[EXCHANGE RATE DATE] 
                                        WHERE A.[CCY]=B.[CURRENCY CODE] and A.[ExchangeRate]=0"
                    cmd.ExecuteNonQuery()
                    'UMRECHNUNG
                    Me.BgwOCBSimport.ReportProgress(32, "Procedure:" & CURRENT_PROC & " - " & "Calculate all Amounts in EURO")
                    cmd.CommandText = "UPDATE [ZINSERTRAG KUNDEN DETAILS] SET [AmountEuro]= [Amount]/[ExchangeRate] 
                                       where [AmountEuro]=0 and [ExchangeRate]<>0"
                    cmd.ExecuteNonQuery()
                    'Duplikate Löschen 
                    Me.BgwOCBSimport.ReportProgress(33, "Procedure:" & CURRENT_PROC & " - " & "Delete Duplicates from ZINSERTRAG KUNDEN DETAILS - Field: IdValueCustomer")
                    cmd.CommandText = "DELETE  FROM [ZINSERTRAG KUNDEN DETAILS] where [ID] not in (Select Min([ID]) from [ZINSERTRAG KUNDEN DETAILS] group by [IdValueCustomer])"
                    ...
                    'Kapitalertragsteuer berechnen
                    Me.BgwOCBSimport.ReportProgress(34, "Procedure:" & CURRENT_PROC & " - " & "Calculate Kapitalertragssteuer in  ZINSERTRAG KUNDEN DETAILS - Parameter:MELDEWESEN/KAPITALERTRAGSTEUER_SATZ")
                    cmd.CommandText = "UPDATE [ZINSERTRAG KUNDEN DETAILS] SET [KapertstG]=FLOOR([AmountEuro]*(Select [NPARAMETER1] from [PARAMETER] where [PARAMETER1]='KAPITALERTRAGSTEUER_SATZ')) 
                                       where [KapertstG]=0"
                    cmd.ExecuteNonQuery()
                    'Kapitalertragssteuer auf null wenn KapitaSteuerbetrag<1
                    Me.BgwOCBSimport.ReportProgress(35, "Procedure:" & CURRENT_PROC & " - " & "Set Kapitalertragssteuer to 0 if Calculated Kapitalertragssteuer <1 in  ZINSERTRAG KUNDEN DETAILS")
                    ...
                    'Kapitalertragssteuer
                    Me.BgwOCBSimport.ReportProgress(38, "Procedure:" & CURRENT_PROC & " - " & "Recalculate Sum Kapitalertragssteuer and Soli in Table: ZINSERTRAG KUNDEN MONAT")
                    cmd.CommandText = "UPDATE A SET A.[SummeKapErSt]=B.K,A.SummeSoli=B.S from [ZINSERTRAG KUNDEN MONAT] A 
                                       INNER JOIN (Select Sum([KapertstG]) as K,Sum([Soli]) as S,[IdZinsertragsMonat] from [ZINSERTRAG KUNDEN DETAILS] 
                                       where [IdZinsertragsMonat]='" & ERTRAGSMONAT & "' and [KAPISTPFLICHTIG]='Y' 
                                       GROUP BY [IdZinsertragsMonat])B on A.Zinsertragsmonat=B.IdZinsertragsMonat"
                    cmd.ExecuteNonQuery()

                    'SUMMEN BERECHEN ERTRAGSJAHR
                    'Kapitalertragssteuer
                    Me.BgwOCBSimport.ReportProgress(39, "Procedure:" & CURRENT_PROC & " - " & "Recalculate Sum Kapitalertragssteuer and Soli in Table: ZINSERTRAG KUNDEN JAHR")
                    cmd.CommandText = "UPDATE A SET A.[SummeKapErSt]=B.K,A.SummeSoli=B.S from [ZINSERTRAG KUNDEN JAHR] A 
                                       INNER JOIN 
                                       (Select Sum([SummeKapErSt]) as K,Sum(SummeSoli) as S,[IdZinsertragJahr] from [ZINSERTRAG KUNDEN MONAT] where [IdZinsertragJahr]='" & ERTRAGSJAHR & "' 
                                       GROUP BY [IdZinsertragJahr])B on A.[ErtragsJahr]=B.[IdZinsertragJahr]"
                    cmd.ExecuteNonQuery()
                    ...
                    'Füllen der Tabelle ZINSERTRAG KDDETAIL
                    Me.BgwOCBSimport.ReportProgress(40, "Procedure:" & CURRENT_PROC & " - " & "Fill Table: ZINSERTRAG KDDETAIL")
                    cmd.CommandText = "INSERT INTO [ZINSERTRAG KDDETAIL]
                                        ([ValDateFrom]
                                        ,[ValDate]
                                        ,[Customer]
                                        ,[ValYear]
                                       ...
                                       SELECT
                                        ...
                                        ,[IdZinsertragsMonat]
                                        ,[IdErtragJahr]
                                        ,[Customer] FROM [ZINSERTRAG KUNDEN DETAILS]"
                    cmd.ExecuteNonQuery()
                    'Duplikate Löschen 
                    Me.BgwOCBSimport.ReportProgress(41, "Procedure:" & CURRENT_PROC & " - " & "Delete Duplicates in Table: ZINSERTRAG KDDETAIL")
                    cmd.CommandText = "DELETE  FROM [ZINSERTRAG KDDETAIL] where [ID] not in (Select Min([ID]) from [ZINSERTRAG KDDETAIL] 
                                       GROUP BY [IdValueCustomer])"
                    cmd.ExecuteNonQuery()
                    'Update Zinsertrag, Kapi-Soli basierend auf [ZINSERTRAG KUNDEN DETAILS]
                    Me.BgwOCBSimport.ReportProgress(41, "Procedure:" & CURRENT_PROC & " - " & "Update Data in ZINSERTRAG KDDETAIL based on [IdValueCustomer] from [ZINSERTRAG KUNDEN DETAILS]")
                    cmd.CommandText = "UPDATE A SET A.[ExchangeRate]=B.[ExchangeRate],A.[AmountEuro]=B.[AmountEuro]
                                       ,A.[KapertstG]=B.[KapertstG],A.[Soli]=B.[Soli],A.[KAPISTPFLICHTIG]=B.[KAPISTPFLICHTIG]  FROM [ZINSERTRAG KDDETAIL] A 
                                       INNER JOIN [ZINSERTRAG KUNDEN DETAILS] B ON A.[IdValueCustomer]=B.[IdValueCustomer]"
                    cmd.ExecuteNonQuery()
                    'Löschen in Tabelle ZINSERTRAG KDBASIC wenn keine Daten für IDSTAMM gibt
                    Me.BgwOCBSimport.ReportProgress(42, "Procedure:" & CURRENT_PROC & " - " & "Delete from Table: ZINSERTRAG KDBASIC where KDSTAMM not in Table ZINSERTRAG KDDETAIL")
                    cmd.CommandText = "DELETE  FROM [ZINSERTRAG KDBASIC] where [KDSTAMM] NOT IN (Select [IdKDSTAMM] from [ZINSERTRAG KDDETAIL])"
                    ...
                    '?????????????????NEUANLAUF KAPISTPFLICHTIG???????????????????
                    Me.BgwOCBSimport.ReportProgress(44, "Procedure:" & CURRENT_PROC & " - " & "Update KAPISTPFLICHTIG in Table: ZINSERTRAG KDBASIC from ZINSERTRAG KDDETAIL")
                    cmd.CommandText = "UPDATE [ZINSERTRAG KDBASIC] SET [KAPISTPFLICHTIG]='N' from [ZINSERTRAG KDBASIC] A 
                                       INNER JOIN [ZINSERTRAG KDDETAIL] B ON A.[KDSTAMM]=B.[Customer] where A.[KAPISTPFLICHTIG]='N'"
                    cmd.ExecuteNonQuery()
                    Me.BgwOCBSimport.ReportProgress(45, "Procedure:" & CURRENT_PROC & " - " & "Update KAPISTPFLICHTIG in Table: ZINSERTRAG KDBASIC from ZINSERTRAG KUNDEN DETAILS")
                    cmd.CommandText = "UPDATE [ZINSERTRAG KDBASIC] SET [KAPISTPFLICHTIG]='N' from [ZINSERTRAG KDBASIC] A 
                                       INNER JOIN [ZINSERTRAG KUNDEN DETAILS] B ON A.[KDSTAMM]=B.[Customer] where A.[KAPISTPFLICHTIG]='N'"
                    cmd.ExecuteNonQuery()
                    ...
                    Me.BgwOCBSimport.ReportProgress(48, "Procedure:" & CURRENT_PROC & " - " & "Recalculate Sum Kapitalertragssteuer in Table: ZINSERTRAG KUNDEN JAHR")
                    cmd.CommandText = "UPDATE A SET A.[SummeKapErSt]=B.K,A.SummeSoli=B.S from [ZINSERTRAG KUNDEN JAHR] A 
                                       INNER JOIN (Select Sum([SummeKapErSt]) as K,Sum(SummeSoli) as S,[IdZinsertragJahr] from [ZINSERTRAG KUNDEN MONAT] 
                                       where [IdZinsertragJahr]='" & ERTRAGSJAHR & "' GROUP BY [IdZinsertragJahr])B 
                                       on A.[ErtragsJahr]=B.[IdZinsertragJahr]"
                    cmd.ExecuteNonQuery()
        ...
        Try
            ...
            If cmd.ExecuteScalar > 0 Then
                ...
                If File.Exists(ExcelFileName) = True Then
                    ...
                    EXCELL.Visible = False
                    'Unmerge Cells
                    xlWorksheet1.Columns("A:AG").unMerge()


                    'Prüfen ob Daten vorhanden sind
                    'If xlWorksheet1.Range("T1").Value.ToString.StartsWith("Report ID:AI-D-249") = True Then
                    ...
                    If AVDate = "" Then
                        ...
                        cmd.ExecuteNonQuery()
                        Me.BgwOCBSimport.ReportProgress(9, "Procedure:" & CURRENT_PROC & " - " & "Reverse Debit Sign if Reversal Flag = Y")
                        cmd.CommandText = "UPDATE [ALL_VOLUMES] SET Amount=Amount*(-1) where [Reversal Flag] in ('Y') and [GL_Rep_Date]='" & rdsql & "'"
                        cmd.ExecuteNonQuery()
                    ...
                    Me.BgwOCBSimport.ReportProgress(6, "Procedure:" & CURRENT_PROC & " - " & "Check if booking entries for Bundesbank Interests are present")
                    cmd.CommandText = "SELECT [Status] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('Email_Bundesbank_Interests_Bookings') and [Id_SQL_Parameters] in ('SEVERAL SELECTIONS')"
                    Dim ParameterStatus As String = cmd.ExecuteScalar
                    If ParameterStatus = "Y" Then
                        Me.BgwOCBSimport.ReportProgress(9, "Procedure:" & CURRENT_PROC & " - " & "Start SEVERAL SELECTIONS/Email_Bundesbank_Interests_Bookings")
                        Me.QueryText = "Select * from [SQL_PARAMETER_DETAILS_SECOND]  where [Id_SQL_Parameters_Details] in (Select [ID] from SQL_PARAMETER_DETAILS where SQL_Name_1 in ('Email_Bundesbank_Interests_Bookings')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
                        da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                        ...
                        For i = 0 To dt.Rows.Count - 1
                            ...
                            If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                                Me.BgwOCBSimport.ReportProgress(3, "Procedure:" & CURRENT_PROC & " - " & "Execute SQL Commands in : " & dt.Rows.Item(i).Item("SQL_Name_1"))
                                cmd.ExecuteNonQuery()
                            End If
                        Next
                    End If
    ...
    Private Sub OCBS_IMPORT_CUSTOMER_VOLUMES()
        Try
            ...
            If cmd.ExecuteScalar > 0 Then
                ...
                Me.BgwOCBSimport.ReportProgress(6, "Procedure:" & CURRENT_PROC & " - " & "Delete Data from CUSTOMER VOLUMES where GL_Rep_Date=" & rd)
                cmd.CommandText = "DELETE FROM [CUSTOMER_VOLUMES] where [GL_Rep_Date]='" & rdsql & "'"
                cmd.ExecuteNonQuery()
                Me.BgwOCBSimport.ReportProgress(7, "Procedure:" & CURRENT_PROC & " - " & "Get last Closing Ledger Balance Date")
                'Maximum GL_Rep_Date definieren
                cmd.CommandText = "SELECT MAX([GL_Rep_Date]) from [CUSTOMER_VOLUMES] where BatchNo in ('CLOSING BALANCE') and [GL_Rep_Date]<'" & rdsql & "'"
