...
Public Class BaisExportAlphaV127
    ...
    Private Sub BaisExportAlphaV127_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub BaisExportAlphaV127_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If IsDate(Me.BusinessDate_Comboedit.Text) = True Then
            If DVTIFF_CheckEdit.CheckState = CheckState.Checked OrElse _
                ...
                If XtraMessageBox.Show("Should the BAIS Interface ALPHA Files creation be started?", "START BAIS INTERFACE ALPHA FILE CREATION", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = System.Windows.Forms.DialogResult.Yes Then
                    ...
                    'Check if latest BAIS Files Nr is equal with ODAS and NGS
        ...
        Try
            Using zip As ZipFile = New ZipFile
                For Each File In Directory.GetFiles(BaisFilesCreationPath, "*.CSV", SearchOption.TopDirectoryOnly)
                    zip.AddFile(File, "")
                Next
                zip.Save(BaisFilesCreationPath & "BAIS_Files_ALPHA_v1.27_from_PSTOOL_" & rdsql & ".rar")
                Dim BaisFilesList As String() = Directory.GetFiles(BaisFilesCreationPath, "*.CSV")
                For Each f In BaisFilesList
                    File.Delete(f)
                Next
            End Using
            Me.BaisExportEvents_RichTextBox.Text = Me.BaisExportEvents_RichTextBox.Text & vbNewLine & "File(s) zipped in " & "BAIS_Files_ALPHA_v1.27_from_PSTOOL_" & rdsql & ".rar"
        Catch ex As Exception
                    ...
                    If ParameterStatus = "Y" Then
                        ...
                        For i = 0 To dt.Rows.Count - 1
                            ...
                            If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                                BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_COMMON_INTERFACE_FILES_V1.27/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                                cmd.ExecuteNonQuery()
                            End If
                        Next
                    End If
