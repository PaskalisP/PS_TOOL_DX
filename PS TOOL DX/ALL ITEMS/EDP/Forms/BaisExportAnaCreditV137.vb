...
'Description:
'Modify log:  
'    1. Add by Papas; Time:24.05.2023; Content: BAIS system version is updated to Version 1.37. Add the BaisExportAnaCreditV134 class for Bais Version 1.37

'******************************************

Public Class BaisExportAnaCreditV137
    ...
    Private Sub BaisExportAnaCreditV137_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub BaisExportAnaCreditV137_Load(sender As Object, e As EventArgs) Handles Me.Load
...
#End Region
    ...
    Private Sub BgwAnaCreditDataCheck_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgwAnaCreditDataCheck.DoWork
        Try
            ...
            QueryText = "Select * from [SQL_PARAMETER_DETAILS_SECOND]  where [Id_SQL_Parameters_Details] in (Select [ID] from SQL_PARAMETER_DETAILS where SQL_Name_1 in ('BAIS_ANACREDIT_DATA_CHECK')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
            da = New SqlDataAdapter(QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
    ...
    Private Sub BgwAnaCreditDataCheck_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs) Handles BgwAnaCreditDataCheck.RunWorkerCompleted
        ...
        QueryText = "Select 'Result'=Case when (Select Count(ID) from AnaCreditCheckResults where CheckResult in ('N'))=0 then 'OK' else 'NO' END"
        da = New SqlDataAdapter(QueryText.Trim(), conn)
        dt = New System.Data.DataTable()
        da.Fill(dt)
        ...
        Try
            Using zip As ZipFile = New ZipFile
                For Each File In Directory.GetFiles(BaisFilesCreationPath, "*.CSV", SearchOption.TopDirectoryOnly)
                    zip.AddFile(File, "")
                Next
                zip.Save(BaisFilesCreationPath & "BAIS_Files_ALPHA_AnaCredit_v1.37_from_PSTOOL_" & rdsql & ".rar")
                Dim BaisFilesList As String() = Directory.GetFiles(BaisFilesCreationPath, "*.CSV")
                For Each f In BaisFilesList
                    File.Delete(f)
                Next
            End Using
            Me.BaisExportEvents_RichTextBox.Text = Me.BaisExportEvents_RichTextBox.Text & vbNewLine & "File(s) zipped in " & "BAIS_Files_ALPHA_AnaCredit_v1.37_from_PSTOOL_" & rdsql & ".rar"
        Catch ex As Exception
    ...
    Private Sub ACCPIF_CREATION()
        ...
        Dim ACCPIF_MITAE As String = Nothing 'New 1.37
        Dim ACCPIF_MIEST As String = Nothing 'New 1.37
        Dim ACCPIF_BILSE As String = Nothing 'New 1.37
        Dim ACCPIF_BIEST As String = Nothing 'New 1.37
        Dim ACCPIF_UMMIE As String = Nothing 'New 1.37
        Dim ACCPIF_UMEST As String = Nothing 'New 1.37
        ...
        If CountAnaCreditCustomers > 0 Then
            ...
            Try
                ...
                cmd.CommandText = "SELECT [Status] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37') and [Id_SQL_Parameters] in ('BAIS')"
                ParameterStatus = cmd.ExecuteScalar
                If ParameterStatus = "Y" Then
                    QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD] where Id_SQL_Parameters_Details in (Select ID from [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('BAIS_ACCPIF') and [Status] in ('Y') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37'))) and Status in ('Y') order by SQL_Float_1 asc"
                    da = New SqlDataAdapter(QueryText.Trim(), conn)
                    dt = New System.Data.DataTable()
                    da.Fill(dt)
                    For i = 0 To dt.Rows.Count - 1
                        ...
                        cmd.CommandText = SqlCommandText
                        If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                            BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_ANACREDIT_FILES_V1.37/ACCPIF/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                            cmd.ExecuteNonQuery()
                        End If
                    Next
                End If
                ...
                '++++++++++++++
                QueryText = "SELECT  * FROM  [BAIS_ACCPIF] where [ACCPIF_DXIFD]='" & rdsql & "'"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
                    ...
                    ACCPIF_MITAE = dt.Rows.Item(i).Item("ACCPIF_MITAE").ToString.Replace(",", ".") & "|"
                    ACCPIF_MIEST = dt.Rows.Item(i).Item("ACCPIF_MIEST") & "|"
                    ACCPIF_BILSE = dt.Rows.Item(i).Item("ACCPIF_BILSE").ToString.Replace(",", ".") & "|"
                    ACCPIF_BIEST = dt.Rows.Item(i).Item("ACCPIF_BIEST") & "|"
                    ACCPIF_UMMIE = dt.Rows.Item(i).Item("ACCPIF_UMMIE").ToString.Replace(",", ".") & "|"
                    ACCPIF_UMEST = dt.Rows.Item(i).Item("ACCPIF_UMEST") & "|"
    ...
    Private Sub ACFDIF_CREATION()
        ...
        Try
            ...
            ParameterStatus = cmd.ExecuteScalar
            If ParameterStatus = "Y" Then
                QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD] where Id_SQL_Parameters_Details in (Select ID from [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('BAIS_ACFDIF') and [Status] in ('Y') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37'))) and Status in ('Y') order by SQL_Float_1 asc"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
                    ...
                    cmd.CommandText = SqlCommandText
                    If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                        BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_ANACREDIT_FILES_V1.37/ACFDIF/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                        cmd.ExecuteNonQuery()
                    End If
                Next
            End If
            ...
            If Count > 0 Then
                ...
                QueryText = "SELECT * FROM  [BAIS_ACFDIF] where [ACFDIF_DXIFD]='" & rdsql & "'"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
    ...
    Private Sub ACFAIF_CREATION()
        ...
        Try
            ...
            ParameterStatus = cmd.ExecuteScalar
            If ParameterStatus = "Y" Then
                QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD] where Id_SQL_Parameters_Details in (Select ID from [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('BAIS_ACFAIF') and [Status] in ('Y') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37'))) and Status in ('Y') order by SQL_Float_1 asc"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
                    ...
                    cmd.CommandText = SqlCommandText
                    If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                        BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_ANACREDIT_FILES_V1.37/ACFAIF/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                        cmd.ExecuteNonQuery()
                    End If
                Next
            End If
            ...
            If Count <> 0 Then
                ...
                '++++++++++++++
                QueryText = "SELECT  * FROM  [BAIS_ACFAIF] where [ACFAIF_DXIFD]='" & rdsql & "'"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
    ...
    Private Sub ACIPIF_CREATION()
        ...
        Try
            ...
            ParameterStatus = cmd.ExecuteScalar
            If ParameterStatus = "Y" Then
                QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD] where Id_SQL_Parameters_Details in (Select ID from [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('BAIS_ACIPIF') and [Status] in ('Y') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37'))) and Status in ('Y') order by SQL_Float_1 asc"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
                    ...
                    cmd.CommandText = SqlCommandText
                    If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                        BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_ANACREDIT_FILES_V1.37/ACIPIF/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                        cmd.ExecuteNonQuery()
                    End If
                Next
            End If
            ...
            If Count <> 0 Then
                ...
                '++++++++++++++
                QueryText = "SELECT  * FROM  [BAIS_ACIPIF] where [ACFAIF_DXIFD]='" & rdsql & "'"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
    ...
    Private Sub ACCFIF_CREATION()
        ...
        ParameterStatus = cmd.ExecuteScalar
        If ParameterStatus = "Y" Then
            QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD] where Id_SQL_Parameters_Details in (Select ID from [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('BAIS_ACCFIF') and [Status] in ('Y') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37'))) and Status in ('Y') order by SQL_Float_1 asc"
            da = New SqlDataAdapter(QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
            For i = 0 To dt.Rows.Count - 1
                ...
                cmd.CommandText = SqlCommandText
                If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                    BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_ANACREDIT_FILES_V1.37/ACCFIF/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                    cmd.ExecuteNonQuery()
                End If
            Next
        End If
        ...
        If Count <> 0 Then
            Try
                ...
                '++++++++++++++
                QueryText = "SELECT * FROM  [BAIS_ACCFIF] where [ACCFIF_DXIFD]='" & rdsql & "'"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
    ...
    Private Sub ACIFIF_CREATION()
        ...
        ParameterStatus = cmd.ExecuteScalar
        If ParameterStatus = "Y" Then
            QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD] where Id_SQL_Parameters_Details in (Select ID from [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('BAIS_ACIFIF') and [Status] in ('Y') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37'))) and Status in ('Y') order by SQL_Float_1 asc"
            da = New SqlDataAdapter(QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
            For i = 0 To dt.Rows.Count - 1
                ...
                cmd.CommandText = SqlCommandText
                If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                    BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_ANACREDIT_FILES_V1.37/ACIFIF/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                    cmd.ExecuteNonQuery()
                End If
            Next
        End If
        ...
        If Count <> 0 Then
            Try
                ...
                '++++++++++++++
                QueryText = "SELECT * FROM  [BAIS_ACIFIF] where [ACIFIF_DXIFD]='" & rdsql & "'"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
    ...
    Private Sub ACPCIF_CREATION()
        ...
        ParameterStatus = cmd.ExecuteScalar
        If ParameterStatus = "Y" Then
            QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD] where Id_SQL_Parameters_Details in (Select ID from [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('BAIS_ACPCIF') and [Status] in ('Y') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37'))) and Status in ('Y') order by SQL_Float_1 asc"
            da = New SqlDataAdapter(QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
            For i = 0 To dt.Rows.Count - 1
                ...
                cmd.CommandText = SqlCommandText
                If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                    BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_ANACREDIT_FILES_V1.37/ACPCIF/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                    cmd.ExecuteNonQuery()
                End If
            Next
        End If
        ...
        If Count <> 0 Then
            Try
                ...
                '++++++++++++++
                QueryText = "SELECT * FROM  [BAIS_ACPCIF] where [ACPCIF_DXIFD]='" & rdsql & "'"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
    ...
    Private Sub ACPDIF_CREATION()
        ...
        ParameterStatus = cmd.ExecuteScalar
        If ParameterStatus = "Y" Then
            QueryText = "Select * from [SQL_PARAMETER_DETAILS_THIRD] where Id_SQL_Parameters_Details in (Select ID from [SQL_PARAMETER_DETAILS_SECOND] where  [SQL_Name_1] in ('BAIS_ACPDIF') and [Status] in ('Y') and [Id_SQL_Parameters_Details] in (SELECT [ID] FROM [SQL_PARAMETER_DETAILS] where  [SQL_Name_1] in ('BAIS_ANACREDIT_FILES_V1.37'))) and Status in ('Y') order by SQL_Float_1 asc"
            da = New SqlDataAdapter(QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
            For i = 0 To dt.Rows.Count - 1
                ...
                cmd.CommandText = SqlCommandText
                If dt.Rows.Item(i).Item("SQL_Name_1") <> "" Then
                    BgwBaisFilesCreation.ReportProgress(2, "Execute SQL Commands in BAIS_ANACREDIT_FILES_V1.37/ACPDIF/" & dt.Rows.Item(i).Item("SQL_Name_1"))
                    cmd.ExecuteNonQuery()
                End If
            Next
        End If
        ...
        If Count > 0 Then
            ...
            Try
                ...
                QueryText = "SELECT * FROM  [BAIS_ACPDIF] where [ACPDIF_DXIFD]='" & rdsql & "'"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
