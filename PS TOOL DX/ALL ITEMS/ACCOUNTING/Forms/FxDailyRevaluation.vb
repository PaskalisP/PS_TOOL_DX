...
Public Class FxDailyRevaluation
    ...
    Private Sub PrintableComponentLink2_CreateMarginalHeaderArea(sender As Object, e As CreateAreaEventArgs) Handles PrintableComponentLink2.CreateMarginalHeaderArea
        Dim reportfooter As String = "IDW + NGS FX Revaluation Differences on " & Me.FxDailyRevaluationDateEdit.Text
        e.Graph.DrawString(reportfooter, New RectangleF(0, 0, e.Graph.ClientPageSize.Width, 20))
    End Sub
    ...
    Private Sub TabbedControlGroup1_SelectedPageChanged(sender As Object, e As DevExpress.XtraLayout.LayoutTabPageChangedEventArgs) Handles TabbedControlGroup1.SelectedPageChanged
        If Me.TabbedControlGroup1.SelectedTabPage.Text = "FX DEALS - DAILY REVALUATION" Then
            ActiveTabGroup = "1"
        ElseIf Me.TabbedControlGroup1.SelectedTabPage.Text = "IDW + NGS FX Revaluation Differences" Then
            ActiveTabGroup = "2"
    ...
    Private Sub IDW_To_Excel_btn_Click(sender As Object, e As EventArgs) Handles IDW_To_Excel_btn.Click
        If IsDate(Me.FxDailyRevaluationDateEdit.Text) = True Then
            Try
                ...
                Dim RangeIDW_Q As CellRange = worksheet.Range("Q3:Q" & IDW_LastDtRow) 'Trade Date Buy Sell Amount
                RangeIDW_Q.formula = "=(H3/O3)-(J3/P3)"
                Dim RangeIDW_R As CellRange = worksheet.Range("R3:R" & IDW_LastDtRow) 'Amortization to Risk Date
                RangeIDW_R.Formula = "=(Q3/(F3-D3))*(($D$1-D3)+1)"
                Dim RangeIDW_S As CellRange = worksheet.Range("S3:S" & IDW_LastDtRow) 'Buy2Buy1Amount
                RangeIDW_S.Formula = "=(H3/M3)-(H3/O3)"
                Dim RangeIDW_T As CellRange = worksheet.Range("T3:T" & IDW_LastDtRow) 'Sell2Sell1Amount
                RangeIDW_T.Formula = "=(J3/N3)-(J3/P3)"
                Dim RangeIDW_U As CellRange = worksheet.Range("U3:U" & IDW_LastDtRow) 'IDW_Amount (Standard)
                RangeIDW_U.Formula = "=S3-T3"
                Dim RangeIDW_V As CellRange = worksheet.Range("V3:V" & IDW_LastDtRow) 'Difference IDW_Amount (Standard)
                RangeIDW_V.Formula = "=R3+U3-L3"
                Dim RangeIDW_AD As CellRange = worksheet.Range("AD3:AD" & IDW_LastDtRow) 'Datediff End Date - Next Woprking Date or First Date Next Month
                RangeIDW_AD.Formula = "=IF($D$1>=AC3;$AD$1-AC3;0)"
                Dim RangeIDW_AE As CellRange = worksheet.Range("AE3:AE" & IDW_LastDtRow) 'Datediff End Date - Maturity Date
                RangeIDW_AE.Formula = "=IF($D$1>=AC3;F3-AC3;0)"
                Dim RangeIDW_AF As CellRange = worksheet.Range("AF3:AF" & IDW_LastDtRow) 'Difference IDW_Amount (Internal)
