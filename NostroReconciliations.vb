...
Public Class NostroReconciliations
    ...
    Friend WithEvents BgwReconciliation_ReExecute As BackgroundWorker
    Friend WithEvents BgwReconciliation_Missing As BackgroundWorker
    ...
    Private Sub NostroReconciliations_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim objCMD As SqlCommand = New SqlCommand("SELECT * from NOSTRO_ACC_RECONCILIATIONS_ALL order by ReconcileDate desc", conn)
        ...
        Dim objCMD1 As SqlCommand = New SqlCommand("Select [CCY_IB],[AccountNr_Internal],[AccountName_Internal],AccountIdentifierStatement 
                                                    from [NOSTRO_ACC_RECONCILIATIONS_ALL] 
                                                    GROUP BY [CCY_IB],[AccountNr_Internal],[AccountName_Internal],AccountIdentifierStatement 
    ...
    Private Sub NostroReconciliations_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub NOSTRO_RECONCILIATIONS_FILL()
        ...
        Dim objCMD1 As SqlCommand = New SqlCommand("Select [BalanceDate] as 'BookingDate_IB',[Value Balance] as 'Amount_IB' 
                                                    from [NOSTRO BALANCES] where [BalanceDate]= 
                                                    CASE WHEN exists(Select [BalanceDate] from [NOSTRO BALANCES] 
                                                    where [Nostro Code]='" & Me.NostroAcc_SearchLookUpEdit.Text & "' 
                                                    and [BalanceDate]='" & rdsql & "') 
                                                    THEN  (Select [BalanceDate] from [NOSTRO BALANCES] 
                                                    where [Nostro Code]='" & Me.NostroAcc_SearchLookUpEdit.Text & "' and [BalanceDate]='" & rdsql & "') 
                                                    ELSE (SELECT MAX([BalanceDate]) AS second 
                                                    FROM   [NOSTRO BALANCES] WHERE  [BalanceDate] <= '" & rdsql & "' 
                                                    and [Nostro Code]='" & Me.NostroAcc_SearchLookUpEdit.Text & "') END 
                                                    and [Nostro Code]='" & Me.NostroAcc_SearchLookUpEdit.Text & "'", conn)
        objCMD1.CommandTimeout = 5000
        da = New SqlDataAdapter(objCMD1)
        dt = New DataTable()
        da.Fill(dt)
    ...
    Private Sub SWIFT_ACC_BALANCES_FILL()
        ...
        Try
            'Add by WYQ; Time: 06.10.2022; Content: Update External Balance for EURO interest rate up to +0.75% for account '90000902011'
            Dim objCMD4 As SqlCommand = New SqlCommand("select 'overnightBalance'= Case when (Select abs(sum(Amount)) as overnightBalance from SWIFT_ACC_STATEMENTS where ReceivedDate>='20220914' 
                                                        and ReceivedDate <='" & rdsql & "' and AccountIdentification='DEMARKPPCBCDEFFXXX' and TransactionTypeID='S204' 
                                                        and (SupplementaryDetails like '%MARKDEFFXXX%' or SupplementaryDetails like '%PCBCDEFFXXX%') 
                                                        and ReferenceAccountOwner not like 'AUT%') is not null then 
                                                        (Select abs(sum(Amount)) as overnightBalance from SWIFT_ACC_STATEMENTS where ReceivedDate>='20220914' 
                                                        and ReceivedDate <='" & rdsql & "' and AccountIdentification='DEMARKPPCBCDEFFXXX' and TransactionTypeID='S204' 
                                                        and (SupplementaryDetails like '%MARKDEFFXXX%' or SupplementaryDetails like '%PCBCDEFFXXX%') 
                                                        and ReferenceAccountOwner not like 'AUT%')
                                                        else 0
                                                        end", conn)
            objCMD4.CommandTimeout = 5000
            da4 = New SqlDataAdapter(objCMD4)
            dt4 = New DataTable()
            da4.Fill(dt4)
            Dim OvernightBalance As Double = 0
            If dt4 IsNot Nothing AndAlso dt4.Rows.Count > 0 AndAlso IsDBNull(dt4) = False Then
                OvernightBalance = CDbl(dt4.Rows.Item(0).Item("overnightBalance"))
            Else
                OvernightBalance = 0
            End If
            ...
            Dim objCMD2 As SqlCommand = New SqlCommand("SELECT [BookingDate],[Amount] FROM  [SWIFT_ACC_STATEMENTS] WHERE  [InternalAccount]= '" & Me.NostroAcc_SearchLookUpEdit.Text & "' 
                                                        AND [BookingDate] ='" & rdsql & "' and SwiftTag in ('62F') ", conn)
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                'Add by WYQ; Time: 06.10.2022; Content: Update External Balance for EURO interest rate up to +0.75% for account '90000902011'
                If Me.NostroAcc_SearchLookUpEdit.Text = "9000902011" Then
                    Me.ExternalBookedBalanceAmount_TextEdit.Text = FormatNumber(CDbl(dt.Rows.Item(0).Item("Amount")) + OvernightBalance, 2)
                End If
            ...
            Else
                ...
                Dim objCMD3 As SqlCommand = New SqlCommand("SELECT [BookingDate],[Amount] FROM  [SWIFT_ACC_STATEMENTS] WHERE  ([InternalAccount]= '" & Me.NostroAcc_SearchLookUpEdit.Text & "') 
                                                            AND [SwiftTag] in ('62F') AND ([BookingDate] = (SELECT MAX([BookingDate]) AS second  
                                                            FROM [SWIFT_ACC_STATEMENTS] where [InternalAccount]='" & Me.NostroAcc_SearchLookUpEdit.Text & "' 
                                                            and SwiftTag in ('62F') and  [BookingDate] < (SELECT MAX([BookingDate]) AS first 
                                                            FROM  [SWIFT_ACC_STATEMENTS] where [BookingDate]='" & rdsql & "' and SwiftTag in ('62F'))))", conn)
                ...
                If dt1 IsNot Nothing AndAlso dt1.Rows.Count > 0 Then
                    ...
                    'Add by WYQ; Time: 06.10.2022; Content: Update External Balance for EURO interest rate up to +0.75% for account '90000902011'
                    If Me.NostroAcc_SearchLookUpEdit.Text = "9000902011" Then
                        Me.ExternalBookedBalanceAmount_TextEdit.Text = FormatNumber(CDbl(dt1.Rows.Item(0).Item("Amount")) + OvernightBalance, 2)
                    End If
    ...
    Private Sub NostroAcc_SearchLookUpEdit_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles NostroAcc_SearchLookUpEdit.ButtonClick
        ...
        If e.Button.Caption = "Reload" Then
            ...
            SplashScreenManager.Default.SetWaitFormCaption("Reload all Nostro Reconciliations")
        ...
        If e.Button.Caption = "Restart" Then
            If Me.NostroAcc_SearchLookUpEdit.Text <> "" Then
                If XtraMessageBox.Show("Should the Reconciliation for Nostro Account: " & NostroAccount & " be re-executed for the last outstanding Bookings?", "MANUAL RECONCILIATION", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                    BgwReconciliation_ReExecute = New BackgroundWorker
                    bgws.Add(BgwReconciliation_ReExecute)
                    BgwReconciliation_ReExecute.WorkerSupportsCancellation = True
                    BgwReconciliation_ReExecute.WorkerReportsProgress = True
                    BgwReconciliation_ReExecute.RunWorkerAsync()

                    Me.LayoutControlGroup4.Visibility = LayoutVisibility.Never
                    Me.LayoutControlGroup3.Visibility = LayoutVisibility.Never
                    Me.Print_Export_btn.Enabled = False
                    Me.Reports_Dropdownbutton.Enabled = False
                    Me.NostroAcc_SearchLookUpEdit.Enabled = False
                    Me.GridControl_Internal.Enabled = False
                    Me.GridControl_External.Enabled = False
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    ...
                    Me.BgwReconciliation_ReExecute.ReportProgress(10, SqlEventText)
                    Dim SqlCommandText As String = dt.Rows.Item(i).Item("SQL_Command_1").ToString.Replace("<RiskDate>", rdsql)
                    Dim SqlCommandTextNew As String = SqlCommandText.ToString.Replace("<InternalNostroAccount>", NostroAccount)
    ...
    Private Sub BgwReconciliation_ReExecute_ProgressChanged(sender As Object, e As ProgressChangedEventArgs) Handles BgwReconciliation_ReExecute.ProgressChanged
        Me.ProgressPanel1.Caption = e.UserState.ToString
    End Sub

    Private Sub BgwReconciliation_ReExecute_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs) Handles BgwReconciliation_ReExecute.RunWorkerCompleted

        Try
            If e.Cancelled = True Then
                Workers_Complete(BgwReconciliation_ReExecute, e)
                Me.LayoutControlGroup6.Text = ""
                Me.LayoutControlGroup6.Visibility = LayoutVisibility.Never
                Me.LayoutControlGroup4.Visibility = LayoutVisibility.Always
                Me.LayoutControlGroup3.Visibility = LayoutVisibility.Always
                Me.Print_Export_btn.Enabled = True
                Me.Reports_Dropdownbutton.Enabled = True
                Me.NostroAcc_SearchLookUpEdit.Enabled = True
                Me.GridControl_Internal.Enabled = True
                Me.GridControl_External.Enabled = True
                Me.GridControl3.Enabled = True
                Me.GridControl4.Enabled = True
                Me.GridControl_OutstandingBookings.Enabled = True
            ...
            Else
                ...
                Me.LayoutControlGroup6.Visibility = LayoutVisibility.Never
                SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                SplashScreenManager.Default.SetWaitFormCaption("Load Data for Reporting Date: " & rd)
                NOSTRO_RECONCILIATIONS_FILL()
                SWIFT_ACC_BALANCES_FILL()

                'Balance difference (Internal-External)
                SplashScreenManager.Default.SetWaitFormCaption("Recalculate Balance Difference for Nostro Account: " & NostroAccount & "  on " & rd)
                ...
                SplashScreenManager.Default.SetWaitFormCaption("Recalculate Reconciliation Difference for Nostro Account: " & NostroAccount & "  on " & rd)
                ...
                If LastRecDate = rd Then
                    cmd.CommandText = "SELECT 'SUM_INTERNAL_OUTSTANDINGS'=Case when (Select Sum([Amount_IB]) FROM [NOSTRO_ACC_RECONCILIATIONS_OPEN] 
                                       where AccountNr_Internal='" & Me.NostroAcc_SearchLookUpEdit.Text & "' 
                                       and [BookingRoute_IB] in ('INTERNAL') and [ValueDate_IB]<='" & rdsql & "') is not NULL then (Select Sum([Amount_IB]) 
                                       FROM [NOSTRO_ACC_RECONCILIATIONS_OPEN] where AccountNr_Internal='" & Me.NostroAcc_SearchLookUpEdit.Text & "' 
                                       and [BookingRoute_IB] in ('INTERNAL') and [ValueDate_IB]<='" & rdsql & "') else 0 end"
                    Outstandings_Credits = cmd.ExecuteScalar
                    cmd.CommandText = "SELECT 'SUM_ALL_EXTERNAL_OUTSTANDINGS'=Case when (Select Sum([Amount_EB]) FROM [NOSTRO_ACC_RECONCILIATIONS_OPEN] 
                                       where AccountNr_Internal='" & Me.NostroAcc_SearchLookUpEdit.Text & "' and [BookingRoute_EB] in ('EXTERNAL')) is not NULL 
                                       then (Select Sum([Amount_EB]) FROM [NOSTRO_ACC_RECONCILIATIONS_OPEN] where AccountNr_Internal='" & Me.NostroAcc_SearchLookUpEdit.Text & "' 
                                       and [BookingRoute_EB] in ('EXTERNAL')) else 0 end"
    ...
    Private Sub NostroAcc_SearchLookUpEdit_EditValueChanged(sender As Object, e As EventArgs) Handles NostroAcc_SearchLookUpEdit.EditValueChanged
        If NostroAcc_SearchLookUpEdit.Text <> "" Then
            ...
            Me.FromDateEdit.Properties.Items.Clear()
            Me.QueryText = "Select Max(A.[RLDC Date]) as 'RLDC Date' from (Select Max([ReconcileDate]) as 'RLDC Date' from [NOSTRO_ACC_RECONCILIATIONS] 
                            where AccountNr_Internal='" & Me.NostroAcc_SearchLookUpEdit.Text & "' 
                            UNION ALL 
                            Select Max([ReconcileDate]) as 'RLDC Date' from [NOSTRO_ACC_RECONCILIATIONS_OPEN] 
                            where AccountNr_Internal='" & Me.NostroAcc_SearchLookUpEdit.Text & "')A" 'GROUP BY [ReconcileDate] ORDER BY [ReconcileDate] desc"
            da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
            dt = New System.Data.DataTable()
            da.Fill(dt)
            For Each row As DataRow In dt.Rows
...
#Region "CREATE INTERNAL DUMMY BOOKING for RECONCILIATION EXTERNAL BOOKING"
    ...
    Private Sub NostroOutstandingBookings_GridView_RowCellClick(sender As Object, e As RowCellClickEventArgs) Handles NostroOutstandingBookings_GridView.RowCellClick
        ID_MAIN = 0
        BOOKING_ROUTE_MAIN = Nothing
        BOOKING_ID_MAIN = Nothing
        DESCRIPTION_MAIN = Nothing
        BOOKING_DATE_MAIN = Nothing
        VALUE_DATE_MAIN = Nothing
        CUR_MAIN = Nothing
        AMOUNT_MAIN = 0
    ...
    Private Sub NostroOutstandingBookings_GridView_FocusedRowChanged(sender As Object, e As FocusedRowChangedEventArgs) Handles NostroOutstandingBookings_GridView.FocusedRowChanged
        ID_MAIN = 0
        BOOKING_ROUTE_MAIN = Nothing
        BOOKING_ID_MAIN = Nothing
        DESCRIPTION_MAIN = Nothing
    ...
    Private Sub CreateDummyBooking_ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles CreateDummyBooking_ToolStripMenuItem.Click
        ...
        If BOOKING_ROUTE_MAIN = "EXTERNAL" AndAlso AMOUNT_MAIN <> 0 Then
            If LastRecDate = rd Then
                If XtraMessageBox.Show("Should for this External Booking of " & vbNewLine & CUR_MAIN & "  " & FormatNumber(AMOUNT_MAIN, 2) & vbNewLine & "a dummy Internal booking be created  in order to reconcile?", "RECONCILIATION - CREATE INTERNAL DUMMY BOOKING", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                    SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
                    SplashScreenManager.Default.SetWaitFormCaption("Creating Internal dummy booking for the Amount of " & CUR_MAIN & "  " & FormatNumber(AMOUNT_MAIN * (-1), 2))
                    ...
                    cmd.CommandText = "INSERT INTO [NOSTRO_ACC_RECONCILIATIONS_OPEN]
                                      ...
                                      VALUES 
                                        ...
                                        ,'" & Lastrdsql & "'
                                        ,'" & CUR_MAIN & "'
                                        ," & Str(DUMMY_BOOKING_AMOUNT) & "
                                        ,'INTERNAL DUMMY BOOKING CREATED FOR RECONCILIATION'
                                        ,'" & Lastrdsql & "')"
                    cmd.ExecuteNonQuery()
            ...
            If NostroAccount_LastReconcile <> Nothing And IsDate(rdlr) = True Then
                ...
                If dt.Rows.Count > 0 Then
                    For i = 0 To dt.Rows.Count - 1
                        Dim SqlEventText As String = dt.Rows.Item(i).Item("SQL_Name_1").ToString
                        SplashScreenManager.Default.SetWaitFormCaption(SqlEventText)
                        Dim SqlCommandText As String = dt.Rows.Item(i).Item("SQL_Command_1").ToString.Replace("<RiskDate>", rdlrsql)
                        Dim SqlCommandTextNew As String = SqlCommandText.ToString.Replace("<InternalNostroAccount>", NostroAccount_LastReconcile)
                        cmd.CommandText = SqlCommandTextNew
                        cmd.ExecuteNonQuery()
                    Next i
    ...
    Private Sub CreateMissingRecon_btn_Click(sender As Object, e As EventArgs) Handles CreateMissingRecon_btn.Click
        ...
        If IsNothing(Me.NostrosForMissingRecon_SearchLookUpEdit.Text) = False AndAlso IsNumeric(Me.NostrosForMissingRecon_SearchLookUpEdit.Text) = True Then
            ...
            If ReconCount = 0 Then
                If XtraMessageBox.Show("Should the missing Reconciliation for Nostro Account: " & Me.NostrosForMissingRecon_SearchLookUpEdit.Text & " be executed?", "CREATION OF MISSING RECONCILIATION", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                    Try
                        ...
                        cmd.CommandText = "Select Sum(A.K) from (SELECT COUNT(*) as K FROM ALL_VOLUMES where GL_Rep_Date='" & mrdsql & "' and AccountNo='" & Me.NostrosForMissingRecon_SearchLookUpEdit.Text & "' UNION ALL Select COUNT(*) as K from [NOSTRO_ACC_RECON_NEW_EXTERNAL_BOOKINGS] where InternalAccount='" & Me.NostrosForMissingRecon_SearchLookUpEdit.Text & "')A"
                        If cmd.ExecuteScalar > 0 Then
                            Me.QueryText = "Select * from SQL_PARAMETER_DETAILS_SECOND where Id_SQL_Parameters_Details in (Select ID from SQL_PARAMETER_DETAILS where SQL_Name_1 in ('NOSTRO_RECONCILIATIONS_AUTO')) and Status in ('Y') order by SQL_Float_1 asc"
                            da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                            dt = New System.Data.DataTable()
                            da.Fill(dt)
                            If dt.Rows.Count > 0 Then
                                For i = 0 To dt.Rows.Count - 1
                                    ...
                                    Dim SqlCommandTextNew As String = SqlCommandText.ToString.Replace("<InternalNostroAccount>", Me.NostrosForMissingRecon_SearchLookUpEdit.Text)
                                    cmd.CommandText = SqlCommandTextNew
                                    cmd.ExecuteNonQuery()
                                Next i
                        ...
                        cmd.CommandText = "DELETE FROM NOSTRO_ACC_RECONCILIATIONS_ALL
                            TRUNCATE TABLE NOSTRO_ACC_RECONCILIATIONS_ALL
 
                            INSERT INTO NOSTRO_ACC_RECONCILIATIONS_ALL
                            (ReconcileDate
                            ,CCY_IB
                            ,AccountNr_Internal
                            ,AccountName_Internal
                            ,AccountIdentifierStatement
                            ,AllReference)
                            Select 
                            C.[ReconcileDate]
                            ,C.[CCY_IB]
                            ,C.[AccountNr_Internal]
                            ,C.[AccountName_Internal]
                            ,C.AccountIdentifierStatement
                            ,Convert(varchar(10)
                            ,C.[ReconcileDate],104)+C.[CCY_IB]+C.[AccountNr_Internal]  
                            from (Select A.[ReconcileDate] as 'ReconcileDate',A.[CCY_IB] as 'CCY_IB',A.[AccountNr_Internal] as 'AccountNr_Internal'
                            ,A.[AccountName_Internal] as 'AccountName_Internal',B.AccountIdentifierStatement as 'AccountIdentifierStatement' 
                            from [NOSTRO_ACC_RECONCILIATIONS] A INNER JOIN SSIS B on A.[AccountNr_Internal]=B.[INTERNAL ACCOUNT] 
                            GROUP BY A.[ReconcileDate],A.[CCY_IB],A.[AccountNr_Internal],A.[AccountName_Internal],B.AccountIdentifierStatement 
                            UNION ALL 
                            Select 
                            A.[ReconcileDate] as 'ReconcileDate'
                            ,A.[CCY_IB] as 'CCY_IB'
                            ,A.[AccountNr_Internal] as 'AccountNr_Internal' 
                            ,A.[AccountName_Internal] as 'AccountName_Internal'
                            ,B.AccountIdentifierStatement as 'AccountIdentifierStatement' 
                            from [NOSTRO_ACC_RECONCILIATIONS_OPEN] A INNER JOIN SSIS B on A.[AccountNr_Internal]=B.[INTERNAL ACCOUNT] 
                            GROUP BY A.[ReconcileDate],A.[CCY_IB],A.[AccountNr_Internal],A.[AccountName_Internal],B.AccountIdentifierStatement
                            UNION ALL 
                            Select A.[ReconcileDate] as 'ReconcileDate'
                            ,A.[CCY_IB] as 'CCY_IB'
                            ,A.[AccountNr_Internal] as 'AccountNr_Internal'
                            ,A.[AccountName_Internal] as 'AccountName_Internal'
                            ,B.AccountIdentifierStatement as 'AccountIdentifierStatement' 
                            from [NOSTRO_ACC_RECONCILIATIONS_OPEN_HISTORY] A INNER JOIN SSIS B on A.[AccountNr_Internal]=B.[INTERNAL ACCOUNT] 
                            GROUP BY A.[ReconcileDate],A.[CCY_IB],A.[AccountNr_Internal],A.[AccountName_Internal],B.AccountIdentifierStatement)C 
                            where C.[CCY_IB] is not NULL 
                            GROUP BY C.[ReconcileDate],C.[CCY_IB],C.[AccountNr_Internal],C.[AccountName_Internal],C.AccountIdentifierStatement 
                            ORDER BY C.[ReconcileDate] desc

                            DELETE from NOSTRO_ACC_RECONCILIATIONS_ALL where ID not in (Select Min(ID) from NOSTRO_ACC_RECONCILIATIONS_ALL GROUP BY AllReference)"
                        cmd.ExecuteNonQuery()
