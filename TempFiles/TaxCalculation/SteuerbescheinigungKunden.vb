...
Public Class SteuerbescheinigungKunden
    ...
    Dim SteuerYear As String = Nothing
    Dim SteuerCustomerNr As String = Nothing
    Dim ZinsertragDetails_BasicView_Caption As String = Nothing
    ...
    Dim focusedView As GridView = Nothing
    ...
    Private Sub Reload_btn_Click(sender As Object, e As EventArgs) Handles Reload_btn.Click
        SplashScreenManager.ShowForm(Me, GetType(WaitForm1), True, True, False)
        SplashScreenManager.Default.SetWaitFormCaption("Reload all data")
        Me.ZINSERTRAG_KDDETAILTableAdapter.Fill(Me.AccountingDataSet.ZINSERTRAG_KDDETAIL)
        Me.ZINSERTRAG_KDBASICTableAdapter.Fill(Me.AccountingDataSet.ZINSERTRAG_KDBASIC)
        Me.BANKTableAdapter.Fill(Me.AccountingDataSet.BANK)
    ...
    Private Sub PrintableComponentLink1_CreateMarginalHeaderArea(sender As Object, e As CreateAreaEventArgs) Handles PrintableComponentLink1.CreateMarginalHeaderArea
        If Me.GridControl1.FocusedView.Name = "ZinsertragDetails_BasicView" Then
            Dim reportfooter As String = ZinsertragDetails_BasicView_Caption
    ...
    Private Sub dxOK_ALL_JAHR_click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If cmd.Connection.State = ConnectionState.Closed Then
            cmd.Connection.Open()
        End If
        Dim MM_BASIC_CUSTOMER_Da As New SqlDataAdapter("SELECT * FROM [ZINSERTRAG KDBASIC] where [KAPISTPFLICHTIG] in ('Y') and [KDSTAMM] in (Select [Customer] from [ZINSERTRAG KDDETAIL] where [IdErtragJahr]='" & ZinsJahr & "' and [KapertstG]<>0 and [Steuerbescheinigung_Jahr] in ('Y')) ", conn)
        Dim MM_DETAIL_CUSTOMER_Da As New SqlDataAdapter("SELECT * FROM [ZINSERTRAG KDDETAIL] where [IdErtragJahr]='" & ZinsJahr & "' and [KapertstG]<>0 and [Steuerbescheinigung_Jahr] in ('Y') ", conn)
        Dim General_Dataset As New DataSet("MM_REPORT")
        MM_BASIC_CUSTOMER_Da.Fill(General_Dataset, "ZINSERTRAG KDBASIC")
        MM_DETAIL_CUSTOMER_Da.Fill(General_Dataset, "ZINSERTRAG KDDETAIL")
        If cmd.Connection.State = ConnectionState.Open Then
        ...
        End If
        '+++++++++++++++++++++++++++++++++++++++++++++++
        If General_Dataset.Tables(0).Rows.Count > 0 Then
            Dim CrRep As New ReportDocument
        ...
        Else
            SplashScreenManager.CloseForm(False)
            XtraMessageBox.Show("Keine Daten für das Jahr: " & ZinsJahr & " vorhanden", "KUNDEN STEUERBESCHEINIGUNGEN KÖNNEN NICHT ERSTELLT WERDEN", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1)
            Exit Sub
        End If
    End Sub
    ...
    Private Sub SteuerBesch_EinzelnerKundeJahr_BarButtonItem_ItemClick(sender As Object, e As ItemClickEventArgs) Handles SteuerBesch_EinzelnerKundeJahr_BarButtonItem.ItemClick
        If KundenZinsenOhnePlzBundesland = 0 Then
            If KundenZinsenPLZ_EAEG = 0 Then
                ...
                DATESFORM_ACC_JAHR_SINGLE.LabelControl3.Visible = False
                DATESFORM_ACC_JAHR_SINGLE.ComboBoxEdit2.Visible = True
                DATESFORM_ACC_JAHR_SINGLE.ComboBoxEdit3.Visible = False
                ...
                Me.QueryText = "SELECT [IdKDSTAMM],[CustomerName] from [ZINSERTRAG KDDETAIL] where [KAPISTPFLICHTIG]='Y' 
                                GROUP BY [IdKDSTAMM],[CustomerName] ORDER BY [CustomerName] asc"
                da = New SqlDataAdapter(Me.QueryText.Trim(), conn)
                dt = New DataTable()
                da.Fill(dt)
                For Each row As DataRow In dt.Rows
    ...
    Private Sub dxOK_SINGLE_JAHR_click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If cmd.Connection.State = ConnectionState.Closed Then
            cmd.Connection.Open()
        End If
        Dim MM_BASIC_CUSTOMER_Da As New SqlDataAdapter("SELECT * FROM [ZINSERTRAG KDBASIC] where [KAPISTPFLICHTIG] in ('Y') and [KDSTAMM]='" & Kunde & "' and [KDSTAMM] in (Select [Customer] from [ZINSERTRAG KDDETAIL] where [IdErtragJahr]='" & ZinsJahr & "' and [KapertstG]<>0 and [Steuerbescheinigung_Jahr] in ('Y')) ", conn)
        Dim MM_DETAIL_CUSTOMER_Da As New SqlDataAdapter("SELECT * FROM [ZINSERTRAG KDDETAIL] where [Customer] ='" & Kunde & "' and [IdErtragJahr]='" & ZinsJahr & "' and [KapertstG]<>0 and [Steuerbescheinigung_Jahr] in ('Y') ", conn)
        Dim General_Dataset As New DataSet("MM_REPORT")
        MM_BASIC_CUSTOMER_Da.Fill(General_Dataset, "ZINSERTRAG KDBASIC")
        MM_DETAIL_CUSTOMER_Da.Fill(General_Dataset, "ZINSERTRAG KDDETAIL")
        If cmd.Connection.State = ConnectionState.Open Then
        ...
        Else
            SplashScreenManager.CloseForm(False)
            XtraMessageBox.Show("Keine Daten für die Kunden-Nr.: " & Kunde & " für das Jahr: " & ZinsJahr & " vorhanden!" & vbNewLine & "Bitte prüfen ob die Erstellung der Steuerbescheinigung für das Jahr: " & ZinsJahr & " deaktiviert wurde!", "KUNDEN STEUERBESCHEINIGUNG KANN NICHT ERSTELLT WERDEN", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1)
            Exit Sub
        End If
    ...
    Private Sub ZinsertragDetails_BasicView_PopupMenuShowing(sender As Object, e As PopupMenuShowingEventArgs) Handles ZinsertragDetails_BasicView.PopupMenuShowing
        If e.MenuType = GridMenuType.GroupRow Then
            Dim ColumnMenu As DevExpress.XtraGrid.Menu.GridViewGroupRowMenu = CType(e.Menu, DevExpress.XtraGrid.Menu.GridViewGroupRowMenu)

            Dim menuItem_DisplayAll As New DevExpress.Utils.Menu.DXMenuItem("AKTIVE JÄHRLICHE STEUERBESCHEINIGUNG", New EventHandler(AddressOf MyMenuItem_AktiveSteuerbescheinigung), ImageCollection1.Images.Item(5))
            Dim menuItem_DisplayDefault As New DevExpress.Utils.Menu.DXMenuItem("KEINE JÄHRLICHE STEUERBESCHEINIGUNG", New EventHandler(AddressOf MyMenuItem_InaktiveSteuerbescheinigung), ImageCollection1.Images.Item(11))
    ...
    Private Sub MyMenuItem_AktiveSteuerbescheinigung(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If XtraMessageBox.Show("Soll die Erstellung der Steuerbescheinigung für das Jahr:  " & SteuerYear & " aktiviert werden?", "AKTIVIERUNG JÄHRLICHE STEUERBESCHEINIGUNG", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
            If cmd.Connection.State = ConnectionState.Closed Then
                cmd.Connection.Open()
            End If
            cmd.CommandText = "UPDATE [ZINSERTRAG KDDETAIL] SET [Steuerbescheinigung_Jahr]='Y' where [ValYear]='" & SteuerYear & "' and [Customer]='" & SteuerCustomerNr & "'"
            cmd.ExecuteNonQuery()
            If cmd.Connection.State = ConnectionState.Open Then
                cmd.Connection.Close()
            End If
    ...
    Private Sub MyMenuItem_InaktiveSteuerbescheinigung(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If XtraMessageBox.Show("Soll die Erstellung der Steuerbescheinigung für das Jahr: " & SteuerYear & " deaktiviert werden?", "DEAKTIVIERUNG JÄHRLICHE STEUERBESCHEINIGUNG", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
            If cmd.Connection.State = ConnectionState.Closed Then
                cmd.Connection.Open()
            End If
            cmd.CommandText = "UPDATE [ZINSERTRAG KDDETAIL] SET [Steuerbescheinigung_Jahr]='N' where [ValYear]='" & SteuerYear & "' and [Customer]='" & SteuerCustomerNr & "'"
            cmd.ExecuteNonQuery()
            If cmd.Connection.State = ConnectionState.Open Then
                cmd.Connection.Close()
            End If
    ...
    Private Sub ZinsertragDetails_BasicView_RowCellClick(sender As Object, e As RowCellClickEventArgs) Handles ZinsertragDetails_BasicView.RowCellClick

        Dim view = TryCast(sender, GridView)
        Dim rowHandle As Integer = view.FocusedRowHandle
        If view.IsGroupRow(rowHandle) Then
            SteuerYear = view.GetGroupRowValue(rowHandle).ToString()
            Dim i As Integer = view.GetChildRowHandle(rowHandle, 0).ToString
            SteuerCustomerNr = view.GetRowCellValue(i, colCustomer).ToString
        Else
            'Me.Text = view.GetGroupRowValue(view.GetParentRowHandle(e.FocusedRowHandle)).ToString()
        End If


    End Sub

    Private Sub ZinsertragDetails_BasicView_FocusedRowChanged(sender As Object, e As FocusedRowChangedEventArgs) Handles ZinsertragDetails_BasicView.FocusedRowChanged

        Dim view = TryCast(sender, GridView)
        If view.IsGroupRow(e.FocusedRowHandle) Then
            SteuerYear = view.GetGroupRowValue(e.FocusedRowHandle).ToString()
            Dim i As Integer = view.GetChildRowHandle(e.FocusedRowHandle, 0).ToString
            SteuerCustomerNr = view.GetRowCellValue(i, colCustomer).ToString
        Else
            'Me.Text = view.GetGroupRowValue(view.GetParentRowHandle(e.FocusedRowHandle)).ToString()
        End If
    ...
    Private Sub Zinsertrag_BasicView_MasterRowExpanded(sender As Object, e As CustomMasterRowEventArgs) Handles Zinsertrag_BasicView.MasterRowExpanded
        If Me.GridControl1.FocusedView.Name = "Zinsertrag_BasicView" Then

            Dim view As GridView = DirectCast(sender, GridView)
            ZinsertragDetails_BasicView_Caption = "Details der Zinserträge für Kunden: " & view.GetFocusedRowCellValue("KDNAME1").ToString & " - Kunden-Nr.: " & view.GetFocusedRowCellValue("KDSTAMM").ToString
            Me.ZinsertragDetails_BasicView.ViewCaption = ZinsertragDetails_BasicView_Caption
            Me.ZinsertragDetails_BasicView.BestFitColumns()

        End If
    End Sub

    Private Sub Zinsertrag_BasicView_MasterRowExpanding(sender As Object, e As MasterRowCanExpandEventArgs) Handles Zinsertrag_BasicView.MasterRowExpanding
        If Me.GridControl1.FocusedView.Name = "Zinsertrag_BasicView" Then
