...
Public Class ZvStatistic_NewFraudelentCard
    ...
    Public Sub ZvStatistic_NewFraudelentCard_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Private Sub ALL_ZV_MELDESCHEMAS_initData()
        Dim objCMD1 As SqlCommand = New SqlCommand("SELECT B.PositionNr as PositionNr,B.PositionNr + ' - ' + A.PositionName as PositionName from ZVSTAT_Parameters_from2014 A INNER JOIN
                                                    ...
                                                    in ('ZVSTA_2022_H') and FormNr in ('ZVS5.2')
    ...
    Private Sub ZV_COUNTRIES_InitLookUp()
        Me.AcquirerZvCountry_SearchLookUpEdit.Properties.DataSource = BS_All_ZvCountries
        Me.AcquirerZvCountry_SearchLookUpEdit.Properties.DisplayMember = "CountryName"
        Me.AcquirerZvCountry_SearchLookUpEdit.Properties.ValueMember = "CountryCode"

        Me.ReceiverZvCountry_SearchLookUpEdit1.Properties.DataSource = BS_All_ZvCountries
        Me.ReceiverZvCountry_SearchLookUpEdit1.Properties.DisplayMember = "CountryName"
        Me.ReceiverZvCountry_SearchLookUpEdit1.Properties.ValueMember = "CountryCode"
    ...
    Public Sub ZVSTA_ADD_FRAUDELENT_CARDS(ByVal ZVSTA_MELDEPOSITION As String, ByVal ZVSTA_COUNTRY_A As String, ByVal ZVSTA_COUNTRY_B As String, ByVal ZVSTA_LANDKONTEXT As String, ByVal ZVSTA_MELDESCHEMA As String, ByVal ZVSTA_MELDEPERIODE As String)
        Try

            SplashScreenManager.ShowForm(Me.ZvReporting, GetType(WaitForm1), False, False, True, SplashFormStartPosition.Default)
            SplashScreenManager.Default.SetWaitFormCaption("Start adding ZVSTA Positions for Fraudelent cards with Schema " & ZVSTA_MELDESCHEMA & " reporting Period: " & ZVSTA_MELDEPERIODE & " and Acquirer Country Code: " & ZVSTA_COUNTRY_A & " and Receiver Country Code: " & ZVSTA_COUNTRY_B & " and Landkontext: " & ZVSTA_LANDKONTEXT)
            System.Threading.Thread.Sleep(ExecutingSQLThreadSleep)

            SplashScreenManager.Default.SetWaitFormCaption("Execute SQL Parameter: ZV_STAT\ZVSTAT_HALFYEARLY_ADD_FRAUDELENT_CARDS_TRANSACTIONS")
            System.Threading.Thread.Sleep(ExecutingSQLThreadSleep)
            cmd.CommandText = "Select [Status] FROM SQL_PARAMETER_DETAILS where  [SQL_Name_1] In ('ZVSTAT_HALFYEARLY_ADD_FRAUDELENT_CARDS_TRANSACTIONS') and [Id_SQL_Parameters] in ('ZV_STAT')"
            Dim ParameterStatus As String = cmd.ExecuteScalar.ToString
            If ParameterStatus = "Y" Then
                QueryText = "Select * from SQL_PARAMETER_DETAILS_SECOND  where [Id_SQL_Parameters_Details] in (Select [ID] from SQL_PARAMETER_DETAILS where SQL_Name_1 in ('ZVSTAT_HALFYEARLY_ADD_FRAUDELENT_CARDS_TRANSACTIONS')) and [SQL_Command_1] is not NULL  and [Status] in ('Y') order by [SQL_Float_1] asc"
                da = New SqlDataAdapter(QueryText.Trim(), conn)
                dt = New System.Data.DataTable()
                da.Fill(dt)
                For i = 0 To dt.Rows.Count - 1
                    SqlCommandText = dt.Rows.Item(i).Item("SQL_Command_1").ToString.Replace("<ZV_MELDESCHEMA>", ZVSTA_MELDESCHEMA).ToString.Replace("<ZV_MELDEPERIODE>", ZVSTA_MELDEPERIODE).ToString.Replace("<ZV_MELDEPOSITION>", ZVSTA_MELDEPOSITION).ToString.Replace("<ZV_COUNTRY_A>", ZVSTA_COUNTRY_A).ToString.Replace("<ZV_COUNTRY_B>", ZVSTA_COUNTRY_B).ToString.Replace("<ZV_LANDKONTEXT>", ZVSTA_LANDKONTEXT)
                    ...
                    If dt.Rows.Item(i).Item("SQL_Name_1").ToString <> "" Then
                        ...
                        cmd.ExecuteNonQuery()
                        'Add to Events
                        cmd.CommandText = "INSERT INTO [Events_Journal] ([ProcDate],[Event],[ProcName],[SystemName],[SystemUser]) Values(GETDATE(),@Event,'ZVSTAT_HALFYEARLY_ADD_FRAUDELENT_CARDS_TRANSACTIONS','ZV_STATISTIK','" & CurrentUserWindowsID & "')"
